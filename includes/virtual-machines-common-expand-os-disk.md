...
## <a name="for-resizing-data-disks"></a>Изменение размера дисков данных
В этой статье основное внимание уделено увеличению неуправляемого или управляемого диска ОС виртуальной машины. Но разработанный сценарий можно также использовать для увеличения дисков данных, подключенных к виртуальной машине. Например, чтобы увеличить первый диск с данными, подключенный к виртуальной машине, замените объект ```OSDisk``` в ```StorageProfile``` на массив ```DataDisks``` и используйте числовой индекс для получения ссылки на первый подключенный диск с данными, как показано ниже:
