...
| Пиринговых связей между виртуальными сетями на виртуальную сеть |50** |100 |
| DNS-серверов на виртуальную сеть |9 |25 |
| Частные IP-адреса на виртуальную сеть |16 384** |16 384 |
| Частные IP-адреса на сетевой интерфейс |256 |256 |
| Параллельные потоки TCP или UDP для каждого сетевого адаптера виртуальной машины или экземпляра роли |500K |500K |
| Сетевые карты (NIC) |24 000** |24 000 |
...
| Группы безопасности приложений |500 |3000 |
...
** Эти обновленные ограничения по умолчанию применяются к подпискам, для которых эти ограничения ранее не повышались по запросу в службу поддержки. Если служба поддержки уже подняла для вас эти ограничения в прошлом и вы хотите обновить их до новых значений по умолчанию, [откройте запрос в службу поддержки (бесплатно)](../articles/azure-resource-manager/resource-manager-quota-errors.md).
