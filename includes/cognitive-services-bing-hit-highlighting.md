...
В следующем примере показан ответ `Computation`, содержащий метки индекса по поисковому запросу log(2). `expression`Поле содержит маркеры, только если `textDecoration` имеет значение **true**.
