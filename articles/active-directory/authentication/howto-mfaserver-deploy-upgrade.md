...
1. Следуйте указаниям в разделе, посвященном [загрузке сервера Многофакторной идентификации Azure](howto-mfaserver-deploy.md#download-the-mfa-server), чтобы скачать последнюю версию сервера Azure MFA.
...
- Настройте для своих приложений проверку подлинности Windows, как описано в статье [Проверка подлинности Windows и сервер Azure Multi-Factor Authentication](howto-mfaserver-windows.md).
