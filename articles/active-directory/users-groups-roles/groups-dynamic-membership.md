---
title: Правила динамического членства в группах на основе атрибутов в Azure Active Directory | Документы Майкрософт
...
ms.locfileid: "39248895"
---
# <a name="create-dynamic-groups-with-attribute-based-membership-in-azure-active-directory"></a>Создание динамических групп с членством на основе атрибутов в Azure Active Directory

В Azure Active Directory (Azure AD) можно создать сложные, основанные на атрибутах правила динамического членства в группах. В этой статье подробно описываются атрибуты и синтаксис для создания правил динамического членства пользователей или устройств. Вы можете настроить правило динамического членства для групп безопасности или групп Office 365.
...
3. Выберите **Все группы**, а затем — **Новая группа**.
   ...
   ![Добавление новой группы](./media/groups-dynamic-membership/new-group-creation.png)
...
4. В колонке **Группа** введите имя и описание новой группы. Для параметра **Тип членства** выберите значение **Динамический пользователь** или **Динамическое устройство** в зависимости от того, требуется создать правило для пользователей либо устройств, а затем щелкните **Добавить динамический запрос**. Вы можете использовать конструктор правил, чтобы создать простое правило, или самостоятельно создать расширенное правило. Эта статья содержит дополнительные сведения о доступных атрибутах пользователей и устройств, а также примеры расширенных правил.
   ...
   ![Добавление правила динамического членства](./media/groups-dynamic-membership/add-dynamic-group-rule.png)
...
## <a name="status-of-the-dynamic-rule"></a>Состояние динамического правила

Вы можете просматривать состояние обработки членства и дату последнего обновления на странице "Обзор" для динамической группы.
  
  ![Отображение состояния динамической группы](./media/groups-dynamic-membership/group-status.png)
...
![сообщение об ошибке обработки](./media/groups-dynamic-membership/processing-error.png)
...
Настраиваемые атрибуты синхронизируются из локального каталога Windows Server AD или подключенного приложения SaaS в формате user.extension_[GUID]\__[Attribute], где [GUID] — уникальный идентификатор в Azure AD для приложения, создавшего атрибут в AAD, а [Attribute] — имя атрибута, присвоенное при создании. Пример правила, которое использует настраиваемый атрибут:
...
1. На странице **Свойства** выбранной группы щелкните **Тип членства** > **Динамический пользователь**, затем выберите "Да" в диалоговом окне, объясняющем изменения в членстве в группе, чтобы продолжить. 
   ...
   ![Выбор типа членства "Динамический пользователь"](./media/groups-dynamic-membership/select-group-to-convert.png)
  
2. Выберите **Добавить динамический запрос**, а затем укажите правило.
  
   ![Ввод правила](./media/groups-dynamic-membership/enter-rule.png)
