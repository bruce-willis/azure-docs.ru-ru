...
Предполагается знание [однотенантных](quickstart-v1-integrate-apps-with-azure-ad.md) и [мультитенантных](active-directory-devhowto-multi-tenant-overview.md) приложений, а также [распространенных шаблонов аутентификации](authentication-scenarios.md).
...
В этом сценарии рассматривается вариант, при котором собственное приложение вызывает веб-службу или API. Эта служба, в свою очередь, выполняет [поток On-Behalf-Of](authentication-scenarios.md#application-types-and-scenarios) для вызова подчиненной службы. В нашем случае применяется политика условного доступа к подчиненной службе (веб-API 2) и используется собственное приложение, а не серверное приложение или управляющая программа. 
