...
5.  В колонке **Тестирование единого входа** введите корпоративные учетные данные, чтобы войти в целевое приложение. Вы можете выполнить вход как текущий пользователь или как другой пользователь. Если вы войдете от имени другого пользователя, откроется окно проверки подлинности.
    ...
    ![Страница тестирования SAML](media/active-directory-saml-debugging/testing.png)
...
![Ошибка входа](media/active-directory-saml-debugging/error.png)
...
3. Обратите внимание на следующие элементы в маркере ответа SAML:
    ...
    - Сертификат, использованный для подписи маркера. Сведения о том, как получить ответ SAML, см. в статье [Протокол единого входа SAML](active-directory-single-sign-on-protocol-reference.md).
4. Дополнительные сведения об ответе SAM см. в статье [Протокол единого входа SAML](active-directory-single-sign-on-protocol-reference.md).
...
Теперь, когда единый вход в приложение работает, перейдите к следующим разделам — [Автоматическая подготовка пользователей и ее отзыв для приложений SaaS](../active-directory-saas-app-provisioning.md) или [Начало работы с условным доступом](../active-directory-conditional-access-azure-portal-get-started.md).
