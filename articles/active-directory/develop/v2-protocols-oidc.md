...
Вы можете выбрать проверку маркера идентификации в клиентском коде, но обычно этот маркер отправляется для проверки на внутренний сервер. После проверки подписи маркера идентификации потребуется проверить несколько утверждений. Дополнительные сведения, включая описание [проверки маркеров](v2-id-and-access-tokens.md#validating-tokens) и [важную информацию о смене ключей подписывания](v2-id-and-access-tokens.md#validating-tokens), см. в [справочнике по маркерам версии 2.0](v2-id-and-access-tokens.md). Рекомендуется использовать библиотеку для анализа и проверки маркеров. Для большинства языков и платформ доступна по крайней мере одна такая библиотека.
...
Дополнительные сведения об утверждениях в маркере идентификации можно найти в [справочнике по маркерам конечной точки версии 2.0](v2-id-and-access-tokens.md).

После проверки маркера идентификации можно запустить сеанс пользователя. Используйте утверждения в маркере идентификации для получения сведений о пользователе в вашем приложении. Эти сведения можно использовать для отображения, записей, авторизации и т. д.
...
После получения кода авторизации и маркера идентификации можно выполнить вход пользователя в систему и получить маркеры доступа от его имени. Для входа пользователя необходимо проверить маркер идентификации [в точности, как описано выше](#validate-the-id-token). Для получения маркеров доступа следует выполнить действия, описанные в [документации по протоколу OAuth](v2-oauth2-auth-code-flow.md#request-an-access-token).
