...
ms.locfileid: "39596506"
---
# <a name="azure-active-directory-v20-tokens-reference"></a>Справочник маркерам Azure Active Directory версии 2.0
При обработке каждого [потока аутентификации](v2-app-types.md) конечная точка Azure Active Directory (Azure AD) версии 2.0 выдает маркеры безопасности различных типов. В этом справочнике описывается формат, характеристики безопасности и содержимое маркера каждого типа.
...
> [!TIP]
> Чтобы в целях обучения изучить утверждения в примере маркера идентификации, вставьте его в файл [jwt.ms](http://jwt.ms/).
>
>
...
> [!NOTE]
> Утверждение `x5t` не рекомендуется использовать в конечной точке версии 2.0. Мы рекомендуем использовать утверждение `kid` для проверки маркера.
