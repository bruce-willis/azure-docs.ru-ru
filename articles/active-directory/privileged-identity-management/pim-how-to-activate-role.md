...
5. Для активации некоторых ролей требуется использование Многофакторной идентификации (MFA). Проверка подлинности выполняется один раз за сеанс.
    ...
    ![Проверка с помощью функции MFA перед активацией роли — снимок экрана](./media/pim-how-to-activate-role/PIM_activation_MFA.png)
...
7. Нажмите кнопку **ОК**.  Если роль не требует утверждения, она будет активирована и появится в списке активных ролей (непосредственно под списком назначенных ролей). Если [роль требует утверждения](./azure-ad-pim-approval-workflow.md) для активации, в правом верхнем углу окна браузера ненадолго появится всплывающее уведомление о том, что запрос ожидает утверждения.

    ![Снимок экрана: уведомление о запросе, ожидающем утверждения](./media/pim-how-to-activate-role/PIM_Request_Pending_Toast2.png)
...
5. Выберите **Отмена** в верхней части колонки **Ожидающие утверждения**.
   ...
   ![Снимок экрана: отмена ожидающего запроса](./media/pim-how-to-activate-role/PIM_Request_Pending_Banner_Cancel.png)
