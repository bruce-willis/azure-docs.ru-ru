...
> [!NOTE]
> Некоторые клиенты предпочитают использовать для этого сценария три или четыре сервера. Когда промежуточный сервер обновляется, он не может служить резервным сервером для [аварийного восстановления](how-to-connect-sync-operations.md#disaster-recovery). При наличии трех или четырех серверов можно установить новую версию на одной паре основного и резервного серверов. Это обеспечит постоянную доступность промежуточного сервера.
...
5. Убедитесь, что новая конфигурация не вызывает непредвиденных изменений, выполнив инструкции из раздела "Проверка" в статье [Проверка конфигурации сервера](how-to-connect-sync-operations.md#verify-the-configuration-of-a-server). Если что-то пошло не так, как ожидалось, внесите необходимые исправления, выполните импорт и синхронизацию, после чего проверьте, исправлены ли данные, выполнив приведенные ниже действия.
6. Сделайте промежуточный сервер активным. Это действие соответствует последнему шагу, "Переключение активного сервера", в статье [Проверка конфигурации сервера](how-to-connect-sync-operations.md#verify-the-configuration-of-a-server).
