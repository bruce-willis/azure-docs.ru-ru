---
title: Как настроить управляемое удостоверение службы на виртуальной машине Azure с помощью PowerShell
description: Пошаговые инструкции по настройке управляемого удостоверения службы на виртуальной машине Azure с помощью PowerShell.
...
ms.locfileid: "39258398"
---
# <a name="configure-a-vm-managed-service-identity-using-powershell"></a>Настройка управляемого удостоверения службы на виртуальной машине Azure с помощью PowerShell
...
Если имеется виртуальная машина, для которой не требуется назначаемое системой удостоверение и на которой нет назначаемых пользователем удостоверений, используйте следующие команды.
...
Чтобы удалить расширение виртуальной машины MSI, используйте параметр -Name в командлете [Remove-AzureRmVMExtension](/powershell/module/azurerm.compute/remove-azurermvmextension), указав то же имя, которое вы использовали при добавлении расширения.
...
Если у виртуальной машины несколько назначенных пользователем удостоверений, с помощью приведенных ниже команд можно удалить все удостоверения, кроме последнего. Не забудьте заменить значения параметров `<RESOURCE GROUP>` и `<VM NAME>` собственными. `<MSI NAME>` — это имя назначенного пользователем удостоверения, которое следует оставить в виртуальной машине. Эти сведения можно получить в разделе удостоверений виртуальной машины с помощью команды `az vm show`.
...
Update-AzureRmVm -ResourceGroupName myResourceGroup -VM $vm -IdentityType None
```
Если у виртуальной машины есть как назначенное системой, так и назначенные пользователем удостоверения, вы можете удалить все назначенные пользователем удостоверения, переключившись на использование только назначенного системой.
