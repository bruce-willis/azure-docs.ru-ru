...
В этой статье описывается активация Управляемого удостоверения службы виртуальной машины Windows и его использование для получения ключей к учетной записи хранения. Вы можете использовать ключи доступа к хранилищу стандартным образом при выполнении операций с хранилищем, например при использовании пакета SDK для службы хранилища Azure. В рамках этого руководства мы отправим и скачаем большие двоичные объекты с помощью PowerShell службы хранилища Azure. Вы научитесь:
...
3. Чтобы зарегистрировать и активировать Управляемое удостоверение службы, нажмите кнопку **Да**. Чтобы отключить, нажмите кнопку "Нет".
