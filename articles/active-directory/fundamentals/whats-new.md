...
**Категория услуг.** Мониторинг и отчетность  
**Возможности продукта.** Отчеты
...
2.  На панели навигации слева выберите **Azure Active Directory**, а затем — **Регистрация приложений**.

3.  Выберите приложение, использующее API для журналов действий Azure AD, выберите **Параметры**, а затем в колонке **Параметры** выберите **Необходимые разрешения**.

4.  В колонке **Требуемые разрешения** выберите API **Windows Azure Active Directory**.

5.  В области **Необходимые разрешения** колонки **Разрешить доступ** установите флажок рядом с пунктом **Read directory** (Чтение каталога), а затем выберите **Сохранить**.

    Вы вернетесь к колонке **Необходимые разрешения**.

    ![Делегированные разрешения. Параметр Read directory (Чтение каталога)](./media/whats-new/app-registration-delegate-read-directory.png)

6.  Щелкните **Предоставить разрешения** и затем выберите **Да**.
...
Журналы действий Azure AD, в том числе журналы входа и аудита, теперь доступны через MS Graph. Мы предоставили через MS Graph две конечных точки для доступа к этим журналам. См. [документацию](https://docs.microsoft.com/en-us/azure/active-directory/active-directory-reporting-api-getting-started-azure-portal) по программному доступу к API отчетов Azure AD. 
...
В мае 2018 года мы добавили в коллекцию следующие 18 новых приложений с поддержкой федерации:

 

[AwardSpring](https://docs.microsoft.com/azure/active-directory/active-directory-saas-awardspring-tutorial), [Infogix Data3Sixty Govern](), [Yodeck](https://docs.microsoft.com/azure/active-directory/active-directory-saas-infogix-tutorial), [Jamf Pro](https://docs.microsoft.com/azure/active-directory/active-directory-saas-jamfprosamlconnector-tutorial), [KnowledgeOwl](https://docs.microsoft.com/azure/active-directory/active-directory-saas-knowledgeowl-tutorial), [Envi MMIS](https://docs.microsoft.com/azure/active-directory/active-directory-saas-envimmis-tutorial), [LaunchDarkly](https://docs.microsoft.com/azure/active-directory/active-directory-saas-launchdarkly-tutorial), [Adobe Captivate Prime](https://docs.microsoft.com/azure/active-directory/active-directory-saas-adobecaptivateprime-tutorial), [Montage Online](https://docs.microsoft.com/azure/active-directory/active-directory-saas-montageonline-tutorial), [まなびポケット](https://docs.microsoft.com/azure/active-directory/active-directory-saas-manabipocket-tutorial), OpenReel, [Arc Publishing - SSO](https://docs.microsoft.com/azure/active-directory/active-directory-saas-arc-tutorial), [PlanGrid](https://docs.microsoft.com/azure/active-directory/active-directory-saas-plangrid-tutorial), [iWellnessNow](https://docs.microsoft.com/azure/active-directory/active-directory-saas-iwellnessnow-tutorial), [Proxyclick](https://docs.microsoft.com/azure/active-directory/active-directory-saas-proxyclick-tutorial), [Riskware](https://docs.microsoft.com/azure/active-directory/active-directory-saas-riskware-tutorial), [Flock](https://docs.microsoft.com/azure/active-directory/active-directory-saas-flock-tutorial) и [Reviewsnap](https://docs.microsoft.com/azure/active-directory/active-directory-saas-reviewsnap-tutorial).

 

См. [документацию по этим приложениям](https://aka.ms/appstutorial).

См. дополнительные сведения о том, как [добавить приложение в коллекцию приложений Azure AD](https://docs.microsoft.com/en-us/azure/active-directory/develop/active-directory-app-gallery-listing). 
...
### <a name="deployment-plans---guidance-to-get-you-started-with-azure-active-directory"></a>Планы развертывания — рекомендации по началу работы с Azure Active Directory
...
Ищете пошаговое руководство по развертыванию (SSPR, единый вход, CA, прокси приложения, подготовка пользователей, ADFS в PTA или ADFS в PHS)? У нас есть планы развертывания, которые вам помогут!

Перейдите на страницу [Aka.ms/DeploymentPlans](http://Aka.ms/DeploymentPlans) с комплексными руководствами, которые помогут вам начать по-максимуму использовать доступные возможности.



- [Как получить эти планы развертывания?](http://Aka.ms/DeploymentPlans) 

- Свой отзыв можно оставить [здесь](http://aka.ms/DeploymentPlanFeedback).

- Вопросы? Обращайтесь к нам по адресу: [IDGitDeploy@microsoft.com](mailto:IDGitDeploy@microsoft.com).
...
У вас возникают трудности с поиском приложений или субъектов-служб? Теперь вы можете загружать больше приложений в списке всех корпоративных приложений. По умолчанию отображается 20 приложений. Теперь вы можете просматривать дополнительные приложения. 
...
### <a name="public-preview-of-new-and-improved-sign-ins-user-experience-in-azure-portal"></a>Общедоступная предварительная версия новой улучшенной процедуры входа на портале Azure
...
Мы рады представить вам новую улучшенную процедуру работы с журналами входа на портале Azure AD. Эта новая процедура входа дает клиентам следующие преимущества:

- Сокращение задержки с 2 часов до 5 минут. См. [документацию по задержкам](https://docs.microsoft.com/en-us/azure/active-directory/active-directory-reporting-latencies-azure-portal).

- Возможность динамического добавления фильтров с использованием кнопки "Столбцы". Если добавить столбцы в отчет о входе в пользовательский интерфейс, они автоматически будут отображаться как фильтры, доступные для использования.

- Возможность сортировки по дате, имени пользователя и приложению.

- Включение устаревших способов аутентификации и возможность отфильтровывать их с помощью столбца "Клиентское приложение".

- Включение доступного для скачивания скрипта PowerShell, который можно настраивать на основе условий фильтра, выбранных в пользовательском интерфейсе. Этот скрипт PowerShell позволяет вам получить сколько угодно строк данных (на основе условий фильтра), представляя выходные данные в формате CSV.

Дополнительные сведения об этой функции см. в статье [Отчеты о действиях входа на портале Azure Active Directory](https://docs.microsoft.com/en-us/azure/active-directory/active-directory-reporting-activity-sign-ins).
...
Приложения, созданные 25.04.2018 или позднее, больше не могут запрашивать значение для **id_token** с использованием режима ответа **query**.  Это обеспечивает соответствие Azure AD спецификациям OIDC и помогает снизить уязвимость ваших приложений перед атаками.  Приложения, созданным до 25.04.2018, могут использовать режим ответа **query** с типом запроса **id_token**.  При запросе значения id_token из AAD возвращается ошибка **AADSTS70007: query не является поддерживаемым значением для режима response_mode при запросе токена**.
