...
* Самостоятельное подключение приложений, которые используют протокол SCIM для подготовки пользователей ([процесс описан здесь](manage-apps/use-scim-to-provision-users-and-groups.md))
...
Чтобы настроить Azure AD, введите базовую конфигурацию SAML. Вы можете вручную ввести значения или отправить файл метаданных, чтобы извлечь значения полей.
  ...
  ![Домен и URL-адреса Litware](./media/active-directory-saas-custom-apps/customapp4.png)
...
Дополнительные сведения см. в статье [Запросы на аутентификацию согласно SAML 2.0 и соответствующие отклики, которые поддерживает Azure Active Directory (Azure AD)](https://docs.microsoft.com/en-us/azure/active-directory/develop/active-directory-single-sign-on-protocol-reference?/?WT.mc_id=DOC_AAD_How_to_Debug_SAML).
...
Дополнительные сведения см. в статье [Настройка утверждений, выпущенных в токене SAML для корпоративных приложений](./develop/active-directory-saml-claims-customization.md). 
...
Вы можете скачать сертификат в формате Base64 или в виде необработанных данных из Azure AD. Кроме того, сертификат можно получить, скачав XML-файл метаданных приложения или воспользовавшись URL-адресом метаданных федерации приложения.
  ...
  ![Сертификат](./media/active-directory-saas-custom-apps/certificate.png)
...
Дополнительные сведения см. в статье [Управление сертификатами для федеративного единого входа в Azure Active Directory](manage-apps/manage-certificates-for-federated-single-sign-on.md).
...
Прежде чем тестировать приложение SAML, необходимо настроить его в Azure AD и назначить ему пользователей или группы.

  ![Тестирование](./media/active-directory-saas-custom-apps/testing.png)

На странице единого входа щелкните **Проверить параметры SAML** в разделе "Домен и URL-адреса". Откроется область содержимого с инструкциями о том, как протестировать приложение.

1. Выполните вход в приложение. Если приложение настроено для единого входа, инициированного поставщиком услуг, вы будете перенаправлены на URL-адрес единого входа, по которому вы сможете инициировать вход. Если приложение настроено для единого входа, инициированного поставщиком удостоверений, для вас будет выполнен вход в приложение.
2.  Если на странице входа вашей компании отображается сообщение о какой-либо ошибке, скопируйте его и вернитесь в область содержимого единого входа тестирования Azure AD. Вставьте сообщение об ошибке и щелкните **Получить инструкции по устранению**. Если сообщение об ошибке отображается на странице приложения, обратитесь к поставщику приложения и предоставьте ему сведения о своей конфигурации в Azure AD, чтобы можно было проверить соответствующие значения. 
3.  В зависимости от ошибки Azure AD предложит конкретные действия по устранению проблемы.

Дополнительные сведения см. в статье [Отладка единого входа на основе SAML в приложениях в Azure Active Directory](https://docs.microsoft.com/en-us/azure/active-directory/develop/active-directory-saml-debugging/?WT.mc_id=DMC_AAD_?WT.mc_id=UI_AAD_Configure_NonGalleryApps).
