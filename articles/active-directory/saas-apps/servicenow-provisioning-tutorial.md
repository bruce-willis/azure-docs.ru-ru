...
1. Если в ServiceNow уже настроен единый вход, найдите свой экземпляр ServiceNow с помощью поля поиска. В противном случае щелкните **Добавить** и выполните поиск **ServiceNow** в коллекции приложений. Выберите ServiceNow в результатах поиска и добавьте в список приложений.

1. Выберите экземпляр ServiceNow, а затем перейдите на вкладку **Подготовка**.
...
1. На портале Azure щелкните **Проверить подключение**, чтобы убедиться, что Azure AD может подключиться к приложению ServiceNow. Если подключение отсутствует, убедитесь, что у учетной записи ServiceNow есть разрешения администратора группы, и повторите шаг **Учетные данные администратора**.
...
1. В разделе "Сопоставления" выберите **Synchronize Azure Active Directory Users to ServiceNow** (Синхронизировать пользователей Azure Active Directory с ServiceNow).
...
1. Чтобы включить службу подготовки Azure AD для ServiceNow, измените значение параметра **Состояние подготовки** на **Включено** в разделе "Параметры".
...
* [Настройка единого входа](servicenow-tutorial.md)
