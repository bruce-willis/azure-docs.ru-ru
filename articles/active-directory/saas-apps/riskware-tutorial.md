...
10. На странице **Authentication Configuration** (Настройка аутентификации) выполните следующие действия:
    ...
    f. В текстовом поле **Post Form Field** (Поле формы POST) введите имя поля из ответа POST, которое содержит значение SAML, например SamlResponse.
    ...
    k. Установите флажок **Use POST** (Использовать POST), чтобы передавать запрос SAML в качестве параметра запроса POST.

    l. Установите флажок **Use SAML Request** (Использовать запрос SAML), чтобы передавать запросы SAML единого входа, инициированные поставщиком услуг.
...
Чтобы пользователи Azure AD могли выполнять вход в Riskware, они должны быть подготовлены в Riskware. В Riskware подготовка выполняется вручную.
...
4. Выполните следующие действия на вкладке **Details** (Подробные сведения).
...
6. На вкладке **Organisation** (Организация) сделайте следующее:
    ...
    a. В разделе **Organisation** (Организация) выберите для организации уровень **Level1**.
