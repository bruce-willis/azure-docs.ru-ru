...
Выполнение любой операции (отправка, получение, удаление и т. д.) занимает определенное время. В это время входит обработка операции службой Service Bus, а также задержка запроса и ответа. Чтобы увеличить количество операций в единицу времени, необходимо выполнять их параллельно. Эту согласованность можно получить несколькими способами:

* **Асинхронные операции**. Клиент планирует действия за счет асинхронных операций. Следующий запрос запускается до завершения предыдущего запроса. Вот пример фрагмента кода асинхронной операции отправки:
