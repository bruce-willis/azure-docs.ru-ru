...
1. В Visual Studio перейдите к папке *\samples\DotNet\Microsoft.ServiceBus.Messaging\ServiceBusEventGridIntegration*, а затем откройте файл *SBEventGridIntegration.sln*.

1. Перейдите к проекту **MessageSender**, а затем выберите **Program.cs**.

   ![8][]

1. Введите имя раздела и строку подключения, а затем выполните следующий код из консоли.
...
1. В только что созданной функции выберите знак плюса (+), чтобы добавить функцию триггера HTTP.
...
1. Нажмите кнопку **Веб-перехватчик + API**, а затем **CSharp** и **Создайте эту функцию**.
 
1. Внутри функции вставьте следующий код.
...
1. Заполните форму, как показано в поле **Фильтр суффиксов**, не забудьте указать соответствующий фильтр.

1. Нажмите кнопку **Создать**.

1. Отправьте сообщение в раздел служебной шины, как указано в разделе предварительных требований, и убедитесь, что события передаются через компонент мониторинга функции Azure.
...
1. Введите строку подключения в следующем коде.

    ```Csharp
    const string ServiceBusConnectionString = "YOUR CONNECTION STRING";
    ```

1. На портале Azure скачайте профиль публикации для функции Azure, созданной в разделе "Создание проверочной функции".

    ![11][]

1. Затем в Visual Studio щелкните правой кнопкой мыши **SBEventGridIntegration** и выберите **Опубликовать**. 

1. На панели **Опубликовать** для профиля публикаций, скачанного ранее, выберите **Импортировать профиль**, а затем **Опубликовать**.

    ![12][]

1. После публикации новой функции Azure создайте подписку на службу "Сетка событий Azure", указывающую на новую функцию.  
    Примените правильный фильтр в поле **Ends with** (Заканчивается на), который является именем вашей подписки служебной шины.

1. Отправьте сообщение в раздел служебной шины Azure, созданный ранее, и просматривайте журнал службы "Функции Azure" на портале Azure, чтобы убедиться в том, что события передаются и приходят соответствующие сообщения.
...
1. Добавьте свои данные следующим образом.

    a. В поле **Имя ресурса** введите имя пространства имен. 

    b. В разделе **Дополнительные параметры** в поле **Фильтр суффиксов** введите фильтр своей подписки.
...
1. Добавьте событие завершения процесса, как показано на следующем рисунке.

    ![16][]

1. Сохраните приложение логики и отправьте сообщение в раздел служебной шины, как указано в разделе предварительных требований.  
