...
При создании очереди, раздела или ретранслятора задается имя. В комбинации с именем пространства имен это имя создает уникальный идентификатор объекта. Приложения могут передать это имя служебной шине, а затем использовать соответствующую очередь, раздел или ретранслятор для взаимодействия с другими приложениями.

 >**Пример:**   
     *https://&lt;имя_пространства_имен_служебной_шины&gt;.servicebus.windows.net/&lt;queue-name&gt;*  
     *https://&lt;имя_пространства_имен_служебной_шины&gt;.servicebus.windows.net/&lt;topic-name&gt;*  
     *https://&lt;имя_пространства_имен_служебной_шины&gt;.servicebus.windows.net/&lt;relay-name&gt;*  
...
*Разделы* во многом похожи на очереди. Отправители отправляют сообщения в раздел так же, как в очередь, и эти сообщения выглядят в разделе так же, как в очереди. Отличие заключается в том, что разделы позволяют каждому принимающему приложению создавать собственную *подписку* и при необходимости определять *фильтр*. Подписчик получает копию каждого сообщения в разделе, но c использованием фильтров может получать только те сообщения, которые ему нужные. Например, на рисунке 3 показан отправитель и раздел с тремя подписчиками, у каждого из которых имеется собственный фильтр:
...
* Подписчик 3 задал для фильтра значение **True**, которое означает, что он получает все сообщения. Например, это приложение может вести журнал аудита, поэтому ему нужен доступ ко всем сообщениям.
