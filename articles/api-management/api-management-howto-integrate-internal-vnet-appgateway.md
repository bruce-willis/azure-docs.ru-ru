...
Если путь не соответствует правилам, которые необходимо включить с помощью управления API, то при настройке сопоставления для пути правил также настраивается внутренний пул адресов по умолчанию с именем **dummyBackendPool**. Например, трафик из http://api.contoso.net/calc/sum передается в пул **dummyBackendPool**, так как он определен как пул по умолчанию для несоответствующего трафика.
