...
1. Выберите пункт ![Стрелка](./media/api-management-howto-aad/arrow.png).
1. В поле поиска введите **api**.
1. Выберите **Службы управления API**.
1. Выберите экземпляр службы управления API.
1. В разделе **Безопасность** выберите **Удостоверения**.

1. Выберите **+ Добавить** в верхней части экрана.

    Справа отобразится область **Добавление поставщика удостоверений**.
1. В разделе **Тип поставщика** выберите **Azure Active Directory**.
...
1. Запишите содержимое **URL-адреса перенаправления**.
...
1. В браузере откройте другую вкладку. 
1. Перейдите на [портал Azure](https://portal.azure.com).
1. Выберите пункт ![Стрелка](./media/api-management-howto-aad/arrow.png).
1. Введите **active**. Появится область **Azure Active Directory**.
1. Выберите **Azure Active Directory**.
1. В разделе **Управление** выберите **Регистрация приложения**.
...
1. После регистрации приложения перейдите к параметру **URL-адрес ответа** и убедитесь, что для **URL-адреса перенаправления** установлено значение, полученное на шаге 9. 
1. Чтобы настроить приложение (например, изменить **URL-адрес идентификатора приложения**) выберите **Свойства**.
...
1. Установите разрешения для приложений. Для этого выберите **Требуемые разрешения**.
1. Выберите свое приложение и установите флажки **Чтение данных каталога** и **Вход и чтение профиля пользователя**.
...
1. В области слева скопируйте значение **идентификатора приложения**.

    ![Значение идентификатора приложения](./media/api-management-howto-aad/application-id.png)
1. Вернитесь к приложению управления API. 
...
1. Вернитесь к конфигурации Azure AD и выберите **Ключи**.
1. Создайте ключ, указав его имя и срок действия. 
1. Щелкните **Сохранить**. Будет создан ключ.
...
1. Вернитесь к приложению управления API. 
    ...
    >  
    >
1. В окне **Добавление поставщика удостоверений** также содержится текстовое поле **Разрешенные клиенты**. Укажите домены экземпляров Azure AD, которым требуется предоставить доступ к API экземпляра службы управления API. Несколько доменов можно разделять символами начала новой строки, пробелами или запятыми.
...
1. После указания требуемой конфигурации выберите **Добавить**.
