...
Основная задача API управления состоит в защите серверных ресурсов. Если сервер авторизации, используемый API-интерфейсом, создает [маркеры JWT](http://jwt.io/) в рамках потока OAuth2, как это делается в [Azure Active Directory](../active-directory/hybrid/whatis-hybrid-identity.md), с помощью политики `validate-jwt` можно проверить допустимость маркера. Некоторые серверы авторизации создают так называемые [маркеры ссылок](http://leastprivilege.com/2015/11/25/reference-tokens-and-introspection/), для проверки которых требуется обратный вызов к серверу авторизации.
