...
1. Создайте **учетную запись хранения Azure** общего назначения, если у вас ее нет. В этом пошаговом руководстве хранилище больших объектов используется как **исходное** и **целевое** хранилище данных. в статье [Об учетных записях хранения Azure](../../storage/common/storage-quickstart-create-account.md) .
...
7. На странице **File format settings** (Параметры формата файла) отображаются разделители и схема, которые мастер определяет автоматически при анализе файла. 
    1. Проверьте следующие параметры: a. Убедитесь, что для **формата файла** задано значение **Текстовый формат**. Все поддерживаемые форматы находятся в раскрывающемся списке. Например, JSON, Avro, ORC, Parquet.
        b. Убедитесь, что в качестве **разделителя столбцов** используется `Comma (,)`. Другие разделители столбцов, поддерживаемые фабрикой данных, находятся в раскрывающемся списке. Вы также можете указать пользовательский разделитель.
        ...
        Е. Убедитесь, что установлен флажок **Treat empty column value as null** (Значение пустого столбца рассматривается как null).
