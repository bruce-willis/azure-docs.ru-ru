...
13. В окне **New Linked Service** (Новая связанная служба) убедитесь, что в разделе **Connect via integration runtime** (Подключение через среду выполнения интеграции) выбрана созданная выше **среда выполнения интеграции**. 
...
22. Теперь снова отобразится окно с целевым набором данных. На вкладке **Подключение** сделайте следующее: 
       ...
       Б. В качестве имени **папки**/ **каталога** в поле **Путь к файлу** введите значение **adftutorial/incrementalcopy**. Если указанной папки выходных данных не существует в контейнере adftutorial, она будет автоматически создана в фабрике данных.
