...
1. Создайте локальную среду выполнения интеграции. Для создания локальной среды IR можно использовать пользовательский интерфейс ADF. Ниже приведен пример скрипта PowerShell.
...
## <a name="setting-up-self-hosted-ir-on-azure-vm-using-azure-resource-manager-template-automatation"></a>Настройка локальной среды IR на виртуальной машине Azure с помощью шаблона Azure Resource Manager (автоматизация)
Вы можете автоматизировать настройку локальной среды IR на виртуальной машине Azure с помощью [этого шаблона Azure Resource Manager](https://github.com/Azure/azure-quickstart-templates/tree/master/101-vms-with-selfhost-integration-runtime). Таким образом можно легко создать полностью функционирующую локальную среду IR внутри виртуальной сети Azure с функцией масштабируемости и высокой доступности (при условии, что количество узлов будет равным 2 или выше).
...
Когда **доступной памяти в локальной среде IR недостаточно**, а **ЦП сильно загружен**, можно добавить новый узел, чтобы распределить нагрузку по нескольким компьютерам. Если действия не выполняются из-за истечения времени ожидания или из-за того, что узел локальной среды IR находится в автономном режиме, можно добавить узел к шлюзу.

#### <a name="scale-up"></a>Увеличение масштаба

Если доступная память и ЦП используются неэффективно и выполнение параллельных заданий достигает ограничения, следует увеличить масштаб, увеличив количество одновременных заданий, которые могут выполняться на узле. Кроме того, масштаб можно увеличить, если время ожидания действия истекает из-за перегрузки локальной среды IR. Как показано на следующем рисунке, можно увеличить максимальную емкость для узла.  
...
## <a name="sharing-the-self-hosted-integration-runtime-ir-with-multiple-data-factories"></a>Совместное использование локальной среды IR (IR) с несколькими фабриками данных

Вы можете повторно использовать имеющуюся инфраструктуру локальной среды IR, которую вы, возможно, уже настроили в фабрике данных. Это позволяет создать **связанную локальную среду IR** в другой фабрике данных, ссылаясь на уже имеющуюся локальную среду IR (общую).
...
В локальной среде IR, к которой нужно предоставить общий доступ,

1. предоставьте разрешения Фабрике данных, в которой нужно создать связанную среду IR. 
...
В Фабрике данных, которой были предоставлены разрешения,

3. создайте локальную среду IR (связанную) и введите **идентификатор ресурса**, который вы записали.
...
1. Количество связанных сред IR по умолчанию, которые можно создать в одной локальной среде IR, — **20**. Если требуется больше, обратитесь в службу поддержки. 

2. У фабрики данных, в которой нужно создать связанную среду IR, должно быть управляемое удостоверение службы ([MSI](https://docs.microsoft.com/azure/active-directory/managed-service-identity/overview)). По умолчанию для фабрик данных, создаваемых на портале Ibiza или с помощью командлетов PowerShell, управляемые удостоверения службы генерируются неявным образом. Однако в некоторых случаях, когда фабрика данных создается с помощью шаблона Azure Resorce Manager или пакета SDK, свойство **идентификатора** **нужно задать** явным образом, чтобы Azure Resorce Manager создал фабрику данных с управляемым удостоверением службы. 

3. Версия локальной среды IR должна быть равной 3.8.xxxx.xx или выше. [Загрузите последнюю версию](https://www.microsoft.com/download/details.aspx?id=39717) локальной среды IR.

4. У фабрики данных, в которой нужно создать связанную среду IR, должно быть управляемое удостоверение службы ([MSI](https://docs.microsoft.com/azure/active-directory/managed-service-identity/overview)). По умолчанию для фабрик данных, создаваемых на портале Ibiza или с помощью командлетов PowerShell, MSI ([управляемые удостоверения службы](https://docs.microsoft.com/azure/active-directory/managed-service-identity/overview))
создаются неявным образом. Однако для фабрик данных, созданных с помощью шаблона Azure Resource Manager (ARM) или пакета SDK, необходимо задать свойство идентификатора для обеспечения создания удостоверения MSI.
...
1. Запустите **Диспетчер конфигурации Microsoft для среды выполнения интеграции**.
   ...
   - Щелкните ссылку **Изменить** в разделе **HTTP-прокси**, чтобы открыть диалоговое окно **Указание HTTP-прокси**.
   - После нажатия кнопки **Далее** появится диалоговое окно предупреждения, запрашивающее разрешение на сохранение настроек прокси-сервера и перезапуск службы узла среды выполнения интеграции.
...
1. При попытке регистрации локальной среды выполнения интеграции появляется следующая ошибка: "Зарегистрировать этот узел среды выполнения интеграции не удалось! Убедитесь, что используется действительный ключ проверки подлинности и что на этом компьютере запущена служба узла среды выполнения. "
   - При открытии диспетчера конфигурации среды выполнения интеграции отображается состояние "**Отключено**" или "**Подключение**". В средстве просмотра событий Windows в разделе "Журналы приложения и служб" > "Шлюз управления данными" отображается сообщение об ошибке следующего вида:
