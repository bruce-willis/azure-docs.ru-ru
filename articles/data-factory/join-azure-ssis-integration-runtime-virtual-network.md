...
Виртуальная сеть может быть развернута с использованием классической модели развертывания или модели развертывания с помощью Azure Resource Manager. Если вы планируете присоединить IR Azure SSIS к *виртуальной сети*, уже присоединенной с помощью Управляемого экземпляра (предварительная версия), среда IR Azure SSIS должна находиться в *другой подсети*.   
...
IR Azure–SSIS должна создать определенные сетевые ресурсы в той же группе ресурсов, в которой находится виртуальная сеть, в том числе подсистему балансировки нагрузки Azurе, общедоступный IP-адрес Azure и рабочую группу безопасности сети.
...
5. На странице **Общие параметры** окна **Настройка среды выполнения интеграции** нажмите кнопку **Далее**. 
...
8. Теперь можно запустить среду выполнения интеграции, нажав кнопку **запуска** в столбце **Действия** для выбранной среды выполнения интеграции Azure SSIS. Запуск IR Azure SSIS занимает приблизительно от 20 до 30 минут. 
