...
* Со связанными службами Azure HDInsight, которые доступны по требованию, нельзя использовать действие скрипта. Если вам нужно установить другие зависимости, попробуйте в Службе автоматизации Azure выполнить скрипт PowerShell, который делает следующее:  
  а) создает кластер HDInsight;  
  б) выполняет действие скрипта, например, для установки других зависимостей;  
  в) запускает конвейер Фабрики данных;  
  г) удаляет кластер.  
