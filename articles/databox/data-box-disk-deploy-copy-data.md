...
- Диски распакованы и включены.
- Имеется главный компьютер для копирования данных на диски. На главном компьютере должно быть следующее ПО:
    ...
    - [установлена платформа .NET Framework 4.5](https://www.microsoft.com/download/details.aspx?id=30653).
...
## <a name="verify-data-integrity"></a>Проверка целостности данных

Чтобы проверить целостность данных, выполните следующие действия.

1. Запустите `AzureExpressDiskService.ps1` для проверки контрольной суммы. В проводнике перейдите в папку *​​AzureImportExport* диска. Щелкните правой кнопкой мыши и выберите **Выполнить с помощью PowerShell**. 

    ![Запуск Checksum](media/data-box-disk-deploy-copy-data/data-box-disk-checksum.png)

2. В зависимости от размера данных, это действие может занять некоторое время. Сводка процесса проверки целостности данных и времени завершения процесса отображается после завершения скрипта. Чтобы выйти из окна командной строки, нажмите клавишу **Enter**.
