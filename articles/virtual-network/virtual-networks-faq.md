...
- Управление виртуальными сетями, развернутыми по модели [Azure Resource Manager](/powershell/module/azurerm.network) или [классической модели развертывания](/powershell/module/servicemanagement/azure/?view=azuresmps-3.7.0), с помощью PowerShell.
...
Да. Глобальный пиринг виртуальных сетей позволяет создавать пиринговую связь между виртуальными сетями в разных регионах. Глобальный пиринг виртуальных сетей доступен во всех общедоступных регионах Azure. Вы не можете устанавливать глобальное пиринговое подключение из общедоступных регионов Azure в национальные облака. Глобальный пиринг в настоящее время недоступен в национальных облаках.
...
В настоящее время невозможно установить пиринг виртуальной сети (локальный или глобальный), если ваши подписки принадлежат к разным клиентам Azure Active Directory.
...
Если пиринговое подключение находится в состоянии "Начато", это означает, что вы создали только одну связь. Чтобы установить успешное подключение, необходимо создать двунаправленную связь. Например, чтобы установить пиринговое подключение виртуальной сети А к виртуальной сети В, нужно создать связь из сети VNetA к сети VNetB и из сети VNetB к сети VNetA. Создание обеих связей изменит состояние на *Подключено.*
...
Если пиринговое подключение виртуальной сети находится в состоянии "Отключено", это означает, что одна из созданных связей была удалена. Чтобы повторно установить пиринговую связь, необходимо удалить и повторно создать связь.
...
### <a name="why-is-my-peering-connection-in-a-disconnected-state"></a>Почему мое пиринговое подключение неактивно?
...
На этапе предварительной версии для разработчиков эта функция доступна в регионе "Центрально-западная часть США". Отслеживаемые сетевые интерфейсы, ресурс TAP виртуальной сети и сборщик или решение для аналитики должны быть развернуты в одном регионе.
...
У отслеживаемого сетевого интерфейса может быть только одна конфигурация TAP. Ознакомьтесь с отдельными [решениями партнеров](virtual-network-tap-overview.md#virtual-network-tap-partner-solutions), чтобы изучить возможности потоковой передачи нескольких копий трафика TAP в выбранные инструменты аналитики.
