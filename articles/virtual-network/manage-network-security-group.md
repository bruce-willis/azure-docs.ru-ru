...
4. <a name = "security-rule-settings"></a>Выберите **+ Добавить**.  Выберите или добавьте значения перечисленных ниже параметров, а затем щелкните **ОК**.
    ...
    |---------|---------|---------|
    |Источник     | Выберите значение **Любой**, **Группа безопасности приложений**, **IP-адреса** или **Тег службы** в качестве правил безопасности для входящего трафика. Если вы создаете правило безопасности для исходящего трафика, у вас будут такие же варианты, как и для параметра **Назначение**.       | Если вы выберете значение **Группа безопасности приложений**, то выберите одну или несколько существующих групп безопасности приложений в одном регионе в качестве сетевого интерфейса. Узнайте, как [создать группу безопасности приложений](#create-an-application-security-group). Если вы выберете значение **Группа безопасности приложений** для **источника** и **назначения**, сетевые интерфейсы в обеих группах безопасности должны находиться в одной виртуальной сети. При выборе параметра **IP-адреса** укажите значение поля **Диапазоны исходных IP-адресов или CIDR**. Можно указать одно значение или список разделенных запятыми значений. Пример нескольких значений: 10.0.0.0/16, 192.188.1.1. Число значений, которые можно указать, ограничено. Дополнительные сведения см. в разделе [Ограничения сети](../azure-subscription-service-limits.md?toc=%2fazure%2fvirtual-network%2ftoc.json#azure-resource-manager-virtual-networking-limits). При выборе параметра **Тег службы** необходимо выбрать один тег службы. Тег службы — это предопределенный идентификатор категории IP-адресов. Дополнительные сведения о доступных тегах служб и том, что они представляют, приведены в разделе [Теги служб](security-overview.md#service-tags). Если указанный IP-адрес назначен виртуальной машине Azure, убедитесь, что вы указываете частный, а не общедоступный IP-адрес, назначенный виртуальной машине. Правила безопасности обрабатываются после того, как Azure преобразует общедоступный IP-адрес в частный для правил безопасности для входящего трафика, и до того, как Azure преобразует частный IP-адрес в общедоступный для правил исходящего трафика. Дополнительные сведения об общедоступных и частных IP-адресах в Azure см. в статье, посвященной [типам IP-адресов](virtual-network-ip-addresses-overview-arm.md).        |
    ...
    |Место назначения     | Выберите значение **Любой**, **Группа безопасности приложений**, **IP-адреса** или **Виртуальная сеть** в качестве правил безопасности для входящего трафика. Если вы создаете правило безопасности для исходящего трафика, у вас будут такие же варианты, как и для параметра **Источник**.        | Если вы выберете значение **Группа безопасности приложений**, то выберите одну или несколько существующих групп безопасности приложений в одном регионе в качестве сетевого интерфейса. Узнайте, как [создать группу безопасности приложений](#create-an-application-security-group). Если вы выберете значение **Группа безопасности приложений**, то выберите одну существующую группу безопасности приложений в одном регионе в качестве сетевого интерфейса. При выборе параметра **IP-адреса** необходимо указать значение поля **Диапазоны IP-адресов назначения или CIDR**. Как и в полях **Источник** и **Диапазоны исходных IP-адресов или CIDR**, можно указать один или несколько адресов или их диапазонов, однако число указываемых значений ограничено. Выбор параметра **Виртуальная сеть**, который является тегом службы, означает, что будет разрешен трафик для всех IP-адресов в диапазоне адресов виртуальной сети. Если указанный IP-адрес назначен виртуальной машине Azure, убедитесь, что вы указываете частный, а не общедоступный IP-адрес, назначенный виртуальной машине. Правила безопасности обрабатываются после того, как Azure преобразует общедоступный IP-адрес в частный для правил безопасности для входящего трафика, и до того, как Azure преобразует частный IP-адрес в общедоступный для правил исходящего трафика. Дополнительные сведения об общедоступных и частных IP-адресах в Azure см. в статье, посвященной [типам IP-адресов](virtual-network-ip-addresses-overview-arm.md).        |
...
2. В поле **Поиск в Marketplace** введите *Группа безопасности приложений*. Когда в результатах поиска появится **Группа безопасности приложений**, выберите ее, потом снова щелкните **Группа безопасности приложений** во вкладке **Все** и нажмите **Создать**.
3. Выберите или введите следующие сведения, а затем щелкните **Создать**.

    | Параметр        | Значение                                                   |
    | ---            | ---                                                     |
    | ИМЯ           | Имя должно быть уникальным в пределах группы ресурсов.        |
...
2. Введите *группы безопасности приложений* в поле **фильтра "Все службы"**, а затем выберите **Группы безопасности приложений** в результатах поиска.
...
1. Выберите **Все службы** в левом верхнем углу на портале Azure.
2. Введите *группы безопасности приложений* в поле **фильтра "Все службы"**, а затем выберите **Группы безопасности приложений** в результатах поиска.
3. Выберите группу безопасности приложений, сведения о которой хотите просмотреть.
...
1. Выберите **Все службы** в левом верхнем углу на портале Azure.
2. Введите *группы безопасности приложений* в поле **фильтра "Все службы"**, а затем выберите **Группы безопасности приложений** в результатах поиска.
3. Выберите группу безопасности приложений, параметры которой хотите изменить. Вы можете добавлять и удалять теги или назначать и удалять разрешения для группы безопасности приложений.
...
1. Выберите **Все службы** в левом верхнем углу на портале Azure.
2. Введите *группы безопасности приложений* в поле **фильтра "Все службы"**, а затем выберите **Группы безопасности приложений** в результатах поиска.
3. Выберите группу безопасности приложений, которую вы хотите удалить.
4. Выберите **Удалить** и нажмите **Да**, чтобы удалить группу безопасности приложений.
