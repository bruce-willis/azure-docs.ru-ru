...
|None                            |None                                            |Null (недоступно при использовании CLI 1.0 в режиме ASM)|
...
Распространение маршрутов BGP можно отключить для подсети с помощью свойства в таблице маршрутов. При обмене маршрутами с Azure с использованием протокола BGP маршруты не добавляются в таблицу маршрутов всех подсетей при отключенном распространении BGP. Активировать VPN-подключения можно с помощью [пользовательских маршрутов](#custom-routes) и VPN с типом следующего прыжка. См. дополнительные сведения об [отключении распространения маршрутов BGP](manage-route-table.md#create-a-route-table).
...
Если виртуальная сеть подключена к шлюзу Azure VPN, не связывайте таблицу маршрутов с [подсетью шлюза](../vpn-gateway/vpn-gateway-about-vpn-gateway-settings.md?toc=%2fazure%2fvirtual-network%2ftoc.json#gwsub), включающей маршрут с назначением 0.0.0.0/0. Это может привести к неправильной работе шлюза.
