...
> [!NOTE]  
> Помимо поддержки SSL для имен личных доменов, Azure CDN помогает масштабировать приложение для быстрой доставки содержимого по всему миру. Для получения дополнительных сведений ознакомьтесь с [обзором Azure CDN](../../cdn/cdn-overview.md).
...
1.  [Интеграция учетной записи хранения Azure с Azure CDN](../../cdn/cdn-create-a-storage-account-with-cdn.md).
    ...
    > [!NOTE]  
    > Пока служба хранилища Azure поддерживает статические веб-сайты в режиме предварительной версии, во всплывающем меню типов источника нужно выбрать пункт "Настраиваемый источник", чтобы добавить конечную точку веб-службы хранилища. На портале Azure сделайте это в своем профиле CDN, а не непосредственно в учетной записи хранения.
...
Cdn-endpoint-name относится к имени, настроенному для конечной точки CDN. Это значение можно выбрать в раскрывающемся списке. Origin-path относится к пути в вашей учетной записи хранения-источнике, в которой размещается статическое содержимое. Если все статическое содержимое размещается в одном контейнере, замените origin-path именем этого контейнера.
...
* [Размещение статических веб-сайтов в службе хранилища Azure (предварительная версия)](storage-blob-static-website.md)
