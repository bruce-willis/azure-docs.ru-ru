...
Подробные сведения об интерфейсе REST API больших двоичных объектов см. в документации [Azure Blob Service API](https://docs.microsoft.com/rest/api/storageservices/blob-service-rest-api).
...
> Функция хранения данных в неизменяемом виде доступна только в учетных записях хранения GPv2 и BLOB-объектов и должна создаваться с помощью [Azure Resource Manager](https://docs.microsoft.com/azure/azure-resource-manager/resource-group-overview).
...
Установите [расширение CLI](http://docs.microsoft.com/cli/azure/install-azure-cli?view=azure-cli-latest) с помощью `az extension add -n storage-preview`
...
Операция "Удалить контейнер" не будет выполнена, если в контейнере есть хотя бы один большой двоичный объект с заблокированной политикой хранения на основе времени или юридическим удержанием. Это также относится к [обратимому удалению](storage-blob-soft-delete.md) данных. Операция "Удалить контейнер" будет выполнена, если в контейнере нет большого двоичного объекта с действующим периодом хранения или юридическим удержанием. Прежде чем удалить контейнер, необходимо сначала удалить BLOB-объекты. 
