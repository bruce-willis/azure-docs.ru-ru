...
* [Учетные записи **Общего назначения версии 2 (GPv2)**](#general-purpose-v2-accounts)
* [Учетные записи **Общего назначения версии 1 (GPv1)**](#general-purpose-v1-accounts)
...
Учетные записи общего назначения v2 (GPv2) являются учетными записями хранения, которые поддерживают все новые функции для больших двоичных объектов, файлов, очередей и таблиц. Учетные записи GPv2 поддерживают все API-интерфейсы, службы и функции, которые поддерживаются учетными записями общего назначения версии 1 (GPv1) и учетными записями хранения BLOB-объектов. Они также сохраняют ту же устойчивость, доступность, масштабируемость и производительность, предоставляемые всеми типами учетных записей хранения. Ценообразование для учетных записей GPv2 было сформировано для обеспечения самых низких цен на гигабайт и конкурентоспособных цен на транзакции в отрасли.
...
Для блочных BLOB-объектов в учетной записи хранения GPv2 можно выбрать между горячим и холодным уровнями хранилища учетной записи или между горячим, холодным и архивным уровнем хранилища для большого двоичного объекта. Чтобы оптимизировать затраты, храните данные, к которым часто, иногда и редко обращаются, на горячем, холодном и активном уровнях хранилища соответственно. 
...
Пользователи могут в любое время обновить учетную запись GPv1 или учетную запись хранилища BLOB-объектов до учетной записи GPv2 с помощью портала Azure, PowerShell или Azure CLI. Это изменение невозможно отменить, а другие типы учетной записи не допускаются. Дополнительные сведения об оценке существующей учетной записи хранения см. в разделе [Оценка учетных записей хранения GPv2 и переход на них](#evaluating-and-migrating-to-gpv2-storage-accounts).
* [Обновление до GPv2 с помощью портала Azure](#upgrade-with-azure-portal)
* [Обновление до GPv2 с помощью PowerShell](#upgrade-with-powershell)
* [Обновление до GPv2 с помощью Azure CLI](#upgrade-with-azure-cli)
...
В обоих случаях сначала стоит оценить затраты на хранение данных, доступ и работу с хранимыми данными в GPv2, а затем сравнить эти затраты с текущими.
...
* Использование хранилища данных (ГБ)
...
* Шаблон доступа к хранилищу (операции и передача данных)
    - Какой объем данных считывается (исходящий трафик) и записывается (входящий трафик) в учетную запись хранения? 
    - Сколько операций над данными выполняется в учетной записи хранения?
    - Какие виды операций (чтение или запись) были проведены с данными?
...
* Транзакции чтения, такие как *GetBlob*.
