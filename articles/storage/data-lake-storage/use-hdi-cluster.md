...
* **Управление жизненным циклом данных**. Все данные в любой файловой системе проходят через свой жизненный цикл: сначала очень ценные и часто используемые, затем менее ценные и менее используемые, после чего архивируются или удаляются. Служба хранилища Azure обеспечивает политики многоуровневого хранения данных и управления жизненным циклом, которые распределяют данные по уровням в соответствии с этапом их жизненного цикла.
...
> [!NOTE]
> На этапе общедоступной предварительной версии Data Lake Storage Gen2 поддерживается только `--type LRS`. Дополнительные возможности избыточности станут доступны во время программы по ознакомлению с предварительной версией.
...
&lt;FileSystem&gt; идентифицирует путь к файловой системе Azure Data Lake Storage.
&lt;AccountName&gt; определяет имя учетной записи хранения Azure. Обязательно использовать полное доменное имя (FQDN).

Если значения для &lt;FileSystem&gt; и &lt;AccountName&gt; не указаны, используется файловая система по умолчанию. Для файлов в файловой системе по умолчанию можно использовать относительный или абсолютный путь. Например, для ссылки на файл *hadoop-mapreduce-examples.jar*, который поставляется с кластерами HDInsight, можно использовать один из следующих вариантов:
