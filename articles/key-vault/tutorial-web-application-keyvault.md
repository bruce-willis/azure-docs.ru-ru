...
В этом руководстве описано, как настроить в веб-приложении Azure чтение данных из Key Vault с помощью управляемых удостоверений для ресурсов Azure. Вы узнаете, как выполнять следующие задачи:
...
В следующем примере создается группа ресурсов с именем *ContosoResourceGroup* в расположении *eastus*.
...
Теперь создайте Key Vault в группе ресурсов, созданной на предыдущем шаге. В этом руководстве для хранилища ключей используется имя Contoso KeyVault, но вам нужно выбрать и использовать другое уникальное имя. Введите следующие сведения:
...
>[!IMPORTANT]
> Если вы получили сообщение об ошибке HTTP 502.5 (сбой процесса),
> > проверьте имя Key Vault, указанное в `Program.cs`.
...
Теперь ваша учетная запись Azure и удостоверение приложения получат права на чтение данных из Key Vault. Обновив страницу, вы увидите целевую страницу сайта. Щелкнув **Сведения**, вы увидите значение, которое хранится в Key Vault.
