...
Создав пару Key Vault, следите за тем, как и когда к вашим ключам и секретам осуществляется доступ. Это можно сделать, включив ведение журнала Key Vault. Для Azure Key Vault можно настроить следующее:
...
- Репликацию содержимого Key Vault в рамках региона и в дополнительный регион. Key Vault обеспечивает высокий уровень доступности и не требует вмешательства администратора для запуска отработки отказа.
