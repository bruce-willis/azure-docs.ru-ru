---
title: Общие сведения о типах сущностей в приложениях LUIS в Azure | Документы Майкрософт
description: Добавляйте сущности (ключевые данные в предметной области приложения) в приложения Language Understanding Intelligent Service (LUIS).
...
| **Список** <br/>[Точное соответствие](#exact-match)|| **Определение**<br>Сущности списка представляют фиксированный, закрытый набор связанных слов вместе с их синонимами в системе. <br><br>Каждая сущность списка может иметь одну или несколько форм. Она лучше всего подходит для известного набора вариантов для представления одного понятия.<br/><br/>LUIS не обнаруживает дополнительные значения для сущностей списка. Найти предложения для новых слов на основе текущего списка можно в [семантическом словаре](luis-glossary.md#semantic-dictionary).<br/><br>Если имеется несколько сущностей списка с одним и тем же значением, в запросе конечной точки возвращается каждая сущность. <br/><br/>[Краткое руководство](luis-quickstart-intent-and-list-entity.md)<br>[Пример ответа для сущности](luis-concept-data-extraction.md#list-entity-data)| 
...
**Прошедшие машинное обучение** сущности лучше всего работают после тестирования с помощью [запросов конечной точки](luis-concept-test.md#endpoint-testing) и [проверки высказываний конечной точки](label-suggested-utterances.md). 
