...
## <a name="create-subscription-key"></a>Создание ключа подписки
Если срок действия бесплатного ключа истек, создайте ключ подписки.
...
В LUIS есть два места для использования ключа. Первое — [панель тестирования](train-test.md#view-bing-spell-check-corrections-in-test-panel). Ключ не сохраняется в LUIS, а является переменной сеанса. Ключ нужно будет задавать каждый раз, когда потребуется, чтобы панель тестирования применила службу API проверки орфографии Bing версии 7 к фрагменту речи. Сведения о настройке ключа см. в разделе [инструкций](train-test.md#view-bing-spell-check-corrections-in-test-panel) панели тестирования.
