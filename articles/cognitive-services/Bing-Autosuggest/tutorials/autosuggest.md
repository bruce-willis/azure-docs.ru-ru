---
title: Получение результатов автозаполнения с помощью API автозаполнения Bing (Azure Cognitive Services) | Документация Майкрософт
description: Демонстрация использования API автозаполнения Bing.
services: cognitive-services
author: v-jaswel
manager: kamrani
ms.assetid: 2575A80C-FC74-4631-AE5D-8101CF2591D3
ms.service: cognitive-services
ms.component: bing-autosuggest
ms.topic: article
ms.date: 09/28/2017
ms.author: v-jaswel
ms.openlocfilehash: f66af61ed06f4c6a7fe36cd44aa7781b71bb18ea
ms.sourcegitcommit: 95d9a6acf29405a533db943b1688612980374272
ms.translationtype: HT
ms.contentlocale: ru-RU
ms.lasthandoff: 06/23/2018
ms.locfileid: "35382148"
---
# <a name="build-a-web-page-autosuggest-client"></a>Создание клиента для автозаполнения веб-страницы

В рамках работы с этим руководством мы создадим веб-страницу, которая позволит пользователям запрашивать API автозаполнения Bing.

В этом учебнике описаны следующие процедуры.

> [!div class="checklist"]
> - Выполнение простого запроса к API автозаполнения Bing.
> - Отображение результатов запроса.

## <a name="prerequisites"></a>предварительным требованиям

Чтобы выполнить задания, описанные в этом руководстве, требуется ключ подписки для API автозаполнения Bing. Если у вас его нет, [зарегистрируйтесь, чтобы воспользоваться бесплатной пробной версией](https://azure.microsoft.com/try/cognitive-services/?api=autosuggest-api).

## <a name="create-a-new-web-page"></a>Создание веб-страницы

Откройте текстовый редактор. Создайте файл с именем, например, autosuggest.html.

## <a name="html-header"></a>Заголовок HTML

Добавьте сведения о заголовке HTML перед разделом script, как показано ниже.

```html
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>Bing Autosuggest</title>

<style type="text/css">
    html, body, div, p, h1, h2 {font-family: Verdana, "Lucida Sans", sans-serif;}

    html, body, div, p  {font-weight: normal;}
    h1, h2 {font-weight: bold;}
    sup {font-weight: normal;}

    html, body, div, p  {font-size: 12px;}
    h1 {font-size: 20px;}
    h2 {font-size: 16px;}
    h1, h2 {clear: left;}

    img#logo {float: right;
</style>

<script type="text/javascript">
```

## <a name="getsubscriptionkey-function"></a>Функция getSubscriptionKey

Функция getSubscriptionKey возвращает ключ API автозаполнения Bing. Он либо извлекается из локального хранилища (то есть из файла cookie), либо запрашивается у пользователя при необходимости.

Начните написание функции getSubscriptionKey и объявите имя файла cookie, как показано ниже.

```html
getSubscriptionKey = function() {

    var COOKIE = "bing-autosuggest-api-key";   // name used to store API key in key/value storage
```

Вспомогательная функция findCookie возвращает значение указанного файла cookie. Если файл cookie не найден, возвращается пустая строка.

```html
    function findCookie(name) {
        var cookies = document.cookie.split(";");
        for (var i = 0; i < cookies.length; i++) {
            var keyvalue = cookies[i].split("=");
            if (keyvalue[0].trim() === name) {
                return keyvalue[1];
            }
        }
        return "";
        }
```

Вспомогательная функция getSubscriptionKeyCookie запрашивает у пользователя значение ключа API автозаполнения Bing и возвращает значение ключа.

```html
    function getSubscriptionKeyCookie() {
        var key = findCookie(COOKIE);
        while (key.length !== 32) {
            key = prompt("Enter Bing Autosuggest API subscription key:", "").trim();
            var expiry = new Date();
            expiry.setFullYear(expiry.getFullYear() + 2);
            document.cookie = COOKIE + "=" + key.trim() + "; expires=" + expiry.toUTCString();
        }
        return key;
    }
```

Вспомогательная функция getSubscriptionKeyLocalStorage сначала пытается получить ключ API автозаполнения Bing, выполняя поиск соответствующего файла cookie. Если файл cookie не найден, значение ключа запрашивается у пользователя. Затем функция возвращает значение ключа.

```html
    function getSubscriptionKeyLocalStorage() {
        var key = localStorage.getItem(COOKIE) || "";
        while (key.length !== 32)
            key = prompt("Enter Bing Autosuggest API subscription key:", "").trim();
        localStorage.setItem(COOKIE, key)
        return key;
    }
```

Вспомогательная функция getSubscriptionKey принимает один параметр — **invalidate**. Если **invalidate** имеет значение **true**, функция getSubscriptionKey удаляет файл cookie, который содержит ключ API автозаполнения Bing. Если **invalidate** имеет значение **false**, функция getSubscriptionKey возвращает значение ключа API автозаполнения Bing.

```html
    function getSubscriptionKey(invalidate) {
        if (invalidate) {
            try {
                localStorage.removeItem(COOKIE);
            } catch (e) {
                document.cookie = COOKIE + "=";
            }
        } else {
            try {
                return getSubscriptionKeyLocalStorage();
            } catch (e) {
                return getSubscriptionKeyCookie();
            }
        }
    }
```

Верните вспомогательную функцию getSubscriptionKey в качестве результата внешней функции getSubscriptionKey. Закройте определение внешней функции getSubscriptionKey.

```html
    return getSubscriptionKey;

}();
```

## <a name="helper-functions"></a>Вспомогательные функции

Вспомогательная функция pre возвращает указанный текст, предварительно отформатированный с помощью HTML-тега [pre](https://www.w3schools.com/tags/tag_pre.asp).

```html
function pre(text) {
    return "<pre>" + text.replace(/&/g, "&amp;").replace(/</g, "&lt;") + "</pre>"
}
```

Функция renderSearchResults отображает указанные результаты из API автозаполнения Bing, используя автоматическое форматирование JSON.

```html
function renderSearchResults(results) {
    document.getElementById("results").innerHTML = pre(JSON.stringify(results, null, 2));
}
```

Функция renderErrorMessage отображает указанное сообщение об ошибке и код ошибки.

```html
function renderErrorMessage(message, code) {
    if (code)
        document.getElementById("results").innerHTML = "<pre>Status " + code + ": " + message + "</pre>";
    else
        document.getElementById("results").innerHTML = "<pre>" + message + "</pre>";
}
```

## <a name="bingautosuggest-function"></a>Функция bingAutosuggest

Функция bingAutosuggest вызывается каждый раз при вводе пользователем текста в поле HTML-формы.
Она принимает два параметра: содержимое поля HTML-формы и ключ API автозаполнения Bing.

```html
function bingAutosuggest(query, key) {
```

Укажите конечную точку API автозаполнения Bing и объявите объект XMLHttpRequest, который мы будем использовать для отправки запросов к конечной точке.

```html
    var endpoint = "https://api.cognitive.microsoft.com/bing/v7.0/Suggestions";

    var request = new XMLHttpRequest();

    try {
        request.open("GET", endpoint + "?q=" + encodeURIComponent(query));
    }
    catch (e) {
        renderErrorMessage("Bad request");
        return false;
    }
```

Задайте в качестве заголовка **Ocp-Apim-Subscription-Key** значение ключа API автозаполнения Bing.

```html
    request.setRequestHeader("Ocp-Apim-Subscription-Key", key);
```

Обработайте ответ конечной точки. Если состояние равно 200 (ОК), отобразите результаты, в противном случае отобразите сведения об ошибке.

```html
    request.addEventListener("load", function() {
        if (this.status === 200) {
            renderSearchResults(JSON.parse(this.responseText));
        }
        else {
            if (this.status === 401) getSubscriptionKey(true);
            renderErrorMessage(this.statusText, this.status);
        }
    });
```

Также обработайте возможные события ошибок из объекта XMLHttpRequest.

```html
    request.addEventListener("error", function() {
        renderErrorMessage("Network error");
    });

    request.addEventListener("abort", function() {
        renderErrorMessage("Request aborted");
    });
```

Отправьте запрос. Закройте функцию bingAutosuggest, тег **script** и тег **head**.

```html
    request.send();
    return false;
}
// --></script>

</head>
```

## <a name="html-body"></a>Основной текст HTML

При загрузке веб-страницы следует убедиться, что указан ключ API автозаполнения Bing, запросив его у пользователя при необходимости.

```html
<body onload="document.forms.bing.query.focus(); getSubscriptionKey();">
```

Отобразите логотип Bing.

```html
<img id="logo" align=base src="data:image/png;base64,">
```

Создайте HTML-форму с текстовым полем. Обработайте событие **oninput** и вызовите функцию bingAutosuggest, передав содержимое текстового поля и ключ API автозаполнения Bing.

```html
<form name="bing" oninput="return bingAutosuggest(this.query.value, getSubscriptionKey())">
    <h2>Autosuggest</h2>
    <input type="text" name="query" size="80" placeholder="Autosuggest" autocomplete=off>
</form>
```

Добавьте HTML-тег **div**, который мы используем для отображения результатов. Определенный ранее код JavaScript обращается к этому тегу **div**.

```html
<h2>Results</h2>
<div id="results">
<p>None yet.</p>

</div>

</body>
</html>
```

Сохраните файл.

## <a name="display-results"></a>Отображение результатов

Откройте веб-страницу в браузере. При появлении запроса введите ключ подписки для API автозаполнения Bing. Затем введите запрос (например, "sail") в текстовое поле **Автозаполнение**. По мере ввода веб-страница будет автоматически обновляться, отображая результаты автозаполнения.

```json
{
  "_type": "Suggestions",
  "queryContext": {
    "originalQuery": "sail"
  },
  "suggestionGroups": [
    {
      "name": "Web",
      "searchSuggestions": [
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=vheQSvKZylM3dlX_B9bQ8-hQEsEJo8zDD2y7H1nsBjE&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailor%2bbrinkley%2bcook%26FORM%3dUSBAPI&p=DevEx,5003.1",
          "displayText": "sailor brinkley cook",
          "query": "sailor brinkley cook",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=EStLqAfxGCa44Ur3jEMXBv-Qp-lXUSFJbkBfnUdKKDg&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailor%2bbrinkley%26FORM%3dUSBAPI&p=DevEx,5004.1",
          "displayText": "sailor brinkley",
          "query": "sailor brinkley",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=gvtP9TS9NwhajSapY2Se6y1eCbP2fq_GiP2n-cxi6OY&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailrite%26FORM%3dUSBAPI&p=DevEx,5005.1",
          "displayText": "sailrite",
          "query": "sailrite",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=c0QOA_j6swCZJy9FxqOwke2KslJE7ZRmMooGClAuCpY&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailboats%2bfor%2bsale%26FORM%3dUSBAPI&p=DevEx,5006.1",
          "displayText": "sailboats for sale",
          "query": "sailboats for sale",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=mnMdREUH20SepmHQH1zlh9Hy_w7jpOlZFm3KG2R_BoA&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailing%2banarchy%26FORM%3dUSBAPI&p=DevEx,5007.1",
          "displayText": "sailing anarchy",
          "query": "sailing anarchy",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=0udadFl0gCTKCp0QmzQTXS3_y08iO8FpwsoKPHPS6kw&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailboatdata%26FORM%3dUSBAPI&p=DevEx,5008.1",
          "displayText": "sailboatdata",
          "query": "sailboatdata",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=BTS0G6AakxntIl9rmbDXtk1n6rQpsZZ99aQ7ClE7dTY&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsail%2bsand%2bpoint%26FORM%3dUSBAPI&p=DevEx,5009.1",
          "displayText": "sail sand point",
          "query": "sail sand point",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=quBMwmKlGwqC5wAU0K7n416plhWcR8zQCi7r-Fw9Y0w&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailflow%26FORM%3dUSBAPI&p=DevEx,5010.1",
          "displayText": "sailflow",
          "query": "sailflow",
          "searchKind": "WebSearch"
        }
      ]
    }
  ]
}
```

## <a name="next-steps"></a>Дополнительная информация

> [!div class="nextstepaction"]
> [Требования к использованию и отображению](../UseAndDisplayRequirements.md)