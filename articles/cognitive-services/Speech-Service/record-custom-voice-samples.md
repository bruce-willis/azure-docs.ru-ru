...
Фразы не обязательно должны быть из одного источника или типа источника. Они даже могут быть не взаимосвязанными. Тем не менее, если в приложении речи вы будете использовать стандартные фразы (например: "Вход выполнен успешно"), включите их в сценарий. Так будет большая вероятность качественного воспроизведения этих фраз с помощью пользовательской голосовой модели. И если вы решите использовать запись вместо синтезированной речи, она уже будет создана с помощью того же голоса, что и синтезированная речь.
...
* Пустой столбец, куда вы будете записывать номер фрагмента или временной код каждого высказывания, поможет найти его в готовой записи речи.
...
Желательно, чтобы вы записывали свой сценарий в профессиональной студии звукозаписи, которая специализируется на работе с голосом. Там используется речевая студия, нужное оборудование и есть квалифицированный персонал. Не стоит экономить на записи.
...
Уровни должны быть установлены таким образом, чтобы большая часть доступного динамического диапазона цифровой записи использовалась без перенапряжения. Это означает, что уровень громкости не должен искажать звук. Ниже приведен пример формы колебаний хорошей записи.
...
> [!NOTE]
> Большинство студий звукозаписи предлагают электронное отображение сценариев в речевой студии. В этом случае введите примечания, которые нужно просмотреть, непосредственно в документ сценария. Однако помните, что вам все равно понадобится бумажная копия, в которую вы будете вносить заметки во время сеанса. Большинству звукоинженеров также понадобится печатная копия. Не лишним будет на всякий случай распечатать третью копию для актера — в случае сбоя компьютера.

Актер озвучивания может спросить, на каком слове в высказывании стоит сделать логическое ударение. Актеры называют такое слово ключевым. Скажите им, что вам нужно естественное чтение без особых выделений слов. Логические ударения можно добавить при синтезе речи, но они не должны быть частью исходной записи.
...
В начале сеанса создайте справочную запись или *файл соответствия* стандартного высказывания. Попросите актера повторять эту строку на каждой странице или после прохождения части страницы. Каждый раз сравнивайте новую запись со справочной. Эта практика поможет актеру придерживаться согласованности в громкости, темпе, подаче тона и интонации. В то же время инженер может использовать файл соответствия как справку по уровням и для общей согласованности звука.
...
> [!TIP]
> Все, что вам действительно нужно, это строки голоса актера, поэтому вы можете сделать монофоническую (с одним каналом) запись только нужных строк. Тем не менее, если вы записываете на стереопроигрыватель, вы можете использовать второй канал для записи обсуждения в диспетчерской, чтобы фиксировать обсуждение конкретных строк или фрагментов. Удалите эту запись из версии, загруженной на портал Custom Voice.
