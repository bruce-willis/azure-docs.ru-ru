...
description: >
  В этой статье рассматриваются различные способы распознавания намерений на основе речи (из файла или с микрофона) с помощью пакета SDK службы "Речь" для C++.
...
Код для этой статьи находится в папке `samples/cpp/windows/console`.
