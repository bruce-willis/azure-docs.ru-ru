...
1. С помощью кабеля Mini-USB, подключенного к ПК или источнику питания, подключите питание к набору средств разработки. Под верхней панелью должен засветиться зеленый индикатор питания.

1. С помощью второго кабеля Mini-USB подключите набор средств разработки к компьютеру.

    ![Подключение набора средств разработки](media/speech-devices-sdk/qsg-1.png)
...
1. Разместите набор разработки соответствующим образом.
    ...
    ![Расположение набора средств разработки с линейной решеткой](media/speech-devices-sdk/qsg-2.png)
...
1.  Подключитесь к беспроводной сети, щелкнув последовательно значок папки, затем **Параметры** и **WLAN**.

    ![Беспроводная локальная сеть Vysor](media/speech-devices-sdk/qsg-4.png)
 
 > [!NOTE]
 > Если в компании существуют политики, касающиеся подключения устройств к системе Wi-Fi, вам необходимо получить MAC-адрес и обратиться в ИТ-отдел, чтобы получить сведения о подключении к беспроводной сети. Чтобы узнать Mac-адрес набора средств разработки, щелкните значок папки с файлом на рабочем столе набора средств разработки, затем щелкните **Параметры**. Найдите элемент "MAC-адрес", щелкните **MAC-адрес**, чтобы перейти в раздел **Advanced WLAN** (расширенные параметры беспроводной сети), и запишите MAC-адрес, расположенный внизу. Кроме того, у некоторых компаний может быть ограничение подключения устройства к Wi-Fi-системам по времени. Возможно, через определенное количество дней понадобится продлить регистрацию набора средств разработки в Wi-Fi системе.  
 ...
 > Если необходимо подключить динамик к набору средств разработки, это можно сделать с помощью выходного канала звуковой частоты. Также следует выбрать качественный динамик с поддержкой разъема 3,5 мм.
...
1. По умолчанию слово для пробуждения (ключевое слово) — "Компьютер".  При желании можно попробовать одно из других предоставленных слов — "Машина" или "Помощник". Файлы ресурсов для этих альтернативных слов находятся в пакете SDK для речевых устройств в папке "keyword". Например, `C:\SDSDK\Android-Sample-Release\keyword\Computer` содержит файлы, используемые для слова "Компьютер".
    ...
    * Скопируйте файлы `kws.table`, `kws_g.fst`, `kws_k.fst` и `words_kw.txt` в папку устройства \data\keyword\. Выполните следующие команды в командном окне. Если было создано [пользовательское слово для активации](speech-devices-sdk-create-kws.md), то файл kws.table, сгенерированный из Интернета, будет находиться в том же каталоге, что и файлы `kws.table`, `kws_g.fst`, `kws_k.fst` и `words_kw.txt`. Чтобы принудительно отправить файл kws.table в пакет средств разработки, используйте команду adb push C:\SDSDK\Android-Sample-Release\keyword\[wake_word_name]\kws.table /data/keyword.
...
Если при использовании службы "Речь" происходят ошибки сертификата, убедитесь, что на устройстве правильно установлены дата и время. Для этого перейдите в системе к разделу **Параметры**, затем щелкните **Дата и время** и **Выберите часовой пояс**, где установите часовой пояс, который станет таковым по умолчанию. Сохраните параметр **Automatic date & time** (Синхронизация даты и времени) в состоянии "включено". Когда время, установленное в наборе средств разработчика, будет соответствовать времени ПК, это будет означать, что набор средств разработчика подключен к Интернету. 

 ![Папка файлов Vysor](media/speech-devices-sdk/qsg-12.png)
 
 ![Папка файлов Vysor](media/speech-devices-sdk/qsg-13.png)
