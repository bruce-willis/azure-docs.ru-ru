---
title: Проверка орфографии с помощью API Bing для проверки орфографии (Microsoft Cognitive Services в Azure) | Документация Майкрософт
description: Сведения об использовании Проверки орфографии Bing.
services: cognitive-services
author: v-jaswel
manager: kamrani
ms.assetid: 2575A80C-FC74-4631-AE5D-8101CF2591D3
ms.service: cognitive-services
ms.component: bing-spell-check
ms.topic: article
ms.date: 09/28/2017
ms.author: v-jaswel
ms.openlocfilehash: 4e4cdbb8a3d6ab01888d8f273083155c33eb06c1
ms.sourcegitcommit: 95d9a6acf29405a533db943b1688612980374272
ms.translationtype: HT
ms.contentlocale: ru-RU
ms.lasthandoff: 06/23/2018
ms.locfileid: "35380668"
---
# <a name="build-a-web-page-spell-check-client"></a>Создание веб-страницы клиента для проверки орфографии

В рамках работы с этим руководством мы создадим веб-страницу, с помощью которой пользователи смогут запрашивать API Bing для проверки орфографии.

В этом учебнике описаны следующие процедуры.

> [!div class="checklist"]
> - Выполнение простого запроса к API Bing для проверки орфографии.
> - Отображение результатов запроса.

## <a name="prerequisites"></a>предварительным требованиям

Чтобы выполнить задачи, описанные в этом руководстве, вам потребуется ключ подписки для API Bing для проверки орфографии. Если у вас его нет, [зарегистрируйтесь, чтобы воспользоваться бесплатной пробной версией](https://azure.microsoft.com/try/cognitive-services/?api=spellcheck-api).

## <a name="create-a-new-web-page"></a>Создание веб-страницы

Откройте текстовый редактор. Создайте файл с именем, например, spellcheck.html.

## <a name="add-html-header"></a>Добавление заголовка HTML

Добавьте сведения о заголовке HTML перед разделом script, как показано ниже.

```html
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8"> 
    <title>Bing Spell Check</title>

<style type="text/css">
    html, body, div, p, h1, h2 {font-family: Verdana, "Lucida Sans", sans-serif;}

    html, body, div, p  {font-weight: normal;}
    h1, h2 {font-weight: bold;}
    sup {font-weight: normal;}

    html, body, div, p  {font-size: 12px;}
    h1 {font-size: 20px;}
    h2 {font-size: 16px;}
    h1, h2 {clear: left;}

    img#logo {float: right;
</style>

<script type="text/javascript">
```

## <a name="getsubscriptionkey-function"></a>Функция getSubscriptionKey

Функция getSubscriptionKey возвращает ключ API Bing для проверки орфографии. Он либо извлекается из локального хранилища (то есть из файла cookie), либо запрашивается у пользователя при необходимости.

Начните написание функции getSubscriptionKey и объявите имя файла cookie, как показано ниже.

```html
getSubscriptionKey = function() {

    var COOKIE = "bing-spell-check-api-key";   // name used to store API key in key/value storage
```

Вспомогательная функция findCookie возвращает значение указанного файла cookie. Если файл cookie не найден, возвращается пустая строка.

```html
    function findCookie(name) {
        var cookies = document.cookie.split(";");
        for (var i = 0; i < cookies.length; i++) {
            var keyvalue = cookies[i].split("=");
            if (keyvalue[0].trim() === name) {
                return keyvalue[1];
            }
        }
        return "";
        }
```

Вспомогательная функция getSubscriptionKeyCookie запрашивает у пользователя значение ключа API Bing для проверки орфографии и возвращает значение ключа.

```html
    function getSubscriptionKeyCookie() {
        var key = findCookie(COOKIE);
        while (key.length !== 32) {
            key = prompt("Enter Bing Spell Check API subscription key:", "").trim();
            var expiry = new Date();
            expiry.setFullYear(expiry.getFullYear() + 2);
            document.cookie = COOKIE + "=" + key.trim() + "; expires=" + expiry.toUTCString();
        }
        return key;
    }
```

Вспомогательная функция getSubscriptionKeyLocalStorage сначала пытается получить ключ API Bing для проверки орфографии, выполняя поиск соответствующего файла cookie. Если файл cookie не найден, значение ключа запрашивается у пользователя. Затем функция возвращает значение ключа.

```html
    function getSubscriptionKeyLocalStorage() {
        var key = localStorage.getItem(COOKIE) || "";
        while (key.length !== 32)
            key = prompt("Enter Bing Spell Check API subscription key:", "").trim();
        localStorage.setItem(COOKIE, key)
        return key;
    }
```

Вспомогательная функция getSubscriptionKey принимает один параметр — **invalidate**. Если **invalidate** имеет значение **true**, функция getSubscriptionKey удаляет файл cookie, который содержит ключ API Bing для проверки орфографии. Если **invalidate** имеет значение **false**, функция getSubscriptionKey возвращает значение ключа API Bing для проверки орфографии.

```html
    function getSubscriptionKey(invalidate) {
        if (invalidate) {
            try {
                localStorage.removeItem(COOKIE);
            } catch (e) {
                document.cookie = COOKIE + "=";
            }
        } else {
            try {
                return getSubscriptionKeyLocalStorage();
            } catch (e) {
                return getSubscriptionKeyCookie();
            }
        }
    }
```

Верните вспомогательную функцию getSubscriptionKey в качестве результата внешней функции getSubscriptionKey. Закройте определение внешней функции getSubscriptionKey.

```html
    return getSubscriptionKey;

}();
```

## <a name="helper-functions"></a>Вспомогательные функции

Вспомогательная функция pre возвращает указанный текст, предварительно отформатированный с помощью HTML-тега [pre](https://www.w3schools.com/tags/tag_pre.asp).

```html
function pre(text) {
    return "<pre>" + text.replace(/&/g, "&amp;").replace(/</g, "&lt;") + "</pre>"
}
```

Функция renderSearchResults отображает указанные результаты из API Bing для проверки орфографии, используя автоматическое форматирование JSON.

```html
function renderSearchResults(results) {
    document.getElementById("results").innerHTML = pre(JSON.stringify(results, null, 2));
}
```

Функция renderErrorMessage отображает указанное сообщение об ошибке и код ошибки.

```html
function renderErrorMessage(message, code) {
    if (code)
        document.getElementById("results").innerHTML = "<pre>Status " + code + ": " + message + "</pre>";
    else
        document.getElementById("results").innerHTML = "<pre>" + message + "</pre>";
}
```

## <a name="bingspellcheck-function"></a>Функция bingSpellCheck

Функция bingSpellCheck вызывается каждый раз, когда пользователь вводит текст в поле HTML-формы.
Она принимает два параметра: содержимое поля HTML-формы и ключ API Bing для проверки орфографии.

```html
function bingSpellCheck(query, key) {
```

Укажите конечную точку API Bing для проверки орфографии и объявите объект XMLHttpRequest, который мы будем использовать для отправки запросов к конечной точке.

```html
    var endpoint = "https://api.cognitive.microsoft.com/bing/v7.0/spellcheck/";

    var request = new XMLHttpRequest();

    try {
        request.open("GET", endpoint + "?mode=proof&mkt=en-US&text=" + encodeURIComponent(query));
    }
    catch (e) {
        renderErrorMessage("Bad request");
        return false;
    }
```

Задайте в качестве заголовка **Ocp-Apim-Subscription-Key** значение ключа API Bing для проверки орфографии.

```html
    request.setRequestHeader("Ocp-Apim-Subscription-Key", key);
```

Обработайте ответ конечной точки. Если указано состояние "200 (ОК)", отобразите результаты. В противном случае отобразите сведения об ошибке.

```html
    request.addEventListener("load", function() {
        if (this.status === 200) {
            renderSearchResults(JSON.parse(this.responseText));
        }
        else {
            if (this.status === 401) getSubscriptionKey(true);
            renderErrorMessage(this.statusText, this.status);
        }
    });
```

Также обработайте возможные события ошибок из объекта XMLHttpRequest.

```html
    request.addEventListener("error", function() {
        renderErrorMessage("Network error");
    });

    request.addEventListener("abort", function() {
        renderErrorMessage("Request aborted");
    });
```

Отправьте запрос. Закройте функцию bingSpellCheck, тег **script** и тег **head**.

```html
    request.send();
    return false;
}
// --></script>

</head>
```

## <a name="html-body"></a>Основной текст HTML

При загрузке веб-страницы следует убедиться, что указан ключ API Bing для проверки орфографии, запросив его у пользователя при необходимости.

```html
<body onload="document.forms.bing.query.focus(); getSubscriptionKey();">
```

Отобразите логотип Bing.

```html
<img id="logo" align=base src="data:image/png;base64,">
```

Создайте HTML-форму с текстовым полем. Обработайте событие **onsubmit** и вызовите функцию bingSpellCheck, передав содержимое текстового поля и ключ API Bing для проверки орфографии.

```html
<form name="bing" onsubmit="return bingSpellCheck(this.query.value, getSubscriptionKey())">
    <h2>Spell Check</h2>
    <input type="text" name="query" size="80" placeholder="Spell Check" autocomplete=off>
</form>
```

Добавьте HTML-тег **div**, который мы используем для отображения результатов. Определенный ранее код JavaScript обращается к этому тегу **div**.

```html
<h2>Results</h2>
<div id="results">
<p>None yet.</p>

</div>

</body>
</html>
```

Сохраните файл.

## <a name="display-results"></a>Отображение результатов

Откройте веб-страницу в браузере. При появлении запроса введите ключ подписки для API Bing для проверки орфографии. Введите запрос (например, "Hollo, wlrd!") в текстовое поле **Spell Check** (Проверка орфографии) и нажмите клавишу **ВВОД**. Веб-страница отобразит результаты запроса.

```json
{
  "_type": "SpellCheck",
  "flaggedTokens": [
    {
      "offset": 0,
      "token": "Hollo",
      "type": "UnknownToken",
      "suggestions": [
        {
          "suggestion": "Hello",
          "score": 0.856629936217145
        },
        {
          "suggestion": "Hollow",
          "score": 0.816717853225633
        }
      ]
    },
    {
      "offset": 7,
      "token": "wlrd",
      "type": "UnknownToken",
      "suggestions": [
        {
          "suggestion": "world",
          "score": 0.856629936217145
        }
      ]
    }
  ]
}
```

## <a name="next-steps"></a>Дополнительная информация

> [!div class="nextstepaction"]
> [Требования к использованию и отображению](../UseAndDisplayRequirements.md)
