...
2. Щелкните действие "Undo step" (Отменить шаг).
    - Последняя запись удаляется, и вы возвращаетесь к этапу "Would you like anything else?" (Вы хотите добавить что-то еще?).  (Снимок экрана представлен ниже.)
...
3. Щелкните "Ветвь".
    - Элемент "Нет" удаляется и весь диалог вплоть до этой точки копируется в новое окно. 
