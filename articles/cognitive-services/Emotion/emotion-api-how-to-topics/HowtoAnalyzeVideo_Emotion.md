---
title: Анализ видео в реальном времени с помощью API распознавания эмоций | Документация Майкрософт
description: Выполнение анализа практически в реальном времени по кадрам, взятым из видеотрансляции, с помощью API распознавания эмоций в службах Cognitive Services.
...
> [!IMPORTANT]
> Срок действия предварительной версии API для поиска видео истек 30 октября 2017 г. Оцените новую [предварительную версию API Индексатора видео](https://azure.microsoft.com/services/cognitive-services/video-indexer/), который позволяет легко извлекать аналитические сведения из видео и оптимизировать возможности обнаружения содержимого (например, результаты поиска) путем определения произнесенных слов, лиц, символов и эмоций. [Узнайте больше](https://docs.microsoft.com/azure/cognitive-services/video-indexer/video-indexer-overview).

В этом руководстве показано, как выполнить анализ практически в реальном времени по кадрам, взятым из видеотрансляции. Основные компоненты такой системы:
...
Выполнять анализ видеопотоков практически в реальном времени можно несколькими способами. Мы опишем три подхода в порядке повышения уровня сложности.

### <a name="a-simple-approach"></a>Простой подход
Простейшая конструкция системы анализа практически в реальном времени — это бесконечный цикл, где в каждой итерации мы захватываем кадр, анализируем его и затем используем результат:
```CSharp
while (true)
{
    Frame f = GrabFrame();
