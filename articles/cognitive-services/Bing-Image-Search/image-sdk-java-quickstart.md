...
[Исходный код примеров для пакета SDK для службы "Поиск изображений Bing" для Java](https://github.com/Azure-Samples/cognitive-services-java-sdk-samples/tree/master/Search/BingImageSearch) доступен на сайте GitHub. 

## <a name="application-dependencies"></a>Зависимости приложения
Получите [ключ доступа Cognitive Services](https://azure.microsoft.com/try/cognitive-services/) в разделе **Поиск**. Установите зависимости пакета SDK Bing для поиска изображений с помощью Maven, Gradle или другой системы управления зависимостями. Для файла POM Maven требуется объявление:
```
 <dependencies>
    <dependency>
...
```
## <a name="image-search-client"></a>Клиент для поиска изображений
...
```
Реализуйте клиент **ImageSearchAPIImpl**. Для этого требуется экземпляр класса **ServiceClientCredentials**.
```
...
```
Выполните поиск изображений по запросу "Canadian Rockies". Проверьте число результатов. Распечатайте значения параметров **firstImageResult**, **pivotSuggestions** и **queryExpansions**.
```
...
```
Выполните поиск изображений по запросу "Gibraltar" и отфильтруйте результаты по анимационным GIF и широкоформатным изображениям. Проверьте число результатов. Распечатайте значения параметров **insightsToken**, **thumbnailUrl** и **webUrl** для первого результата.
```
...
```
Выполните поиск популярных изображений. Проверьте параметры **categories** и **tiles**.
```
...
```
Выполните поиск изображений по запросу "Degas", а затем выполните поиск сведений о первом изображении из списка результатов. 
```
