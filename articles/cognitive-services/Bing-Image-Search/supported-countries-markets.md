---
title: Поддерживаемые страны и языки для API "Поиск изображений Bing" в Azure | Документация Майкрософт
...
ms.locfileid: "35382092"
---
# <a name="bing-image-search-countries-and-languages"></a>Страны и языки в API Bing для поиска изображений

API Bing для поиска изображений поддерживает более трех десятков стран, причем во многих из них используется несколько языков. Указание страны в запросе служит в основном для уточнения результатов поиска на основании интересующей информации в данной стране. Кроме того, результаты могут содержать ссылки на Bing, позволяющие локализовать пользовательский интерфейс Bing в соответствии с указанной страной или языком.

Чтобы указать страну и язык, задайте в качестве значения параметра запроса `mkt` (рынок) код из приведенной ниже таблицы **Рынки**. Код рынка задает как страну, так и язык. Если пользователь предпочитает, чтобы текст отображался на другом языке, установите соответствующий код языка в качестве значения параметра запроса `setLang`.

Кроме того, можно указать страну с помощью параметра запроса `cc`. Указывая страну, необходимо также задать один или несколько кодов языков с помощью HTTP-заголовка `Accept-Language`. Поддерживаемые языки различаются в зависимости от страны; они заданы для каждой страны в таблице "Рынки".
