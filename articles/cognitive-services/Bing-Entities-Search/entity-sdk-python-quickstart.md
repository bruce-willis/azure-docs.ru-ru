---
title: Краткое руководство по пакету SDK для API "Поиск сущностей" для Python | Документация Майкрософт
description: Установка компонентов консольного приложения пакета SDK для API "Поиск сущностей".
...
ms.locfileid: "35382592"
---
# <a name="entity-search-sdk-python-quickstart"></a>Краткое руководство по пакету SDK для API "Поиск сущностей" для Python
...
```
Установите зависимости пакета SDK для API "Поиск сущностей Bing":
```
cd mytestenv
...
```
Выполните поиск одной сущности "Gibralter" и выведите на экран краткое описание:
```
...
```
Выполните поиск и обработайте результаты устранения неоднозначности для неоднозначного запроса "William Gates" (Уильям Гейтс).
```
...
```
Выполните поиск одного магазина "Microsoft Store" и выведите его номер телефона.
```
...
```
Выполните поиск списка ресторанов по запросу "Seattle restaurants" и выведите на экран их названия и номера телефонов.
```
...
```
Отправьте недопустимый запрос и получите в ответ сообщение об ошибке.
```
