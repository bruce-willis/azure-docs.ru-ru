---
title: Что такое Поиск сущностей Bing? | Документация Майкрософт
...
Если ответ или результат содержит поля `contractualRules`, `attributions`, или `provider` поля, вы должны определить принадлежность данных. Если эти поля отсутствуют, это делать не нужно. Когда ответ содержит поле `contractualRules` и поле `attributions` и (или) `provider`, принадлежность данных следует определить с помощью договорных правил.
