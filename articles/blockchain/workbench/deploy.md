...
3. Добавьте новый ключ, указав его в поле **Описание** и выбрав значения длительности в поле **Срок действия истекает**. 
    ...
    ![Создание ключа](media/deploy/app-key-create.png)
...
7.  Завершите **установку Azure Active Directory**.
    ...
    ![Установка Azure AD](media/deploy/blockchain-workbench-settings-aad.png)
...
9.  В разделе **Параметры и производительность сети** выберите "Создать новую сеть блокчейнов или использовать существующую сеть блокчейнов PoA".

    Параметр **Создать**.

    Параметр *Создать* создает набор узлов Ethereum Proof-of Authority (PoA) в рамках подписки одного участника. 

    ![Параметры и производительность сети](media/deploy/blockchain-workbench-settings-network-new.png)
    ...
    * Сеть блокчейнов PoA должна быть настроена так, чтобы цена на газ устанавливалась равной нулю (Заметка: учетные записи Blockchain Workbench не финансируются. Если требуется финансирование, транзакции не выполняются).

    ![Параметры и производительность сети](media/deploy/blockchain-workbench-settings-network-existing.png)

    | Параметр | ОПИСАНИЕ  |
    |---------|--------------|
    | Конечная точка Ethereum RPC | Укажите конечную точку RPC существующей сети блокчейнов PoA. Конечная точка начинается с http:// и заканчивается номером порта. Например, `http://contoso-chain.onmicrosoft.com:8545` |
    | Хранение и производительность | Выберите предпочтительную производительность хранилища виртуальной машины для сети блокчейнов. |
    | размер виртуальной машины; | Выберите предпочтительный размер виртуальной машины для сети блокчейнов. |

10. Чтобы завершить работу с параметрами и производительностью сети, нажмите кнопку **ОК**.
