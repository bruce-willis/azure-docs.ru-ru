...
1. Создайте файловые ресурсы, используя тома, взятые из хранилища StorSimple. Дополнительные сведения см. в статье [Использование службы StorSimple Manager для управления томами](storsimple-manage-volumes.md).
   ...
   1. Откройте портал Azure и выберите вкладку **Тома и устройства**. Щелкните **Автонастройка**. После этого должен отобразиться созданный том.
...
1. [Включите защиту виртуальной машины](../site-recovery/site-recovery-hyper-v-site-to-azure.md) файлового сервера на портале Azure Site Recovery.
...
1. В учетной записи службы автоматизации щелкните **Variables** (Переменные), &gt;**Добавить переменную** и добавьте приведенные ниже переменные. Вы также можете зашифровать эти ресурсы. Переменные зависят от плана восстановления. Если имя плана восстановления, который вы создадите на следующем шаге, — TestPlan, то у переменных будут имена TestPlan-StorSimRegKey, TestPlan-AzureSubscriptionName и т. п.
   ...
   - *RecoveryPlanName***-ResourceGroupName** — группа Resource Manager с ресурсом StorSimple.
   ...
   - *RecoveryPlanName***-DeviceIpAddress** — IP-адрес устройства (можно найти на вкладке **Устройства** в разделе диспетчера устройств StorSimple &gt; **Параметры** &gt; **Сеть** &gt; в группе **Параметры DNS**).
   ...
   - *RecoveryPlanName***-TargetDeviceName** — облачное устройство StorSimple, на которое нужно выполнить отработку отказа контейнеров.
   - *RecoveryPlanName***-TargetDeviceIpAddress** — IP-адрес целевого устройства (его можно найти в разделе **Виртуальная машина** группы &gt; **Параметры**&gt; на вкладке **Сетевые подключения**).
...
1. В группе **Управление** выберите параметр **Планы восстановления (Site Recovery)** и создайте план восстановления, выполнив указанные ниже действия.
   ...
   - Добавьте ручное действие после четырех сценариев, указанных выше, в том же разделе **Группа 1: последующие действия** . Это действие позволяет проверить, все ли работает правильно. Его необходимо добавить только как часть тестовой отработки отказа (поэтому установите только флажок **Тестовая отработка отказа**).
    
   - После ручного действия добавьте скрипт **очистки**, выполнив ту же процедуру, что и для других модулей Runbook. **Сохраните** план восстановления.
...
1. После создания реплицированных виртуальных машин они находятся в состоянии ожидания фиксации. Для фиксации отработки отказа выберите команду **Зафиксировать** .
...
1. После отработки отказа виртуальные машины будут находиться в состоянии ожидании фиксации. Для фиксации отработки отказа выберите команду **Зафиксировать** .
