---
title: 'Пример политики Azure в формате JSON: утвержденные образы виртуальных машин | Документация Майкрософт'
description: Пример политики в формате JSON требует, чтобы в среде развертывались только утвержденные пользовательские образы.
...
- Дополнительные примеры шаблонов для служб "Политика Azure" и "Виртуальные машины" см. в статье [Применение политик к виртуальным машинам Windows с помощью Azure Resource Manager](../../virtual-machines/windows/policy.md).