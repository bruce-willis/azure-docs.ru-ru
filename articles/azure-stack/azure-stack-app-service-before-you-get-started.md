...
В обновлении 1802 Azure Stack добавлена поддержка доменов сбоя. Новые развертывания Службы приложений Azure в Azure Stack распределяются по доменам сбоя, что обеспечивает отказоустойчивость.

Для существующих экземпляров Службы приложений Azure в Azure Stack, развернутых до выпуска обновления 1802, см. статью [Распределение поставщика ресурсов службы приложений по доменам сбоя](azure-stack-app-service-fault-domain-update.md).

Кроме того, разверните необходимые экземпляры файлового сервера и SQL Server в конфигурации с высоким уровнем доступности.
