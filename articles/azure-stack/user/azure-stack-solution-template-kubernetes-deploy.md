...
> [!Note]  
> Kubernetes для службы Azure Kubernetes (AKS) в Azure Stack доступен в закрытой предварительной версии. Вашему оператору Azure Stack потребуется запросить доступ к элементу Kubernetes в Marketplace, необходимому для выполнения инструкций в этой статье.
...
1. Убедитесь, что при входе используется клиент Azure AD, связанный с экземпляром Azure Stack.
...
1. Выберите **Больше служб** > **Пользовательские подписки** > **+ Добавить**.

1. Выберите созданную подписку.
...
1. Выберите **+Создать** > **Среда выполнения приложений** > **Kubernetes Cluster (Кластер Kubernetes)**. Нажмите кнопку **Создать**.

    ![Развертывание шаблона решений](media/azure-stack-solution-template-kubernetes-deploy/01_kub_market_item.png)

1. В колонке "Создание кластера Kubernetes" выберите раздел **Основные сведения**.
...
1. Введите **идентификатор клиента**. Если вам нужна помощь в поиске этого значения, см. раздел [Get tenant ID](https://docs.microsoft.com/azure/azure-resource-manager/resource-group-create-service-principal-portal#get-tenant-id) (Получение идентификатора клиента). 
