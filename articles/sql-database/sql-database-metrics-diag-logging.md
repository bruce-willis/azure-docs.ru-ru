...
Ведение журналов метрик и диагностики не включено по умолчанию. Вы можете включить ведение этих журналов и управлять ими, используя один из следующих методов:
...
При включении ведения журналов метрик и диагностики необходимо указать ресурс Azure, где собираются выбранные данные. Доступны следующие варианты:
...
- [Timeouts](sql-database-metrics-diag-logging.md#time-outs-dataset) содержит сведения о времени ожидания в базе данных.
- [Blocks](sql-database-metrics-diag-logging.md#blockings-dataset) содержит сведения о блокирующих событиях, произошедших в базе данных.
...
- **Audit** / **SQLSecurityAuditEvents** в настоящее время недоступно.
...
Журналы метрик и диагностики Базы данных SQL можно передать в концентратор событий с помощью встроенной возможности **Stream to an event hub** (Передать в концентратор событий) на портале. Можно также включить идентификатор правила служебной шины с помощью параметра диагностики через командлеты PowerShell, Azure CLI или Azure Monitor REST API. 

### <a name="what-to-do-with-metrics-and-diagnostics-logs-in-event-hubs"></a>Что делать с журналами метрик и диагностики в концентраторах событий
...
* **Потоковая передача журналов в сторонние потоки ведения журналов и сбора телеметрии.** С помощью потоковой передачи концентраторов событий вы можете передать журналы метрик и диагностики в другие сторонние решения мониторинга и Log Analytics. 
