...
## <a name="enable-logging-for-a-database"></a>Включение ведения журналов для базы данных

Ведение журналов метрик и диагностики в Базе данных SQL и базе данных Управляемого экземпляра не включено по умолчанию. Вы можете включить ведение этих журналов в базе данных, а также управлять им, используя один из следующих методов:
...
При включении ведения журналов метрик и диагностики необходимо указать ресурс Azure, где будут собраны выбранные данные. Доступны следующие варианты:

- Аналитика SQL
- Центры событий;
- служба хранилища. 

Вы можете подготовить новый ресурс Azure или выбрать имеющийся. После выбора ресурса необходимо указать, какие данные нужно собирать, используя параметр диагностики базы данных. Ниже приведены доступные параметры, поддерживающие Базу данных SQL Azure и базу данных Управляемого экземпляра.

| Мониторинг телеметрии | Поддержка Базы данных SQL Azure | Поддержка базы данных в Управляемом экземпляре |
| :------------------- | ------------------- | ------------------- |
...
**Примечание**. Чтобы использовать журналы аудита и SQLSecurityAuditEvents (хотя эти параметры доступны в параметрах диагностики базы данных), их необходимо включить только через решение **Аудит SQL** для настройки потоковой передачи телеметрии в Log Analytics, концентратор событий или хранилище.
...
Ведение журналов метрик и диагностики для эластичных пулов и Управляемого экземпляра не включено по умолчанию. Вы можете включить ведение этих журналов для эластичного пула или Управляемого экземпляра и управлять им. Для сбора доступны следующие данные:

| Мониторинг телеметрии | Поддержка эластичного пула | Поддержка Управляемого экземпляра |
| :------------------- | ------------------- | ------------------- |
| [Все метрики](sql-database-metrics-diag-logging.md#all-metrics) (эластичные пулы) содержат сведения о проценте использования DTU и ЦП, ограничении DTU и ЦП, проценте чтения физических данных, проценте записей в журнал, проценте сеансов, проценте рабочих ролей, хранилище, проценте хранилища, ограничении хранилища и проценте хранилища XTP. | Yes | Недоступно |
| [ResourceUsageStats](sql-database-metrics-diag-logging.md#resource-usage-stats) (Управляемый экземпляр) содержит количество виртуальных ядер, средний процент использования ЦП, а также сведения о запросах ввода-вывода, прочитанных и записанных байтах, зарезервированном и используемом дисковом пространстве. | Недоступно | Yes |
...
- Чтобы включить сбор журналов метрик и диагностики для Базы данных SQL или баз данных Управляемого экземпляра, перейдите в базу данных и выберите **Параметры диагностики**. Выберите **+Add diagnostic setting** (+ Добавить параметр диагностики), чтобы настроить новый параметр, или **Настройка параметра**, чтобы изменить существующий.

   ![Включение на портале Azure](./media/sql-database-metrics-diag-logging/enable-portal.png)

- Для **Базы данных SQL Azure** создайте или измените существующие параметры диагностики, выбрав целевой объект и телеметрию.

   ![Параметры диагностики](./media/sql-database-metrics-diag-logging/diagnostics-portal.png)

- Для **базы данных Управляемого экземпляра** создайте параметры диагностики или измените существующие, выбрав целевой объект и телеметрию.
...
Найдите подробное содержимое телеметрии мониторинг для метрик и журналов, доступных для Базы данных SQL Azure, эластичных пулов, Управляемого экземпляра и баз данных в Управляемом экземпляре.
