...
В этой статье показано, как Contoso переносит приложение SmartHotel в Azure и перепроектирует его архитектуру. Contoso переносит интерфейсную виртуальную машину в контейнер Azure для Windows, а базу данных приложения — в базу данных Azure SQL.
...
2. Она подготавливают экземпляр SQL Azure и переносит в него базу данных SmartHotel.
...
Контейнер SmartHotel будет запускаться в кластере Azure Service Fabric. Администраторы Contoso создают кластер Service Fabric следующим образом:
...
Теперь администраторы Contoso могут перенести базу данных SmartHotel с помощью DMA.
...
18. По завершении развертывания SmartHotel теперь будет работать в Service Fabric.
...
2. По завершении сборки и развертывания SmartHotel будет работать в Service Fabric. На консоли управления Service Fabric теперь отображаются три службы.
...
В этой статье специалисты компании Contoso выполнили рефакторинг приложения SmartHotel в Azure путем миграции виртуальной машины внешнего интерфейса приложения в Service Fabric. База данных приложения была перенесена в базу данных Azure SQL.
