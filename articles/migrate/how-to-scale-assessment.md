...
- **Параметры статистики.** Перед началом развертывания для сервера vCenter Server нужно задать параметры статистики уровня 3. Следует установить уровень статистики 3 для каждого из интервалов сбора дня, недели и месяца. Если уровень ниже 3 для любого из интервалов сбора, оценка будет выполняться, но данные о производительности хранилища и сети не будут собираться. В этом случае рекомендации по размеру будут основаны на данных производительности ЦП и памяти, а также на данных конфигурации диска и сетевых адаптеров.
...
1.  Создайте пользователя для каждого клиента. Затем с помощью [RBAC](https://docs.microsoft.com/azure/role-based-access-control/role-assignments-portal) назначьте пользователям права только на чтение всех виртуальных машин, принадлежащие конкретному клиенту. Затем используйте эти учетные данные для обнаружения. RBAC гарантирует, что каждый пользователь vCenter получит доступ только к виртуальным машинам соответствующего клиента.
2. Чтобы настроить RBAC для пользователей разных клиентов, воспользуйтесь следующим примером пользователей User#1 и User#2:
    ...
    - Datacenter1 — предоставьте поьзователям User#1 и User#2 разрешения только для чтения. Не распространяйте эти разрешения на дочерние объекты, поскольку эти разрешения вы будете настраивать отдельно для каждой виртуальной машины.

      - VM1 (клиент #1) (разрешение только на чтение для User#1)
      - VM2 (клиент #1) (разрешение только на чтение для User#1)
      - VM3 (клиент #2) (разрешение только на чтение для User#2)
      - VM4 (клиент #2) (разрешение только на чтение для User#2)

   - При обнаружении с учетными данными пользователя User#1 будут обнаружены только виртуальные машины VM1 и VM2.
