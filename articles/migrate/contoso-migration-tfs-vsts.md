...
В этой статье показано, как Contoso выполняет рефакторинг локального развертывания TFS, перемещая его в Visual Studio Team Services (VSTS) в Azure. Команда разработчиков Contoso использовала TFS для совместной работы и контроля версий в течение последних пяти лет. Теперь они хотят перейти на облачное решение для разработки и тестирования, а также для управления версиями. VSTS сыграет роль, когда они перейдут на модель DevOps и разработают новые облачные приложения.

Это одиннадцатый документ в цикле статей о том, как вымышленная компания Contoso переносит локальные ресурсы в облако Microsoft Azure. Этот цикл содержит общие сведения и сценарии, которые иллюстрируют настройку инфраструктуры миграции и выполнение миграций различных типов. Сценарии описаны в порядке возрастания сложности. Со временем мы добавим в этот цикл и другие статьи.
...
Статья 11. Рефакторинг развертывания TFS в VSTS | Перенос TFS приложения разработки в VSTS в Azure | Эта статья
...
Команда ИТ-руководителей тесно сотрудничала с сотрудниками компании для определения будущих целей. Деловые партнеры не слишком озабочены средствами и технологиями разработки, но они отметили следующие моменты:
...
- **Эффективность**. Contoso необходимо оптимизировать процесс и удалить ненужные процедуры для разработчиков и пользователей. Это позволит им более эффективно выполнять требования клиентов. Компания нуждается в быстрой работе ИТ-отдела без потери времени или денег.
...
- Им нужен инструмент для переноса данных в облако. Необходимо несколько процессов, выполняемых вручную.
...
- Они обеспокоены изменениями и хотят протестировать их, прежде чем выполнять полный переход. Они хотят сохранить доступ к TFS даже после перехода на VSTS.
- Они имеют несколько коллекций и хотят начать с той, которая содержит всего несколько проектов, чтобы лучше понять процесс.
- Они понимают, что коллекции TFS поддерживают отношения "один к одному" с учетными записями VSTS, поэтому они будут иметь несколько URL-адресов. Однако это соответствует их текущей модели разделения для баз кода и проектов.
...
1. Потребуется серьезная подготовка. В качестве первого шага компания Contoso должна обновить свою реализацию TFS до поддерживаемого уровня. В настоящее время они используют TFS 2017 с обновлением 3, но для переноса базы данных им необходимо запустить поддерживаемую версию 2018 с последними обновлениями.
2. После обновления они запустят средство миграции TFS и проведут проверку своей коллекции.
3. Они создадут набор файлов подготовки и выполнят пробный перенос для тестирования.
4. Затем они выполнят другой перенос, на этот раз полный, включающий рабочие элементы, ошибки, спринты и код.
...
> * **Шаг 2. Обновление TFS**. Они обновят развертывание до TFS 2018 с обновлением 2. 
> * **Шаг 3. Проверка коллекции**. Они проверят свою коллекцию TFS для подготовки к миграции.
> * **Шаг 4. Создание файла подготовки**. Они создадут файлы миграции с помощью средства миграции TFS. 


## <a name="step-1-create-a-storage-account"></a>Шаг 1. Создание учетной записи хранения

1. На портале Azure компания Contoso создает учетную запись хранения (**contosodevmigration**).
...
Компания Contoso обновляет свой сервер TFS до версии TFS 2018 с обновлением 2. Перед запуском:
...
5. Contoso проверяет установку TFS, просматривая проекты, рабочие элементы и код.
...
Contoso запускает средство миграции TFS в базе данных коллекции ContosoDev для ее проверки перед переносом.
...
4. Contoso находит эту запись, связанную с удостоверением.

    ![TFS](./media/contoso-migration-tfs-vsts/collection4.png)

5. Contoso запускает **TfsMigration validate /help** в командной строке и видит, что для проверки удостоверений требуется значение параметра **/tenantDomainName**.
...
После завершения проверки Contoso может использовать средство миграции TFS для создания файлов миграции.
...
4. Теперь Contoso может видеть, что файлы IdentityMapLog.csv и import.json были созданы в новой папке.

    ![Подготовка.](./media/contoso-migration-tfs-vsts/prep4.png)

5. Файл import.json предоставляет параметры импорта. Он включает в себя информацию, такую ​​как имя требуемой учетной записи и данные учетной записи хранения. Большинство полей заполнены автоматически. Некоторым полям потребовался ввод пользователя. Contoso открывает файл и добавляет созданное имя учетной записи VSTS: **contosodevmigration**. С этим именем их URL-адрес VSTS будет таким: **contosodevmigration.visualstudio.com**.
...
После подготовки Contoso может сосредоточиться на миграции. После выполнения миграции они перейдут с использования TFVC на Git для контроля версий.

Прежде чем начать, Contoso планирует время простоя с командой разработчиков, чтобы перевести коллекцию в автономный режим для миграции. Ниже приведены шаги процесса миграции.
...
5. **Пробное выполнение**. Помогает при пробном переносе коллекции. Пробное выполнение имеет ограниченный срок службы и удаляется до того, как начнется миграция. Экземпляры выполнения автоматически удаляются через заданный период времени. Напоминание о том, когда пробное выполнение будет удалено, включается в сообщение электронной почты, полученное после завершения импорта. Примите его к сведению и планируйте соответствующим образом.
6. **Завершение рабочей миграции**. По завершению пробной миграции Contoso выполнит окончательную миграцию, обновив файл import.json и снова запустив импорт.



### <a name="detach-the-collection"></a>Отключение коллекции

До запуска Contoso берет локальную резервную копию SQL Server и моментальный снимок VMware сервера TFS, прежде чем отключить коллекцию.
...
Contoso создает резервную копию (DACPAC) для импорта в VSTS.
...
Contoso создает DACPAC следующим образом:
...
Ранее Contoso частично заполняла файл спецификации импорта (import.json). Теперь нужно добавить остальные параметры.
...
Contoso выполняет тестовую миграцию, позволяющую убедиться в правильной работе всех компонентов.
...
11. Через 15 минут специалист Contoso переходит по URL-адресу и видит следующую информацию:
...
15. Он также проверяет другие проекты и код, чтобы подтвердить, что исходный код и история были перенесены.
...
После завершения пробного запуска Contoso переходит к миграции рабочей среды. Они удаляют данные пробного выполнения, обновляют параметры импорта и снова запускают импорт.
...
5. На странице входа в Azure AD Contoso указывает учетные данные администратора Contoso.
...
7. Через 15 минут специалист Contoso переходит по URL-адресу и видит следующую информацию:

    ![Производство](./media/contoso-migration-tfs-vsts/full5.png)

8. После завершения миграции команда разработчиков Contoso входит в VSTS, чтобы проверить, работает ли миграция. После входа в систему можно увидеть, что проекты были перенесены.
...
9. Он проверяет данные других рабочих элементов для подтверждения.
...
После завершения миграции Contoso хочет перейти с использования TFVC на Git для управления исходным кодом. Им необходимо импортировать исходный код, в настоящее время находящийся в учетной записи VSTS, в репозиторий Git в той же учетной записи.
...
3. На вкладке **Тип источника** они выбирают **TFVC** и указывают путь к репозиторию. Они решают не переносить историю.
    ...
    > [!NOTE]
    > Из-за различий в хранении информации о контроле версий в TFVC и Git рекомендуется не переносить историю. Это тот подход, который Майкрософт предприняла при переносе Windows и других продуктов из системы централизованного управления версиями в Git.

4. После импорта Contoso проверяет код.
