...
В этой статье показано, как Contoso повторно размещает локальное двухуровневое приложение службы поддержки для Linux (osTicket) путем его переноса в Azure и Azure MySQL.
...
[Статья 5. Повторное размещение на виртуальных машинах Azure](contoso-migration-rehost-vm.md) | Здесь показано, как Contoso переносит свое приложение SmartHotel в виртуальные машины инфраструктуры как услуги Azure, используя службу Site Recovery. | Доступна
...
[Статья 7. Повторное размещение приложения Linux на виртуальных машинах Azure](contoso-migration-rehost-linux-vm.md) | Здесь показано, как Contoso переносит свое приложение osTicket для Linux в виртуальные машины инфраструктуры как услуги Azure, используя Azure Site Recovery. | Доступна
Статья 8. Повторное размещение приложения Linux на виртуальных машинах Azure и сервере MySQL в Azure | Здесь демонстрируется выполняемый в Contoso процесс миграции приложения osTicket для Linux. Для миграции виртуальных машин используется Site Recovery, а для миграции в экземпляре сервера MySQL в Azure — MySQL Workbench. | Эта статья.
