...
Статья 2. Развертывания инфраструктуры Azure (эта статья) | Здесь рассказывается о том, как Contoso готовит свою локальную инфраструктуру Azure для миграции. Для всех сценариев миграции Contoso используется одна и та же инфраструктура. | Доступна
...
[Статья 7. Повторное размещение приложения Linux на виртуальных машинах Azure](contoso-migration-rehost-linux-vm.md) | Здесь демонстрируется выполняемый в Contoso процесс миграции приложения osTicket для Linux в виртуальные машины Azure. | Доступна
[Статья 8. Повторное размещение приложения Linux на виртуальных машинах Azure и сервере MySQL в Azure](contoso-migration-rehost-linux-vm-mysql.md) | Здесь демонстрируется, как Contoso осуществляет миграцию приложения Linux osTicket с помощью Site Recovery и миграцию (резервное копирование и восстановление) в экземпляр сервера MySQL в Azure с помощью MySQL Workbench. | Доступна
...
Крайне важно, чтобы перед миграцией в Azure компания Contoso подготовила свою инфраструктуру.  Как правило, для этого необходимо уделить особое внимание пяти областям.
...
Contoso собирается заключить [Соглашение Enterprise](https://azure.microsoft.com/pricing/enterprise-agreement/) (EA). В связи с этим компании необходимо заплатить аванс за использование Azure, что, в свою очередь, дает большие преимущества, в том числе гибкие варианты оплаты и оптимальные цены.
...
После оплаты Azure Contoso необходимо разобраться, как управлять своими подписками. Компания заключила Соглашение Enterprise, поэтому она может настроить ограниченное количество подписок Azure.
...
- Contoso решает расширить свою локальную службу Active Directory для работы в облаке вместо создания новой отдельной системы в Azure.
- Для этого специалисты компании создают Active Directory на базе Azure.
- У компании Contoso нет подписки на Office 365, поэтому ей нужно подготовить новую службу Azure AD.
...
Contoso использует выпуск Azure AD Free, который предоставляется в составе подписки Azure. Добавьте новый каталог AD, как показано ниже.
...
Специалисты Contoso выбрали в качестве основного региона восточную часть США 2 (находится в Вирджинии), а в качестве дополнительного региона — центральную часть США. Такой выбор был сделан по ряду причин.
...
- У Contoso нет [решения IPAM](https://docs.microsoft.com/windows-server/networking/technologies/ipam/ipam-top), поэтому компании нужно спланировать сетевую маршрутизацию без NAT.
...
В этой статье описано, как специалисты компании Contoso настроили инфраструктуру Azure и определили или спланировали применяемую к ней политику для подписки Azure, удостоверения в гибридной среде, аварийного восстановления, сетевого взаимодействия, управления и обеспечения безопасности. 
