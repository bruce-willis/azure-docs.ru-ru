...
#### <a name="internet-connectivity-with-intercepting-proxy"></a>Подключение к Интернету с помощью перехватывающего прокси-сервера

Если прокси-сервер, используемый для подключения к Интернету, является перехватывающим, необходимо импортировать сертификат прокси-сервера в виртуальную машину вашего сборщика. Ниже приведены указания по импорту сертификата в виртуальную машину сборщика.

1. В виртуальной машине сборщика перейдите в меню **Пуск**, найдите и откройте раздел **Управление сертификатами компьютеров**.
2. В средстве "Сертификаты" в области слева в разделе **Сертификаты — локальный компьютер** найдите **Доверенные издатели**. В разделе **Доверенные издатели** щелкните **Сертификаты**, чтобы просмотреть список сертификатов в области справа.

    ![Средство "Сертификаты"](./media/concepts-intercepting-proxy/certificates-tool.png)

3. Скопируйте сертификат прокси-сервера в виртуальную машину сборщика. Для получения этого сертификата может потребоваться обратиться к команде администраторов сети в вашей организации.
4. Дважды щелкните сертификат, чтобы открыть его. Щелкните **Установить сертификат**. Откроется мастер импорта сертификатов.
5. В мастере импорта сертификатов для параметра "Расположение хранилища" выберите **Локальный компьютер**. Щелкните **Далее**.

    ![Расположение хранилища сертификатов](./media/concepts-intercepting-proxy/certificate-store-location.png)

6. Выберите **Поместить все сертификаты в следующее хранилище**. Нажмите кнопку **Обзор** и выберите **Доверенные издатели** из появившегося списка сертификатов. Щелкните **Далее**.

    ![Хранилище сертификатов](./media/concepts-intercepting-proxy/certificate-store.png)
    
7. Нажмите кнопку **Готово** После этого сертификат будет импортирован. 
8. При необходимости можно проверить, что сертификат импортирован, открыв средство "Сертификаты", как на шаге 1 и 2 выше.
9. В приложении сборщика "Миграция Azure" убедитесь, что проверка готовности к установке подключения к Интернету прошла успешно.
...
Рекомендуем непрерывно выполнять обновление Windows для модуля сборщика. Если сборщик не обновляется в течение 60 дней, он начинает автоматически завершать работу компьютера. Если выполняется обнаружение, компьютер не отключится даже по истечении 60-дневного периода. Когда задание обнаружения будет завершено, компьютер отключится. Если сборщик используется более 45 дней, рекомендуем постоянно поддерживать компьютер в обновленном состоянии, запуская средство обновления Windows.
...
#### <a name="upgrade-to-version-10911"></a>Обновление до версии 1.0.9.11
