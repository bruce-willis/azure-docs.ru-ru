...
> [!NOTE]
> Мы планируем реализовать поддержку Hyper-V и физических серверов. Пока для планирования миграции рабочих нагрузок Hyper-V мы рекомендуем использовать [Планировщик развертывания Azure Site Recovery](http://aka.ms/asr-dp-hyperv-doc), а для планирования физических рабочих нагрузок — [партнерские средства](https://azure.microsoft.com/migration/partners/).


- Вы можете обработать до 1500 виртуальных машин при помощи одной операции обнаружения и до 1500 виртуальных машин в одном проекте. Кроме того, можно обработать до 1500 виртуальных машин при помощи одной операции оценки. Если нужно обнаружить среду большего размера, можно разделить операцию обнаружения и создать несколько проектов ([дополнительные сведения](how-to-scale-assessment.md)). Служба "Миграция Azure" поддерживает до 20 проектов на подписку.
...
**Тип хранилища** | Вы можете указать тип дисков, которые нужно выделить в Azure. Это свойство применимо, если размер изменяется как при определении размера локального экземпляра. В качестве типа целевого диска можно указать управляемые диски класса Premium дисков или Standard. Значение по умолчанию — управляемые диски класса Premium. Для изменения размера на основе производительности рекомендуемый размер диска автоматически определяется на основе данных производительности виртуальных машин. Учтите, что служба "Миграция Azure" поддерживает управляемые диски только для внутренней оценки миграции.
**Условия определения размера** | Условия, используемые в службе "Миграция Azure", для установки правильного размера виртуальных машин Azure. Вы можете определять размер локальных виртуальных машин на основе *журнала производительности* или устанавливать для виртуальных машин Azure размер как *для локальных* без учета журнала производительности. Значение по умолчанию — как при определении размера локального экземпляра.
**Предложение Azure** | Вы можете указать [предложение Azure](https://azure.microsoft.com/support/legal/offer-details/), в котором вы зарегистрированы, и в службе "Миграция Azure" будет выполнена соответствующая оценка расходов.
**Преимущество гибридного использования Azure** | Вы можете указать, есть ли у вас лицензия Software Assurance и используете ли вы программу [Преимущество гибридного использования Azure](https://azure.microsoft.com/pricing/hybrid-use-benefit/), чтобы получить скидку.
**Зарезервированные экземпляры** |  Также вы можете указать, есть ли у вас [зарезервированные экземпляры](https://azure.microsoft.com/pricing/reserved-vm-instances/), и в службе "Миграция Azure" будет выполнена соответствующая оценка расходов.
**Время доступности виртуальной машины** | Если виртуальные машины не должны работать в Azure круглосуточно, можно указать период, на протяжении которого они будут работать в Azure, и будет выполнена соответствующая оценка расходов.
