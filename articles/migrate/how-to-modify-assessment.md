...
2. Измените свойства в соответствии со сведениями, приведенными в таблице ниже.
    ...
    **Тип хранилища** | Вы можете указать тип дисков, которые нужно выделить в Azure. Это свойство применимо, если размер изменяется как при определении размера локального экземпляра. В качестве типа целевого диска можно указать управляемые диски класса Premium или Standard. Для изменения размера на основе производительности рекомендуемый размер диска автоматически определяется на основе данных производительности виртуальных машин. Учтите, что служба "Миграция Azure" поддерживает управляемые диски только для внутренней оценки миграции. | По умолчанию используются управляемые диски класса Premium (с критерием изменения размеров *как в локальной среде*).
