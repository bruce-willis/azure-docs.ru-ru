...
1. Щелкните **Встроенные конечные точки**.
1. Раздел **События** содержит следующие значения: **Event Hub-compatible endpoint** (Конечная точка, совместимая с центрами событий), **Event Hub-compatible name** (Имя, совместимое с центрами событий), **Разделы**, **Время хранения** и **Группы потребителей**.
