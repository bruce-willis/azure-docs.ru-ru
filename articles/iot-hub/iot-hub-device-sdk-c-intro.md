...
Введите в первое поле **строку подключения Центра Интернета вещей** и нажмите кнопку **Обновить**. Теперь средство настроено для обмена данными с Центром Интернета вещей. **Строку подключения** можно найти в разделе **IoT Hub Service (Служба Центра Интернета вещей)** > **Параметры** > **Политика общего доступа** > **iothubowner**.
...
Выполнив описанные выше действия, вы будете готовы к запуску кода. В большинстве примеров в верхней части основного исходного файла есть константа для ввода строки подключения. Например, соответствующая строка из приложения **iothub\_client\_sample\_mqtt** выглядит, как показано ниже.
