...
Если вы хотите направить исходящий трафик ASE не в Интернет, а в другое расположение, доступны приведенные далее варианты.

* Настройка в ASE прямого доступа к Интернету.
* Настройка подсети ASE для игнорирования маршрутов BGP
* Настройка подсети ASE для использования конечной точки службы для Azure SQL и службы хранилища Azure
...
Подсети ASE можно настроить для игнорирования маршрутов BGP.  Если эта настройка выполнена, ASE сможет без проблем обращаться к своим зависимостям.  Однако для доступа приложения к локальным ресурсам потребуется создать определяемые пользователем маршруты.
...
* на портале Azure откройте пользовательский интерфейс для таблицы маршрутов, назначенных подсети ASE.  Выберите конфигурацию.  Задайте параметру распространения маршрутов BGP значение "Выкл.".  Нажмите кнопку "Сохранить". Сведения об отключении см. в разделе [Создание таблицы маршрутов][routetable].

После этого у ваших приложений не будет доступа к локальным ресурсам. Чтобы решить этот вопрос, измените определяемый пользователем маршрут, назначенный подсети ASE, и добавьте маршруты для диапазонов локальных адресов. В качестве значения типа следующего прыжка задайте "Шлюз виртуальной сети". 


## <a name="configure-your-ase-with-service-endpoints"></a>Настройка среды ASE с помощью конечной точки службы ##

 > [!NOTE]
   > Конечные точки службы SQL не работают с ASE в регионах для государственных организаций США.  Следующие сведения действительны только в общедоступных регионах Azure.  
...
2. Включение конечных точек службы хранилища Azure в подсеть ASE
...
4. _Чтобы установить адреса для исходящего трафика в имеющейся среде службы приложений,_ войдите на сайт resource.azure.com и перейдите в раздел Subscription/<subscription id>/resourceGroups/<ase resource group>/providers/Microsoft.Web/hostingEnvironments/<ase name>. Затем вы сможете просмотреть код JSON, в котором описана ваша среда службы приложений. Убедитесь, что вверху отображается строка **read/write**. Выберите **Изменить** Прокрутите вниз. Измените значение **userWhitelistedIpRanges** с **null** на строку, приведенную ниже. Используйте адреса, которые нужно задать в качестве диапазона адресов исходящего трафика. 
   ...
   Щелкните **PUT** в верхней части. Этот параметр запускает операцию масштабирования в среде службы приложений и настраивает брандмауэр.
