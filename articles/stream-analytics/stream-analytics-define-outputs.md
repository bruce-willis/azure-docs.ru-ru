...
| Хранилище больших двоичных объектов Azure | Yes | Используйте в шаблоне пути токены {date} и {time} {fieldname} из полей событий. Выберите формат даты, например ГГГГ-ММ-ДД, ДД-ММ-ГГГГ, ММ-ДД-ГГГГ. Для времени используется формат ЧЧ. В рамках [предварительной версии](https://aka.ms/ASAPreview) выходные данные большого двоичного объекта можно секционировать с помощью одного атрибута настраиваемого события {fieldname} или {datetime:\<описатель>}. | Соответствует секционированию входных данных для [полностью распараллеливаемых запросов](stream-analytics-scale-jobs.md). | 
