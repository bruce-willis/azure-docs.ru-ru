...
3. Выберите параметр **Статический IP-адрес** и укажите в текстовом поле "Адрес" доступный адрес из диапазона службы автоматического назначения IP-адресов (APIPA): 169.254.0.1–169.254.255.254. В данном примере можно использовать любой адрес из этого диапазона. Пример: `169.254.0.1`. Затем нажмите кнопку **ОК**.
