...
### <a name="add-the-front-end-ip-address-for-the-wsfc"></a>Добавление IP-адреса внешнего интерфейса для WSFC

IP-адрес WSFC также должен находиться в подсистеме балансировки нагрузки. 

1. На портале добавьте новую конфигурацию интерфейсного IP-адреса для WSFC. Используйте IP-адрес, настроенный для WSFC в ресурсах ядра кластера. Укажите статический IP-адрес. 
...
1. Настройте следующие правила балансировки нагрузки.
   | Параметр | ОПИСАНИЕ | Пример
   | --- | --- |---
   | **Имя** | текст | WSFCPointListener |
   | **Frontend IP address** (Интерфейсный IP-адрес) | Выберите адрес. |Используйте адрес, который был создан при настройке IP-адреса WSFC. |
   | **Протокол** | Выберите протокол TCP. |TCP |
   | **Порт** | Создайте порт для прослушивателя группы доступности | 58888 |
