...
Каждый подготовленный сервер поставляется с предварительно настроенными наборами Ethernet. Ниже приведены сведения об интерфейсах Ethernet, настроенных на каждой единице HLI.

- **A**: используется для клиентского доступа.
- **B**: используется для обмена данными между узлами. Настраивается на всех серверах (независимо от запрошенной топологии), но используется только для сценариев горизонтального масштабирования.
...
Ethernet "B" должен иметь IP-адрес, используемый для взаимодействия с NFS. Таким образом, эти адреса **не** нужно сохранять в файле etc/hosts, чтобы разрешить трафик между экземплярами в одном клиенте.
...
- Ethernet "B" должен иметь IP-адрес, используемый для взаимодействия с хранилищем NFS. Этот тип адресов не нужно указывать в файле etc/hosts.

- Ethernet "C" должен использоваться исключительно в файле etc/hosts, чтобы обеспечить взаимодействие между разными экземплярами. Эти адреса также применяются в конфигурациях масштабирования HANA как IP-адреса, используемые системой HANA для конфигурации между узлами.

- Ethernet "D" должен использоваться исключительно для доступа устройства STONITH к Pacemaker. Это требуется при настройке репликации системы HANA (HSR) и когда требуется автоматическая отработка отказа на уровне операционной системы с использованием устройства, основанного на SBD.
...
Эта топология поддерживает два узла для конфигурации репликации системы HANA (HSR). 
...
Эта топология поддерживает два узла для конфигурации репликации системы HANA (HSR). Поддерживается как обычное, так и многоцелевое аварийное восстановление. 
...
В этой топологии поддерживается несколько узлов в развертывании с аварийным восстановлением. Поддерживается как обычное, так и многоцелевое аварийное восстановление. На схеме показано только аварийное восстановление узкого назначения. Эту топологию можно запросить с резервным узлом или без него.
