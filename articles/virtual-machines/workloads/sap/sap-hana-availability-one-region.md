...
- Повторное развертывание виртуальных машин, которые изначально были размещены на работоспособном сервере узла, если узел после перезагрузки находится в неработоспособном состоянии. В этом случае исходный узел помечается как неработоспособный. Он не будет использоваться в дальнейших развертываниях, пока не будет очищен или заменен.
...
>[!IMPORTANT]
>При восстановлении работоспособности службы Azure виртуальные машины Linux, на которых находится гостевая ОС с ядром в критическом состоянии, не будут перезапущены. Параметры по умолчанию для обычно используемых выпусков Linux автоматически не перезапускают виртуальные машины или сервер, где ядро Linux находится в критическом состоянии. Вместо этого для ОС сохраняется критическое состояние ядра, чтобы подключить отладчик ядра для анализа. Azure следует такому поведению по умолчанию, не перезапуская виртуальную машину с подобной гостевой ОС автоматически. Предполагается, что такие случаи встречаются крайне редко. Вы можете перезаписать поведение по умолчанию, чтобы инициировался перезапуск виртуальной машины. Чтобы изменить поведение по умолчанию, задайте параметр "kernel.panic" в разделе /etc/sysctl.conf. Для этого параметра устанавливается время в секундах. Рекомендуем подождать 20–30 секунд, прежде чем запускать перезагрузку с помощью этого параметра. Ознакомьтесь с командой <https://gitlab.com/procps-ng/procps/blob/master/sysctl.conf>.
...
Этот сценарий с одной виртуальной машиной можно улучшить, добавив узел холодной отработки отказа в конфигурацию SAP HANA. В документации по SAP HANA это называется [автоматической отработкой отказа узла](https://help.sap.com/viewer/6b94445c94ae495c83a19646e7c3fd56/2.0.01/en-US/ae60cab98173431c97e8724856641207.html). Эту конфигурацию целесообразно использовать в локальных развертываниях, когда аппаратное обеспечение сервера ограничено и вы назначаете один узел сервера узлом автоматической отработки для набора рабочих узлов. Однако в Azure, когда базовая инфраструктура обеспечивает работоспособный целевой сервер для успешной перезагрузки виртуальной машины, нет смысла развертывать сценарий автоматической отработки отказа узла SAP HANA. По причине восстановления работоспособности службы Azure у нас нет эталонной архитектуры, которая предусматривает резервный узел для автоматической отработки отказа узла HANA. 

### <a name="special-case-of-sap-hana-scale-out-configurations-in-azure"></a>Особый случай конфигураций горизонтального масштабирования SAP HANA в Azure
В основе обеспечения высокого уровня доступности для конфигураций горизонтального масштабирования SAP HANA лежит восстановление работоспособности службы виртуальных машин Azure и перезапуск экземпляра SAP HANA при повторном запуске виртуальной машины. Архитектуры высокого уровня доступности на основе репликации системы HANA будут представлены позже. 


## <a name="availability-scenarios-for-two-different-vms"></a>Сценарии доступности с двумя разными виртуальными машинами

При использовании двух виртуальных машин Azure в группах доступности Azure позволяет увеличить время бесперебойной работы между этими двумя виртуальными машинами, если они размещены в группе доступности Azure в пределах одного региона Azure. Базовая установка в Azure будет выглядеть следующим образом:
