...
1. На портале Azure проверьте данные и щелкните **Создать**.
   
    ![Создание на портале][portal]
1. В колонке **Основные данные** введите имя кластера, которое также является именем виртуальной машины на головном узле. Для развертывания можно указать существующую группу ресурсов или создать новую в доступном вам расположении. Расположение влияет на доступность определенных размеров виртуальных машин и других служб Azure (см. статью [Доступность продуктов по регионам](https://azure.microsoft.com/regions/services/)).
1. В колонке **Параметры головного узла** при первом развертывании обычно можно принять параметры по умолчанию. 
   ...
   > 
   > 
1. В колонке **Compute node settings** (Параметры вычислительных узлов) выберите шаблон имени для узлов, их количество и размер, а также дистрибутив Linux для развертывания.
1. В колонке **Параметры инфраструктуры** введите имена виртуальной сети и домена Active Directory, учетные данные администратора домена и виртуальной машины, а также формат имени для учетных записей хранения.
   
   > [!NOTE]
   > Пакет HPC использует домен Active Directory для проверки подлинности пользователей кластера. 
   > 
   > 
1. После выполнения проверок и ознакомления с условиями использования щелкните **Приобрести**.
...
1. Перейдите в каталог, в котором установлен скрипт (в данном случае E:\IaaSClusterScript).
...
1. Откройте окно Windows PowerShell и выполните следующие команды:
