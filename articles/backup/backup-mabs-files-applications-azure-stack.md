---
title: Резервное копирование файлов и приложений Azure Stack
...
Эта статья не охватывает установку Azure Backup Server в среде Azure Stack. Инструкции по установке Azure Backup Server в Azure Stack см. в статье [о подготовке к резервному копированию рабочих нагрузок с помощью Azure Backup Server](backup-mabs-install-azure-stack.md).


## <a name="back-up-azure-stack-vm-file-data-to-azure"></a>Резервное копирование файлов виртуальной машины Azure Stack в Azure

Чтобы настроить защиту виртуальных машин IaaS в Azure Backup Server, откройте консоль Azure Backup Server. Эта консоль позволяет настроить группы защиты и параметры защиты для виртуальных машин.
...
3. На экране **Выбор членов группы** щелкните **+**, чтобы развернуть список вложенных элементов. Установите флажок для всех элементов, которые нужно защитить. Завершив выбор элементов, щелкните **Далее**.
    ...
    Корпорация Майкрософт рекомендует поместить в одну группу защиты все виртуальные машины, для которых применяется одинаковая политика защиты. Полные сведения о планировании и развертывании групп защиты см. в статье System Center DPM [Развертывание групп защиты](https://docs.microsoft.com/en-us/system-center/dpm/create-dpm-protection-groups?view=sc-dpm-1801).
...
12. На экране **Выберите репликацию в сети** укажите, как должна выполняться начальная полная репликация данных. 
    ...
    Можно выполнить репликацию по сети или сделать автономную резервную копию (автономное заполнение). Для автономного резервного копирования используется [функция импорта Azure](./backup-azure-backup-import-export.md).
...
Важные вопросы. Как увеличить объем дискового пространства для краткосрочного хранилища дисков Azure Stack? Какие рекомендации необходимо учитывать в контексте краткосрочного хранилища дисков?
...
2. В разделе точек восстановления есть календарь, в котором полужирным шрифтом выделены даты с доступными точками восстановления. Выберите дату, чтобы восстановить соответствующую точку восстановления.
...
6. Укажите настройки восстановления.
    ...
    * Установите флажок **Разрешить восстановление на основе SAN с использованием моментальных снимков оборудования**, чтобы использовать моментальные снимки оборудования на базе SAN, что позволяет быстрее выполнять восстановление. Этот вариант доступен только в том случае, если у вас настроена сеть SAN с поддержкой аппаратных моментальных снимков. Чтобы точка восстановления была доступна для записи, у SAN должны быть полномочия на создание и разделение клона. Защищенная виртуальная машина и Azure Backup Server должны быть подключены к одной сети SAN.
...
## <a name="register-azure-backup-server-with-a-vault"></a>Регистрация Azure Backup Server в хранилище
Опишите шаги для следующих операций:
