...
1. Создайте [учетную запись хранения Azure](../storage/common/storage-quickstart-create-account.md), чтобы настроить ее для отчетов. Эта учетная запись используется для хранения данных, связанных с отчетами.
...
2. Щелкните **Get Data** (Получить данные) и выберите **Получить** в разделе **Службы** в библиотеке пакетов содержимого. Выполните действия, указанные в статье [Подключение к службам с помощью пакетов содержимого для Power BI](https://powerbi.microsoft.com/documentation/powerbi-content-packs-services/).
...
5. Введите ключ для этой учетной записи хранения. [Ключи учетных записей хранения можно просматривать и копировать](../storage/common/storage-account-manage.md#access-keys), перейдя к своей учетной записи хранения на портале Azure. 
