...
Поддерживаются перечисленные ниже операционные системы. Поддерживаются виртуальные машины Azure SQL marketplace и виртуальные машины, не поддерживающие marketplace (с установленным вручную SQL Server).
...
При возникновении ошибки во время процесса установки **UserErrorSQLNoSysadminMembership** войдите в SQL Server Management Studio (SSMS) с учетной записью, имеющей разрешение sysadmin SQL. Если вам не требуются специальные разрешения, можно использовать проверку подлинности Windows.
...
2. В папке "Logins" (Имена входа) щелкните правой кнопкой мыши и выберите **новое имя входа**, и в диалоговом окне создания имени входа нажмите кнопку **Поиск**
...
5. В меню "Цель резервного копирования", нажмите кнопку **Настройка службы Backup**.
    ...
    > [!NOTE]
    > Для группы доступности SQL Always On мы используем параметры резервного копирования SQL. Но, в связи с ограничениями платформы SQL, полные и разностные резервные копии происходят от основного узла. Резервное копирование журналов может происходить на основе параметров резервного копирования. Из-за этого ограничения первичный узел всегда должен быть зарегистрирован для групп доступности.
    >
...
8. После внесения всех изменений в политику архивации нажмите кнопку **ОК**. 
   ...
   ![принятие новой политики](./media/backup-azure-sql-database/backup-policy-click-ok.png)
...
Кроме того, можно выбрать конкретную полную или разностную резервную копию для восстановления определенной точки восстановления, а не конкретный момент времени.
 > [!Note]
 > До активации восстановления базы данных master запустите SQL Server в однопользовательском режиме с помощью параметра запуска "-m AzureWorkloadBackup". Аргумент для -m — это имя клиента. Только этому клиенту будет разрешено открывать подключение. Для всех системных баз данных (model, master, msdb) остановите службу агента SQL до запуска восстановления. Закройте все приложения, которые могут попытаться использовать подключение к любой из этих баз данных.
>
...
> [!NOTE]
> Вы можете восстановить базы данных на сервере SQL Server в том же регионе Azure, и сервер назначения должен быть зарегистрирован в хранилище служб восстановления. 
>
...
Служба Azure Backup является решением корпоративного класса, поэтому она предоставляет расширенные оповещения и уведомления о резервном копировании на случай сбоя (см. раздел "Оповещения о резервном копировании" ниже). Если вы по-прежнему хотите отслеживать определенные задания, используйте любой из следующих параметров в зависимости от ваших требований:

#### <a name="use-azure-portal-for-all-adhoc-operations"></a>Использование портала Azure для всех нерегламентированных операций
Azure Backup отображает все запущенные вручную или нерегламентированные задания на портале заданий резервного копирования. Задания, доступные на портале, включают все настроенные операции резервного копирования, запущенные вручную операции резервного копирования, операции восстановления, операции регистрации и обнаружения баз данных и прекращение операций резервного копирования. 
![меню "Расширенная конфигурация"](./media/backup-azure-sql-database/jobs-list.png)

> [!NOTE]
> Все запланированные задания резервного копирования, включая полное резервное копирование, разностное резервное копирование и резервное копирование журналов, не будут отображаться на портале. Их можно отслеживать в SQL Server Management Studio, как описано ниже.
>
...
## <a name="sql-database-backup-faq"></a>Вопросы и ответы по резервному копированию базы данных SQL

В разделе ниже приведены дополнительные сведения о резервном копировании базы данных SQL.

### <a name="can-i-throttle-the-speed-of-the-sql-backup-policy-so-it-minimizes-impact-on-the-sql-server"></a>Можно ли регулировать скорость в политике резервного копирования SQL, чтобы свести к минимуму влияние на SQL Server

Да, вы можете регулировать скорость выполнения политики резервного копирования. Чтобы изменить этот параметр:

1. В SQL Server в папке `C:\Program Files\Azure Workload Backup\bin` откройте **TaskThrottlerSettings.json**.
...
4. В SQL Server откройте диспетчер задач и перезагрузите **службу координатора рабочей нагрузки резервного копирования Azure**.
...
### <a name="do-successful-backup-jobs-create-alerts"></a>Существуют ли оповещения об успешно выполненных заданиях резервного копирования

Нет. Успешно выполненные задания резервного копирования не создают оповещений. Оповещения отправляются только в случае сбоя.

### <a name="are-scheduled-backup-job-details-shown-in-the-jobs-menu"></a>Сведения о запланированных заданиях резервного копирования отображаются в меню "Задания"

Нет. В меню "Задания" отображаются сведения о нерегламентированных, но не о запланированных заданиях резервного копирования. В случае сбоя запланированного задания резервного копирования все сведения вы найдете в оповещениях о сбое задания. Все запланированные и нерегламентированные задания резервного копирования можно отслеживать в [SQL Server Management Studio](backup-azure-sql-database.md#use-sql-server-management-studio-for-backup-jobs).

### <a name="if-i-select-a-sql-server-will-future-databases-automatically-be-added"></a>Если я выберу SQL Server, будут ли будущие базы данных добавляться автоматически

Нет. Если при настройке защиты для SQL Server вы установите соответствующий флажок на уровне сервера, все базы данных будут добавлены. Но если вы добавите базы данных в SQL Server после настройки защиты, вам придется добавить их вручную, чтобы обеспечить защиту. Базы данных не включаются в настроенную защиту автоматически.

### <a name="if-i-change-the-recovery-model-how-do-i-restart-protection"></a>Если изменить модель восстановления, как перезапустить защиту

Если вы измените модель восстановления, активируйте полное резервное копирование, и резервное копирование журналов начнется должным образом.
