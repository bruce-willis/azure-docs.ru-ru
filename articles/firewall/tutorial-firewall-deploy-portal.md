...
В первую очередь всегда обрабатываются коллекции правил сети. Применение нескольких правил невозможно, то есть если совпадение найдено в коллекции правил сети, следующие коллекции правил приложений для сеанса не обрабатываются.
...
В примерах из статей о брандмауэре Azure предполагается, что вы уже включили общедоступную предварительную версию брандмауэра Azure. Дополнительные сведения см. в [этой статье](public-preview.md).
...
> [!NOTE]
> Общедоступный IP-адрес должен быть типа SKU "Стандартный".

[//]: # (Не забудьте записать частный IP-адрес для брандмауэра.)
...
1. На странице **Test-FW01** в разделе **Параметры** щелкните **Правила**.
2. Выберите **Добавление коллекции правил приложений**.
3. В поле **Имя** введите **App-Coll01**.
1. В поле **Приоритет** введите **200**.
2. В поле **Действие** выберите **Разрешить**.

6. В разделе **Правила** в качестве **имени** введите **AllowGH**.
7. В поле **Адреса источника** введите **10.0.2.0/24**.
8. В поле **Протокол:порт** введите **http, https**. 
9. В поле **Целевые полные доменные имена** введите **github.com**
10. Щелкните **Добавить**.

> [!NOTE]
> Брандмауэр Azure содержит встроенную коллекцию правил для целевых полных доменных имен инфраструктуры, которые разрешены по умолчанию. Эти доменные имена предназначены для платформы и не могут использоваться для других целей. Разрешенные имена FQDN инфраструктуры предоставляют следующие возможности:
>- доступ к хранилищу репозитория образов платформы (PIR);
>- доступ к хранилищу состояний управляемых дисков;
>- диагностика Windows.
>
> Вы можете переопределить эту встроенную коллекцию правил инфраструктуры, создав коллекцию правил приложений, *запрещающую все*, которая обрабатывается в последнюю очередь. Она всегда будет обрабатываться перед коллекцией правил инфраструктуры. Все, что не входит в коллекцию правил инфраструктуры, по умолчанию будет отклонено.
...
Удалите ставшую ненужной группу ресурсов **Test-FW-RG**, чтобы удалить связанные с брандмауэром ресурсы.
