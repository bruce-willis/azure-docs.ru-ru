...
Служба "Брандмауэр Azure" не поддерживает концепцию правил входящего и исходящего трафика. Это правила приложений и сетей, которые применяются к любому трафику, поступающему в брандмауэр. Сначала применяются правила сети, затем — правила приложения, после чего выполнение правил завершается.

Например, если правило сети выполняется, пакет не обрабатывается правилами приложения. Если правило сети не выполняется, а протокол пакета — HTTP/HTTPS, пакет обрабатывается правилами приложения. Если правило и после этого не выполняется, пакет обрабатывается с использованием коллекции правил инфраструктуры. Если правило по-прежнему не выполняется, пакет отклоняется по умолчанию.
...
> [!Note]
> Сейчас теги FQDN можно настроить только в Azure PowerShell и REST. См. [дополнительные сведения](https://aka.ms/firewallapplicationrule). 
