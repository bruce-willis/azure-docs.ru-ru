...
1. Введите положительное целое число для **приоритета** развертывания. Если одному устройству назначены два или несколько развертываний, будет выбрано развертывание с самым высоким числовым значением параметра приоритета.
1. Введите **условие назначения**, чтобы определить, для каких устройств будет предназначено это развертывание. Условие основывается на тегах двойника устройства и (или) его требуемых свойствах. Оно должно соответствовать формату выражения. Например, `tags.environment='test'` или `properties.desired.devicemodel='4000x'`. 
