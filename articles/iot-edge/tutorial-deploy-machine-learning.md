...
> Ниже приведены шаги по ее отключению. Однако это не подходит для использования в рабочей среде.

Чтобы отключить процесс идентификации, необходимо предоставить IP-адрес и порт для **workload_uri** и **management_uri** в разделе **Подключения** конфигурации управляющей программы IoT Edge.
...
Обновите раздел конфигурации **Подключения**. Например: 
```yaml
connect:
...
1. Добавьте созданный модуль машинного обучения.
    ...
    1. Щелкните **Добавить** и выберите **модуль машинного обучения Azure**.
