...
На этом шаге будет показано, как настроить оповещение, чтобы узнать об успешном развертывании обновлений. Вы создадите оповещение на основе запроса службы Log Analytics. Чтобы охватить множество сценариев, можно написать пользовательский запрос для дополнительных оповещений. На портале Azure перейдите к разделу **Монитор** и затем выберите **Создать оповещение**. 

В окне **Создать правило** в разделе **1. Определение условий оповещения** выберите **Выбор цели**. В разделе **Filter by resource type** (Фильтрация по типу ресурсов) выберите **Log Analytics**. Выберите рабочую область Log Analytics, а затем выберите **Готово**.
...
В разделе **2. Определение сведений об оповещении** введите имя и описание для оповещения. Задайте в разделе **Серьезность** значение **Информационный (уровень серьезности 2)**, так как это оповещение об успешном выполнении.
...
* **Обновляемые компьютеры**. Щелкните сохраненный поиск, импортировать группу или выберите компьютеры в раскрывающемся списке и выберите нужные компьютеры. Если вы выберете **Компьютеры**, состояние готовности нужного компьютера отобразится в столбце **Готовность к обновлению агента**. См. дополнительные сведения о [разных способах создания группы компьютеров в Log Analytics](../log-analytics/log-analytics-computer-groups.md).
...
* **Параметры перезагрузки**. Определяет, как следует выполнять перезагрузку. Доступные параметры:
  * Перезагрузка при необходимости (по умолчанию)
  * Всегда выполнять перезагрузку
  * Никогда не выполнять перезагрузку
  * Только перезагрузка — обновления не будут установлены
