...
Контекст подписки может быть потерян при вызове дочерних модулей Runbook в качестве отдельных заданий. Чтобы дочерний модуль Runbook вызывал командлеты Azure RM в требуемой подписке Azure, он должен выполнить аутентификацию для этой подписки независимо от родительского модуля Runbook.

Если задания в одной учетной записи службы автоматизации работают с несколькими подписками, выбор подписки в одном задании может изменить текущий выбранный контекст подписки для других заданий, что обычно не нужно. Чтобы избежать этой проблемы, сохраните результат вызова командлета `Select-AzureRmSubscription` и передайте этот объект в параметр `DefaultProfile` для всех последующих вызовов командлетов Azure RM. Этот шаблон должен последовательно применяться для всех модулей Runbook, выполняемых в этой учетной записи службы автоматизации.
