...
Для долго выполняющихся задач рекомендуется использовать [гибридную рабочую роль Runbook](automation-hrw-run-runbooks.md#job-behavior). Гибридные рабочие роли Runbook не ограничены равномерным распределением и могут выполняться сколько угодно.
