...
В этой статье перечислены конфигурации и компоненты, поддерживаемые при репликации и восстановлении виртуальных машин Azure из одного региона в другой с использованием службы [Azure Site Recovery](site-recovery-overview.md).
...
**Тип перемещения ресурсов** | **Дополнительные сведения**
--- | --- | ---
**Перемещение хранилища между группами ресурсов** | Не поддерживается<br/><br/> Перемещение хранилища служб восстановления между группами ресурсов невозможно.
...
**Перемещение виртуальных машин между подписками** | Не поддерживается.
...
- SUSE Linux Enterprise Server 12 SP1,SP2,SP3 [ (поддерживаемые версии ядра)](#supported-suse-linux-enterprise-server-12-kernel-versions-for-azure-virtual-machines)
...
**Выпуск** | **Версия службы Mobility Service** | **Версия ядра** |
--- | --- | --- |
Debian 7 | 9.17, 9.18 | С 3.2.0-4-amd64 по 3.2.0-6-amd64, 3.16.0-0.bpo.4-amd64 |
Debian 7 | 9.15, 9.16 | С 3.2.0-4-amd64 по 3.2.0-5-amd64, 3.16.0-0.bpo.4-amd64 |
|||
Debian 8; | 9.17, 9.18 | С 3.16.0-4-amd64 по 3.16.0-6-amd64, с 4.9.0-0.bpo.4-amd64 по 4.9.0-0.bpo.6-amd64 |
Debian 8; | 9.15, 9.16 | С 3.16.0-4-amd64 по 3.16.0-5-amd64, с 4.9.0-0.bpo.4-amd64 по 4.9.0-0.bpo.5-amd64 |
...
>[!NOTE]
>
> Для региона "Южная Бразилия" выполнять репликацию, отработку отказа и восстановление размещения можно только в следующие регионы: центрально-южная часть США, центрально-западная часть США, восточная часть США, восточная часть США 2, западная часть США, западная часть США 2 и центрально-северная часть США.

## <a name="support-for-vmdisk-management"></a>Поддержка управления виртуальной машиной и диском

**Действие** | **Дополнительные сведения**
-- | ---
Изменение размера диска на реплицируемой виртуальной машине | Поддерживаются
Добавление диска к реплицируемой виртуальной машине | Не поддерживается. Вы должны отключить репликацию для виртуальной машины, добавить диск, а затем снова включить репликацию.
...
Управляемые диски уровня "Стандартный" | Поддерживается в регионах Azure, в которых поддерживается Azure Site Recovery. |  
Управляемые диски уровня "Премиум" | Поддерживается в регионах Azure, в которых поддерживается Azure Site Recovery. |
...
Конечные точки службы виртуальной сети | Поддерживаются | Брандмауэры службы хранилища Azure для виртуальных сетей не поддерживаются. Разрешение доступа к определенным виртуальным сетям Azure для учетных записей хранения кэша, используемых для хранения реплицируемых данных, не поддерживается.
Ускорение работы в сети | Не поддерживается | Виртуальную машине с включенным ускорением работы в сети можно реплицировать, но на виртуальной машине после отработки отказа ускорение работы в сети будет отключено. Ускорение работы в сети также будет отключено на исходной виртуальной машине после восстановления размещения.
