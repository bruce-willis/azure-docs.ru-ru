...
2. Установщик определит текущую версию на компьютере. Чтобы начать обновление, нажмите кнопку **Да**. 
3. После завершения обновления будет выполнена проверка сервера конфигурации.

    ![Блокировка изменений](./media/vmware-azure-manage-configuration-server/update3.png)

4. Чтобы закрыть установщик, нажмите кнопку **Готово**.
