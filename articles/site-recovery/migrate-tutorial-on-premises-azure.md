...
> [!WARNING]
> Вы можете перенести виртуальные машины на другие платформы виртуализации (кроме VMware и Hyper-V, — например, XenServer), используя виртуальные машин в качестве физических серверов. Но этот подход еще не тестировался и не проверялся корпорацией Майкрософт. Он может не работать. Например, виртуальные машины, запущенные на платформе XenServer, могут не работать в Azure, если перед переносом на виртуальной машине будут удалены сетевые драйверы, паравиртуализованное хранилище и средства XenServer.
...
В рамках работы с этим руководством вы перенесли локальные виртуальные машины в виртуальные машины Azure. После переноса виртуальных машин можно сделать следующее:
- [Настроить аварийное восстановление](azure-to-azure-replicate-after-migration.md) для перенесенных виртуальных машин.
- Воспользоваться преимуществами [безопасного и хорошо управляемого облака](https://azure.microsoft.com/services/virtual-machines/secure-well-managed-iaas/) Azure для управления виртуальными машинами в Azure.
