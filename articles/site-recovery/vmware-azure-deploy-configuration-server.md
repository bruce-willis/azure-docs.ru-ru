...
1. Можно ли использовать виртуальную машину, на которую установлен сервер конфигурации, для других целей? **Нет**, сервер конфигурации должен быть выделенным сервером, и его использование в качестве общего сервера не поддерживается.
2. Можно ли сменить хранилище, уже зарегистрированное на сервере конфигурации, на новое хранилище? **Нет**, после регистрации хранилища на сервере конфигурации его невозможно изменить.
3. Можно ли использовать один и тот же сервер конфигурации для защиты физических компьютеров и виртуальных машин? **Да**, один сервер конфигурации может использоваться для репликации физических компьютеров и виртуальных машин. Однако восстановление размещения на физический компьютер не поддерживается.
4. Где будет использоваться сервер конфигурации? Ознакомьтесь с архитектурой Azure Site Recovery [здесь](vmware-azure-architecture.md), чтобы узнать больше о сервере конфигурации и его функциональных возможностях.
5. Где можно найти последнюю версию сервера конфигурации? Ее можно скачать из [Центра загрузки Майкрософт](https://aka.ms/asrconfigurationserver). Обратитесь к [этой статье](vmware-azure-manage-configuration-server.md#upgrade-the-configuration-server) за инструкциями по обновлению сервера конфигурации.
