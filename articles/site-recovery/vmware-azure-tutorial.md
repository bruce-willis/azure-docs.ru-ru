...
> [!TIP]
> В этом руководстве для создания сервера конфигурации виртуальной машины VMware используется шаблон OVF. Если вы не может выполнить это, попробуйте [настроить сервер конфигурации вручную](physical-manage-configuration-server.md). 

> [!TIP]
> В этом руководстве предусмотрено, что Site Recovery скачает и установит MySQL на сервере конфигурации. Если вы не хотите, чтобы это делала служба Site Recovery, выполните настройку вручную. [Узнайте больше](vmware-azure-deploy-configuration-server.md#prepare-for-mysql-installation).
...
4. Скачайте шаблон OVF для сервера конфигурации. К этому шаблону прилагается оценочная лицензия, который действует в течение 100 дней. После этого периода вам нужно будет приобрести собственную лицензию.
...
1. Выберите **Finalize configuration** (Завершение конфигурации) для завершения регистрации. 
2. По окончании регистрации на портале Azure убедитесь, что сервер конфигурации и VMware указаны на странице **Источник** в хранилище. Затем нажмите кнопку **OK** (ОК), чтобы настроить параметры целевого объекта.
...
1. Site Recovery проверяет наличие одной или нескольких совместимых учетных записей хранения и сетей Azure. Вы должны были их создать при настройке компонентов Azure, описанной в [первом руководстве](tutorial-prepare-azure.md) из этой серии.
...
2. В разделе **Виртуальные машины** > **Выбор виртуальных машин**, выберите каждую машину, которую нужно реплицировать. Можно выбрать только те компьютеры, для которых можно включить репликацию. Нажмите кнопку **ОК**.
3. В разделе **Свойства** > **Configure properties** (Настройка свойств) выберите учетную запись, которая будет использоваться сервером обработки для автоматической установки службы Mobility Service на компьютере.
4. Выберите **Параметры репликации** > **Настройка параметров репликации** и убедитесь, что выбрана правильная политика репликации.
5. Выберите **Включить репликацию**. Site Recovery установит службу Mobility Service, если репликация включена для виртуальной машины.
6. Ход выполнения задания **включения защиты** можно отслеживать, выбрав **Параметры** > **Задания** > **Задания Site Recovery**. После выполнения задания **Завершить подготовку защиты** виртуальная машина будет готова к отработке отказа.
