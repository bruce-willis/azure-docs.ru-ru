...
- Это четвертое руководство в серии, в которой показано, как настроить аварийное восстановление для виртуальных машин VMware в Azure. Предполагается, что вы ознакомились с первыми двумя руководствами:
...
В этой статье описано, как выполнять аварийное восстановление локальных компьютеров в Azure путем отработки тестового отказа. Выполнение проверяет стратегию репликации без потери данных. Вы узнаете, как выполнять следующие задачи:
...
В этом руководстве рассматривается настройка аварийного восстановления VMware для Azure с использованием наиболее простых параметров. Больше узнать об отработке тестового отказа можно в этом [разделе](site-recovery-test-failover-to-azure.md).

## <a name="verify-vm-properties"></a>Проверка свойств виртуальной машины

Прежде чем запускать отработку тестового отказа, проверьте свойства виртуальной машины VMware и убедитесь, что виртуальная машина Hyper-V[hyper-v-azure-support-matrix.md#replicated-vms], [виртуальная машина VMware или физический сервер](vmware-physical-azure-support-matrix.md#replicated-machines) соответствуют требованиям Azure.
