...
2. На вкладке **Параметры приложения** найдите параметр `FUNCTIONS_EXTENSION_VERSION` и укажите значение допустимой версии среды выполнения (для версии 1.x) или значение `~2` (для версии 2.0). Тильда с основным номером версии означает использование последней версии этого основного номера версии (например, "~1"). 
...
```
В этом коде следует заменить `<function_app>` именем приложения-функции. Также замените `<my_resource_group>` именем группы ресурсов для приложения-функции. Замените `<version>` значением допустимой версии среды выполнения (для версии 1.x) или значением `~2` (для версии 2). 
