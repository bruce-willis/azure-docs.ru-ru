...
ms.locfileid: "39344136"
---
# <a name="partition-and-scale-in-azure-cosmos-db"></a>Секционирование и масштабирование в Azure Cosmos DB

[Azure Cosmos DB](https://azure.microsoft.com/services/cosmos-db/) — это глобально распределенная многомодельная служба баз данных, предназначенная для обеспечения высокой и прогнозируемой производительности. Она легко масштабируется вместе с вашим приложением. В этой статье представлены сведения о работе секционирования для всех моделей данных в Azure Cosmos DB. В ней также описывается, как настраивать контейнеры Azure Cosmos DB для эффективного масштабирования приложений.
...
Если контейнер соответствует [требованиям секционирования](#prerequisites), секционирование будет полностью прозрачно для вашего приложения. Azure Cosmos DB распределяет данные по физическим и логическим секциям, а также направляет запросы в нужную секцию. 

## <a name="how-does-partitioning-work"></a>Как работает секционирование

Каждый документ должен иметь *ключ секции* и *ключ строки*, которые однозначно идентифицируют его. Ключ секции выступает в качестве логической секции для ваших данных и предоставляет Azure Cosmos DB естественную границу для распределения данных между физическими секциями. **Данные для одной логической секции должны находиться внутри одной физической секции. Управление физической секцией осуществляется с помощью Azure Cosmos DB.** 
...
Azure Cosmos DB использует секционирование на основе хэша. При написании элемента Azure Cosmos DB хэширует значение ключа секции и использует хэшированный результат, чтобы определить, в какой секции сохранить элемент. 

> [!NOTE]
> Azure Cosmos DB сохраняет все элементы с помощью одного ключа в одной физической секции. 
...
* Выбор ключа раздела с более высокой кратностью обычно предпочтительнее, потому что обычно он обеспечивает лучшее распределение и масштабируемость. Например, чтобы увеличить кратность, искусственный ключ можно создать путем объединения значений из нескольких свойств.  
...
Если вы создали контейнер **фиксированного** размера без ключа секции или с пропускной способностью менее 1000 ЕЗ в секунду, он не будет автоматически масштабироваться. Чтобы перенести данные из контейнера фиксированного размера в контейнер неограниченного размера, вам нужно использовать [средство миграции данных](import-data.md) или [библиотеку канала изменений](change-feed.md). 
...
## <a name="create-partition-key"></a>Создание ключа секции 
...
## <a name="form-partition-key-by-concatenating-multiple-fields"></a>Создание ключа секции путем объединения нескольких полей

Ключ секции можно также создать, заполнив и объединив значения нескольких свойств в одно искусственное свойство элемента partitionKey. Такие ключи называют искусственными.

Предположим, что у вас есть такой документ:
...
Один из вариантов — задать partitionKey для /deviceId или /date. Если нужно создать ключ секции на основе идентификатора устройства и даты, объедините эти два значения в искусственное свойство partitionKey и укажите для ключа секции значение /partitionKey.
...
В реальных сценариях могут быть тысячи документов, поэтому на стороне клиента следует определить логику для объединения значений в искусственные ключи, вставить искусственный ключ в документы и использовать его для указания ключа секции.
