...
Дополнительные сведения см. на странице [цен на Azure Cosmos DB](https://azure.microsoft.com/pricing/details/cosmos-db/). Плата за использование Azure Cosmos DB определяется числом подготовленных контейнеров, временем, в течение которого они были в сети, а также подготовленной пропускной способностью для каждого контейнера. *Контейнеры* в этой статье относятся к коллекциям API SQL, графам API Gremlin (Graph), коллекциям API MongoDB, а также таблицам API таблиц. 
...
Если вы ранее не работали с Azure, можете зарегистрироваться, чтобы получить [бесплатную учетную запись Azure](https://azure.microsoft.com/free/) на 30 дней, а также деньги на счет, которые можно потратить на любые службы Azure. Если у вас есть подписка на Visual Studio, вам доступны [бесплатные кредиты Azure](https://azure.microsoft.com/pricing/member-offers/msdn-benefits-details/), которые также можно потратить на любые службы Azure. 
...
* Один контейнер для каждой подписки для учетных записей SQL, API Gremlin и службы таблиц.
...
Служба Azure Cosmos DB доступна на портале Azure. Во-первых, вам нужно зарегистрироваться для получения подписки Azure. После этого вы сможете добавить учетную запись API SQL, API Gremlin, API таблиц, API MongoDB или API Cassandra в свою подписку Azure.
...
### <a name="how-do-i-create-fixed-collection-with-partition-key"></a>Создание фиксированной коллекции с ключом секции

Сейчас можно создать коллекцию с пропускной способностью ключа секции с помощью метода [CreatePartitionedCollection](https://github.com/Azure/azure-documentdb-dotnet/blob/master/samples/code-samples/CollectionManagement/Program.cs#L118) пакета SDK для .Net или [Azure CLI](https://docs.microsoft.com/cli/azure/cosmosdb/collection?view=azure-cli-latest#az-cosmosdb-collection-create). Создание фиксированной коллекции с помощью портала Azure сейчас не поддерживается.  
...
* Для учетных записей API SQL и MongoDB контейнер сопоставляется с коллекцией. 
...
### <a name="i-have-setup-my-container-to-use-lazy-indexing-i-see-that-my-queries-do-not-return-expected-results"></a>Для моего контейнера настроено использование отложенного индексирования, и я вижу, что запросы не возвращают ожидаемых результатов. 
Как описано в соответствующем разделе, отложенное индексирование может вызвать такое поведение. Всегда следует использовать согласованное индексирование для всех приложений. 
...
### <a name="is-the-simba-driver-for-mongodb-supported-for-use-with-azure-cosmosdb-mongodb-api"></a>Можно ли использовать драйвер Simba для MongoDB с API MongoDB для Azure Cosmos DB?
Да, вы можете использовать драйвер ODBC для Mongo от Simba с API MongoDB для Azure Cosmos DB.
...
* Идентификатор имени свойства сущности сейчас не поддерживается
...
В качестве основных соединителей для службы API Gremlin для Azure Cosmos DB использует драйверы с открытым кодом. Следовательно, рекомендуется использовать [драйверы, поддерживаемые Apache Tinkerpop](http://tinkerpop.apache.org/).
...
Плата за ЕЗ формируется на основе обрабатываемого, но не результирующего набора данных. Например, если нужно получить одну вершину, но при этом будут затронуты другие объекты, оплата будет учитывать все объекты графа, используемые при вычислении этой вершины.
...
Azure Cosmos DB выполняет [горизонтальное секционирование](partition-data.md), чтобы автоматически увеличивать объем хранилища и пропускную способность в соответствии с требованиями. Максимальная пропускная способность и объем хранилища рабочей нагрузки определяется количеством разделов, связанных с определенной коллекцией. Но вы можете воспользоваться рекомендациями по работе с коллекцией API Gremlin, чтобы обеспечить оптимальную производительность. См. [дополнительные сведения и рекомендации по секционированию](partition-data.md#best-practices-when-choosing-a-partition-key). 
...
Собственные драйверы Tinkerpop Gremlin позволяют использовать словарь параметров для выполнения запросов. См. пример с использованием [Gremlin.Net]((http://tinkerpop.apache.org/docs/3.2.7/reference/#gremlin-DotNet)) и [Gremlin-Javascript](https://github.com/Azure-Samples/azure-cosmos-db-graph-nodejs-getting-started/blob/master/app.js).
...
API Gremlin для Azure Cosmos DB реализует ряд функций, определенных в Gremlin. Дополнительные сведения см. в статье о [поддержке Gremlin](gremlin-support.md).
...
В этих выходных данных показано, сколько времени понадобилось на получение объектов (вершин и ребер), а также указан размер рабочего набора данных. На основе этих значений и формируется стоимость запросов Azure Cosmos DB.
