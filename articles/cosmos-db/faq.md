...
### <a name="i-have-setup-my-collection-to-use-lazy-indexing-i-see-that-my-queries-do-not-return-expected-results"></a>Для моих коллекций настроено использование отложенного индексирования, и я вижу, что запросы не возвращают ожидаемых результатов. 
...
Создание разрешения с помощью ResourceTokens разрешено на уровне коллекции и ее потомков (например, документов и вложений). Это означает, что возможность создать разрешение в базе данных или на уровне учетной записи сейчас не поддерживается.
