...
Многие клиенты, с которыми мы общались, говорили, что неплохо было бы, если бы в эмуляторе можно было создавать сценарии. Поэтому в этот выпуск мы добавили некоторые возможности написания скриптов. Теперь эмулятор включает в себя модуль PowerShell для запуска, остановки, удаления этого эмулятора и получения его состояния: `Microsoft.Azure.CosmosDB.Emulator`. 
