...
>[!NOTE]
> В новых конструкциях советуем использовать Load Balancer ценовой категории "Стандартный". 
...
| [Реакция на отключение проверки работоспособности](load-balancer-custom-probe-overview.md) | Подключения по протоколу TCP остаются активными при отключении проверки экземпляра __и__ отключении всех проверок. | Подключения по протоколу TCP остаются активными при отключении проверки экземпляра. Все подключения по протоколу TCP завершаются при отключении всех проверок. |
