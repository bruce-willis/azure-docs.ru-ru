...
| [Реакция на сбой проверки работоспособности](load-balancer-custom-probe-overview.md) | TCP-подключения остаются активными при сбое проверки экземпляра __и__ при сбое всех проверок. | TCP-подключения остаются активными при сбое проверки экземпляра. Все подключения TCP завершаются при сбое всех проверок. |
