...
ms.locfileid: "39186450"
---
# <a name="understand-load-balancer-probes"></a>Основные сведения о проверках подсистемы балансировки нагрузки

Azure Load Balancer использует пробы работоспособности, чтобы определить, какой экземпляр внутреннего пула должен получить новые потоки.   Пробы работоспособности можно использовать для обнаружения сбоя приложения в экземпляре серверной части.  С помощью ответа пробы работоспособности из приложения можно также указать Load Balancer, следует ли продолжать отправлять новые потоки или остановить их отправку в экземпляр серверной части, чтобы управлять нагрузкой или плановым простоем.

Пробы работоспособности управляют установкой новых потоков, передаваемых в работоспособные экземпляры серверной части. Если проба работоспособности выдает сбой, Load Balancer прекращает отправлять новые потоки в соответствующий неработоспособный экземпляр.  После сбоя пробы работоспособности установленные TCP-подключения продолжают работать.  Существующие потоки UDP будут перемещены из неработоспособного экземпляра в работоспособный экземпляр во внутреннем пуле.

Если все пробы для внутреннего пула завершатся сбоем, подсистемы Load Balancer (цен. категория "Базовый") завершат все имеющиеся потоки TCP, передаваемые во внутренний пул, тогда как подсистемы Load Balancer (цен. категория "Стандартный") не завершат имеющиеся потоки TCP, но не будут отправлять новые потоки во внутренний пул.  Если все пробы для внутреннего пула завершатся сбоем, будут прекращены все существующие потоки UDP для подсистем Load Balancer (цен. категория "Базовый") и Load Balancer (цен. категория "Стандартный").  UDP не требует соединения, поэтому состояние этого потока не отслеживается.  Пока в процессе хэширования возвращаются те же результаты, поток датаграмм остается в определенном экземпляре.  Если изменить пробу работоспособности во внутреннем пуле, в другой экземпляр в этом пуле могут переместиться новые датаграммы.

Роли облачной службы (рабочие роли и веб-роли) используют гостевой агент для мониторинга проб. При использовании облачных служб с виртуальными машинами IaaS, защищенными с помощью Load Balancer, необходимо настроить пользовательские пробы работоспособности TCP или HTTP.
