...
Чтобы подключить DevKit к созданному экземпляру службы подготовки устройств, выполните следующие действия.
...
Теперь нужно создать сертификат X.609. 

### <a name="windows"></a>Windows

1. Откройте проводник и перейдите в папку, которая содержит пример кода Службы подготовки устройств, клонированный ранее. В папке **.build** найдите и скопируйте файлы **DPS.ino.bin** и **DPS.ino.map**.
...
3. На панели "Add enrollment" (Добавление регистрации):
   ...
   - Щелкните "Select a file" (Выберите файл) в разделе **Primary Certificate .pem or .cer file** (Файл PEM или CER основного сертификата).
  ...
  > [!NOTE]
  > Если возникает ошибка со следующим сообщением:
  >
  ...
  >
  > Откройте файл сертификата **PEM** в виде текста (с помощью Блокнота или другого текстового редактора) и удалите следующие строки:
  >
