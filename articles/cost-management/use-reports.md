...
ms.locfileid: "37906380"
---
# <a name="use-cost-management-reports"></a>Использование отчетов службы "Управление затратами"

В этой статье описано, для чего предназначены отчеты службы "Управление затратами", которые предоставляются на портале Cloudyn. Также вы здесь узнаете, как их можно эффективно использовать. Большинство отчетов удобны для использования и имеют стандартный внешний вид. В каждом из отчетов вы можете выполнять одни и те же основные действия. Общие сведения о том, как использовать отчеты службы "Управление затратами", в том числе о настройке, сохранении и планировании отчетов, см. в статье [Интерпретация отчетов о затратах](understanding-cost-reports.md).

## <a name="report-types"></a>Типы отчетов

Существует три вида отчетов службы "Управления затратами".
...
Отчеты по _распределению затрат_ становятся доступны после создания модели затрат с помощью [комплексного распределения затрат](tutorial-manage-costs.md#use-custom-tags-to-allocate-costs). Процессы в службе управления затратами обрабатывают данные о затратах и выставлении счетов, а также _сопоставляют_ их с данными о потреблении и тегами для облачных учетных записей. Для такого сопоставления службе "Управление затратами" нужен доступ к данным об использовании. Если в данных встречаются учетные записи, для которых отсутствуют учетные данные, они помечаются как _ресурсы без категории_.
...
Многие отчеты об управлении затратами отображают сведения о бюджете, если вы вручную создали его. Это означает, что при отсутствии бюджета сведения о бюджете также отсутствуют. См. дополнительные сведения о [параметрах бюджета](#budget-settings).
...
В файле выставления счетов единовременными платежами считаются такие, у которых даты (метки времени) начала и завершения использования службы имеют одинаковые значения. Служба "Управление затратами" применяет амортизацию к таким единовременным платежам. Другие службы с оплатой по мере использования и потреблением по запросу не амортизируются.
...
Как служба "Управление затратами" рассчитывает этот отчет?

Служба "Управление затратами" гарантирует, что выделение сохраняет целостность каждой связанной учетной записи, применяя _сопоставление учетных записей_. Сопоставление гарантирует, что с учетной записью, которая не использует определенную службу, не будут связаны какие-либо затраты за эту выделенную службу. Расходы, вычисленные в этой учетной записи, остаются в этой учетной записи и не рассчитываются политиками распределения. Например, у вас может быть 5 связанных учетных записей. Если только три из них используют службы хранения, то стоимость услуг хранения распределяется только по тэгам в трех учетных записях.
...
Дополнительные сведения о создании пользовательских моделей затрат см. в [руководстве по управлению затратами с помощью службы "Управление затратами Azure"](tutorial-manage-costs.md).
...
Отчет обозревателя ресурсов Azure отображает общий список всех ресурсов Azure, доступных в службе "Управление затратами". Чтобы эффективно использовать этот отчет, следует включить расширенные метрики в учетных записях Azure. Расширенные метрики предоставляют службе "Управление затратами" доступ к виртуальным машинам Azure. См. дополнительные сведения о [добавлении расширенных метрик для виртуальных машин Azure](azure-vm-extended-metrics.md).

### <a name="azure-resources-over-time-report"></a>Отчет по ресурсам Azure за период времени

В отчете по ресурсам Azure за период времени отображаются подробные сведения обо всех ресурсах, активных в определенный период времени. Чтобы эффективно использовать этот отчет, следует включить расширенные метрики в учетных записях Azure. Расширенные метрики предоставляют службе "Управление затратами" доступ к виртуальным машинам Azure. См. дополнительные сведения о [добавлении расширенных метрик для виртуальных машин Azure](azure-vm-extended-metrics.md).
...
Служба "Управление затратами" оценивает время работы каждого компьютера за последние 30 дней. Служба "Управление затратами" предоставляет рекомендации по покупке зарезервированных экземпляров, если такой вариант окажется экономичнее при текущем уровне активности.
...
- **Анализ экономии**. Этот раздел содержит возможные суммы экономии и срок достижения окупаемости, если будет выполнена рекомендация службы "Управление затратами". Экономия в денежном и процентном выражении выделяются красным цветом.

- **Сравнение типов зарезервированных экземпляров EC2**. В этом разделе содержатся основные показатели рентабельности по предложенным рекомендациям, в том числе для разных применимых вариантов. В этом сегменте отчета предполагается, что виртуальная машина будет работать постоянно (100 % времени). Щелкните **Увеличить**, чтобы открыть подробный отчет.
...
- **Анализ экономии**. Этот раздел содержит возможные суммы экономии и срок достижения окупаемости, если будет выполнена рекомендация службы "Управление затратами". Экономия в денежном и процентном выражении выделяется красным цветом.
...
У вас есть шесть запущенных экземпляров m3.xlarge. Анализ службы "Управление затратами" показывает, что пять из них имеют низкую загрузку ЦП. Для них можно уменьшить размер.
...
- Раздел **Потенциальная ежегодная экономия** отображает потенциальное снижение стоимости при уменьшении размера в соответствии с рекомендацией.
...
Отчет по рекомендациям по изменению размера RDS содержит предложения по изменению размера, позволяющие оптимизировать использование облачных служб. Он содержит список слабо нагруженных экземпляров, для которых есть смысл уменьшить размер. Рекомендации службы "Управление затратами" основываются на данных о потреблении и производительности за последние 30 дней. Вы можете отфильтровать рекомендации по имени учетной записи, региону, типу экземпляра и (или) состоянию.

### <a name="sizing-threshold-manager-report"></a>Отчет диспетчера пороговых значений для изменения размера

Встроенные рекомендации службы "Управление затратами" вычисляются по сложному алгоритму, который позволяет получить точные рекомендации по изменению размера. Вы можете изменить настроенные пороговые значения для рекомендаций по уменьшению размера.
