...
|Аргумент|ОПИСАНИЕ|
| --- | --- |
| --cluster-config            [обязательный параметр] | Конфигурация кластера, которая будет применяться к нему. |
...
Запуск обновления конфигурации кластера: sfctl sa-cluster config-upgrade --cluster-config <YOUR CLUSTER CONFIG> --application-health-policies "{"fabric:/System":{"ConsiderWarningAsError":true}}"
