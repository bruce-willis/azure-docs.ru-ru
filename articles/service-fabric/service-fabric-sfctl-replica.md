...
|Аргумент|ОПИСАНИЕ|
| --- | --- |
| --application-id [обязательный параметр] | Идентификатор приложения. Обычно это полное имя приложения без указания схемы универсального кода ресурса (URI) 'fabric\:'. Начиная с версии 6.0, иерархические имена разделяются символом "\~". Например, если приложение имеет имя "fabric\:/myapp/app1", то в версии 6.0 и в более поздних версиях будет использоваться идентификатор приложения "myapp\~app1", а в предыдущих версиях — "myapp/app1". |
| --node-name [обязательный параметр] | Имя узла. |
