...
1. Чтобы начать отладку, разверните приложение, нажав в Visual Studio клавишу **F5**.  Нажмите кнопку **Да**, если появится окно сообщения с запросом на предоставление группе ServiceFabricAllowedUsers разрешений на чтение и выполнение в каталоге проекта Visual Studio.
    ...
    >[!NOTE]
    >Чтобы выполнить это упражнение, требуется 5 кластеров узлов (в отличии от 1 узла). Это можно проверить следующим образом. Запустите средство Service Fabric Explorer, щелкнув правой кнопкой мыши область уведомлений **диспетчера локального кластера Service Fabric**, а затем выберите **Switch Cluster Mode** (Переключить режим кластера). Если сейчас выбран 1 узел, щелкните **5 Node** (5 узлов).
...
7. Запустите средство Service Fabric Explorer, щелкнув правой кнопкой мыши область уведомлений **диспетчера локального кластера Service Fabric**, а затем выберите **Manage Local Cluster** (Управление локальным кластером).
...
8. В области слева разверните **Кластер** > **Узлы** и найдите узел, в котором выполняется код приложения. Выберите **Действия** > **Отключить (перезапустить)** для имитации перезапуска компьютера.
    ...
    Вы сразу же увидите срабатывание точки останова в Visual Studio. Это означает, что процесс вычислений с отключенного узла передан на другой узел как единое целое. Нажмите клавишу **F5**, чтобы продолжить.
