...
В. **Какое значение политики TaskApprovalPolicy, NodeWise или UpgradeDomainWise нужно выбрать для моего кластера?**
...
О. Установка исправлений на узел может занять от нескольких минут (например, для [обновления определений Защитника Windows](https://www.microsoft.com/wdsi/definitions)) до нескольких часов (например, для [накопительных обновлений Windows](https://www.catalog.update.microsoft.com/Search.aspx?q=windows%20server%20cumulative%20update)). Время, необходимое для исправления узла, главным образом, зависит от 
 - размера обновлений;
...
О. Время, необходимое для исправления всего кластера, зависит от перечисленных ниже факторов.

- Время, необходимое для исправления узла.
- Политика службы координатора. — Политика по умолчанию (`NodeWise`) ограничивает исправление только одним узлом за раз — это медленнее, чем `UpgradeDomainWise`. Например, если исправление узла занимает примерно 1 час, для применения исправлений к кластеру из 20 узлов (одного типа) с пятью доменами обновления, каждый из которых содержит 4 узла,
...
О. Сведения об обновлении некоторых продуктов отображаются только в соответствующем журнале обновлений или исправлений. Например, сведения об обновлении Защитника Windows могут не отображаться в журнале Центра обновлений Windows в Windows Server 2016.
