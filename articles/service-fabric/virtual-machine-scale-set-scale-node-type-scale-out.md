---
title: Горизонтальное масштабирование кластера Azure Service Fabric путем добавления масштабируемого набора виртуальных машин | Документация Майкрософт
...
ms.locfileid: "42887794"
---
# <a name="scale-a-service-fabric-cluster-out-by-adding-a-virtual-machine-scale-set"></a>Развертывание кластера Service Fabric путем добавления масштабируемого набора виртуальных машин
В этой статье описывается, как масштабировать кластер Azure Service Fabric путем добавления нового масштабируемого набора виртуальных машин в существующий кластер. Кластер Service Fabric — это подключенный к сети набор виртуальных машин или физических компьютеров, в котором вы развертываете микрослужбы и управляете ими. Компьютер или виртуальная машина, которая входит в состав кластера. Масштабируемые наборы виртуальных машин относятся к вычислительным ресурсам Azure. Их можно использовать для развертывания коллекции виртуальных машин и управления ею как набором. Каждый тип узла, определенный в кластере Azure, [настроен как отдельный масштабируемый набор](service-fabric-cluster-nodetypes.md). Затем каждым типом узла можно управлять отдельно. После создания кластера Service Fabric можно масштабировать тип узла кластера вертикально (изменение ресурсов узлов), обновить ОС виртуальных машин типа узла или добавить масштабируемый набор виртуальных машин в существующий кластер.  Кластер можно масштабировать в любое время, даже когда в нем выполняются рабочие нагрузки.  Вместе с кластером автоматически масштабируются ваши приложения.
...
## <a name="adding-an-additional-scale-set-to-an-existing-cluster"></a>Добавление дополнительного масштабируемого набора в существующий кластер
Добавление нового масштабируемого набора виртуальных машин NodeType в существующий кластер аналогично вышеупомянутому обновлению типа первичного узла, за исключением того, что вы не используете тот же NodeTypeRef. Очевидно, не будут отключаться какие-либо активно используемые масштабируемые наборы виртуальных машин, и вы не потеряете доступность кластера, если не обновите тип первичного узла. 
