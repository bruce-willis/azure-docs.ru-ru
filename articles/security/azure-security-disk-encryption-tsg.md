...
Ошибка может возникнуть при попытке шифрования диска ОС в целевой среде виртуальной машины, чей готовый образ из коллекции был изменен. Примеры отклонений от поддерживаемого образа, которые могут мешать предоставляемой расширением возможности отключать диск ОС, включают:
