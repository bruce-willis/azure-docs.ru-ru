...
> * подготовить виртуальное устройство в гипервизоре;
...
* Выполнены все инструкции из статьи [Руководство: подготовка к развертыванию Шлюза Azure Data Box (предварительная версия)](data-box-gateway-deploy-prep.md).
...
- Изучите сетевые требования к развертыванию Шлюза Data Box и настройте сеть центра обработки данных в соответствии с требованиями. Дополнительные сведения см. в [этом разделе](data-box-gateway-system-requirements.md#networking-requirements).
...
## <a name="check-the-host-system"></a>Проверка главной системы
...
* Доступ к главной системе под управлением VMware ESXi Server 6.0 или 6.5. Главная система должна быть в состоянии выделить указанный ниже объем ресурсов для виртуального устройства:
  ...
  * Диск ОС на 250 ГБ.
  * Виртуальный диск данных размером 2 ГБ.
* Наличие в системе клиента VMware vSphere для управления узлом ESXi.


## <a name="provision-a-virtual-device-in-hypervisor"></a>Подготовка виртуального устройства в гипервизоре
...
7. Чтобы настроить сеть в командной строке, используйте команду `Get-HcsIpAddress` для получения списка сетевых интерфейсов, включенных на виртуальном устройстве. Если на устройстве включен один сетевой интерфейс, по умолчанию ему назначается имя `DATA1`.
...
> [!div class="checklist"]
> * Соответствие минимальным требованиям узла к устройству.
> * Подготовка виртуального устройства в гипервизоре.
