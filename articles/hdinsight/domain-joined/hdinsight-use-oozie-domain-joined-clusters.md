---
title: Рабочие процессы Apache Hadoop Oozie в присоединенных к домену кластерах Azure HDInsight
...
- Присоединенный к домену кластер Azure HDInsight Hadoop. Ознакомьтесь со статьей [Настройка присоединенных к домену кластеров HDInsight с помощью доменных служб Azure Active Directory](./apache-domain-joined-configure-using-azure-adds.md).

    > [!NOTE]
    > Подробные инструкции по использованию Oozie в неприсоединенных к домену кластерах см. в статье [Использование Oozie с Hadoop для определения и запуска рабочих процессов в Azure HDInsight под управлением Linux](../hdinsight-use-oozie-linux-mac.md).

## <a name="connect-to-a-domain-joined-cluster"></a>Подключение к присоединенному к домену кластеру
...
Веб-интерфейс Oozie позволяет получить информацию о состоянии задания Oozie в кластере. Чтобы получить веб-интерфейс, в присоединенных к домену кластерах вам потребуется:
