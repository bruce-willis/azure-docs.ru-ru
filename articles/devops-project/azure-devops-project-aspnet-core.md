---
title: Создание конвейера CI/CD для .NET с помощью проекта Azure DevOps | Быстрое начало
description: Проект DevOps упрощает начало работы с Azure. Это поможет вам запустить приложение .NET в службе Azure по вашему выбору за несколько быстрых шагов.
ms.prod: devops
...
ms.locfileid: "37967101"
---
# <a name="create-a-cicd-pipeline-for-net-with-the-azure-devops-project"></a>Создание конвейера CI/CD для .NET с помощью проекта Azure DevOps

Настройте непрерывную интеграцию (CI) и непрерывную доставку (CD) для вашего приложения .NET Core или ASP.NET с помощью проекта **Azure DevOps**.  Проект Azure DevOps упрощает первоначальную конфигурацию конвейера сборки и выпуска VSTS.
...
1. Выберите платформу приложения **.NET Core**.  Этот пример представляет собой приложение ASP.NET Core MVC. Когда все будет готово, щелкните **Далее**.

1. **Веб-приложение в Windows** является целью развертывания по умолчанию.  При необходимости можно выбрать веб-приложение в Linux или веб-приложение для контейнеров.  Исполняющая среда, которую вы выбрали на предыдущих шагах, определяет тип цели развертывания службы Azure, доступной здесь.  Оставьте службу по умолчанию и нажмите кнопку **Далее**.

## <a name="configure-vsts-and-an-azure-subscription"></a>Настройка VSTS и подписки Azure 

1. **Создайте** бесплатную учетную запись VSTS или выберите **имеющуюся**.  Выберите **имя** для проекта VSTS.  Выберите **подписку Azure**, **расположение** и укажите **имя** приложения.  Когда все будет готово, щелкните **Done** (Готово).

    ![Предоставление сведений о VSTS](_img/azure-devops-project-aspnet-core/vstsazureinfo.png)
...
1. В левой части браузера перейдите к файлу **Views/Home/index.cshtml**.

1. Щелкните **Изменить** и внесите изменения в заголовок h2.  Например, введите **Начало работы с проектом Azure DevOps** или внесите некоторые изменения.
...
1. Щелкните **многоточие**.  После этого откроется меню, в котором можно начать выполнение нескольких действий, таких как постановка новой сборки в очередь, приостановка создания и изменение определения сборки.

1. Выберите **Изменить**

    ![Определение сборки](_img/azure-devops-project-aspnet-core/builddef.png)
...
1. Выберите **Триггеры**.  Проект Azure DevOps автоматически создает триггер CI, а каждая фиксация в репозитории создает новую сборку.  При желании выберите включение или исключение ветвей из процесса непрерывной интеграции.
...
Чтобы узнать больше об изменении определений сборки и выпуска в соответствии с потребностями вашей команды, см. это руководство:
