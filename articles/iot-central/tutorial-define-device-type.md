...
4. Чтобы создать приложение Azure IoT Central, сделайте следующее:
    ...
    * Выберите понятное имя для приложения, например **Кондиционеры Contoso**. Azure IoT Central создаст уникальный префикс URL-адреса. Вы можете изменить этот префикс на что-то более запоминающееся.
    
    * Выберите Azure Active Directory и подписку Azure. Дополнительные сведения о каталогах и подписках см. в статье о [создании приложения Azure IoT Central](howto-create-application.md).
    
    * Либо используйте существующую группу ресурсов, либо создайте новую с именем по вашему выбору. Например, **contoso-rg**.
    
    * Выберите ближайший географический регион.
    
    * Выберите шаблон приложения **Пользовательское приложение**.
    
    * Выберите план оплаты **Free 30 Day Trial Application** (Бесплатная 30-дневная пробная версия приложения).
...
Параметры, свойства и свойства устройства — это разные значения, определенные в шаблоне устройства и связанные с каждым отдельным устройством:
...
## <a name="use-properties--device-properties"></a>Использование свойств и свойств устройства

Используйте *свойства* для хранения сведений о вашем устройстве в приложении. В этом разделе вы добавляете свойства устройства в шаблон **подключенного кондиционера**, в котором будет храниться серийный номер устройства и версия встроенного ПО. Обратите внимание, что оба эти значения являются свойствами, предназначенными только для чтения, сообщаемые устройством (вы не можете присваивать для них значения). Для свойств, которые вы можете использовать, можно присвоить значения, чтобы добавить расположение устройства, сведения о владельце и данные даты и времени последнего обслуживания устройства.
...
1. Перейдите на страницу **​Свойства** вашего шаблона устройства **подключенного кондиционера**:
    ...
    Вы можете создавать свойства устройства разных типов, например цифровые или текстовые. Чтобы добавить свойство серийного номера в шаблон устройства, щелкните **Текст**.

2. Для настройки свойства серийного номера используйте сведения в следующей таблице:
...
3. Чтобы добавить свойство версии встроенного ПО в свойства устройства, щелкните **Текст**.

4. Для настройки свойства устройства версии встроенного ПО используйте сведения в следующей таблице:
...
Используйте _команды_, чтобы позволить оператору выполнять команды непосредственно на устройстве. В этом разделе добавьте в шаблон устройства **подключенного кондиционера** команду, позволяющую оператору выводить на экран подключенного кондиционера определенные сообщения (эта функция доступна с примером кода MxChip).

1. Перейдите на страницу **Команды** шаблона устройства **подключенного кондиционера**:

    ![Подготовка к добавлению параметра](media/tutorial-define-device-type/commandsecho.png)

    Вы можете создать команды различных типов, в зависимости от ваших требований. 

1. Щелкните **New Command** (Новая команда), чтобы добавить команду на устройство.
...
1. Для настройки новой команды используйте сведения в следующей таблице:
    ...
    | Отображаемое имя         | Отображаемый текст    |
    | Display Type (Отображаемый тип)         | текст            |  

Вы можете добавить дополнительные входные данные для команды, щелкнув **+** для входных данных.

2. Нажмите **Сохранить**.

3. Вы можете настроить макет страницы **Команды**, перемещая и изменяя размеры плиток команд:

    ![Настройка макета параметров](media/tutorial-define-device-type/commandstileresize.png)
...
2. Щелкните **Линейная диаграмма**, чтобы добавить компонент на **панель мониторинга**:

    ![Компоненты панели мониторинга](./media/tutorial-define-device-type/dashboardcomponents1.png)

3. Настройте компонент **Линейная диаграмма**, используя информацию в следующей таблице:
...
4. Настройте компонент **Event Chart** (Диаграмма событий), используя информацию в следующей таблице:
...
5. Настройте компонент **State Chart** (Диаграмма состояний), используя информацию в следующей таблице:
...
6. Чтобы добавить заданный параметр температуры на панель мониторинга, выберите **Settings and Properties** (Параметры и свойства):

    ![Компоненты панели мониторинга](./media/tutorial-define-device-type/dashboardcomponents4.png)

7. Настройте компонент **Settings and Properties** (Параметры и свойства), используя сведения в следующей таблице:
...
8. Чтобы добавить на панель мониторинга серийный номер устройства, щелкните **Settings and Properties** (Параметры и свойства).

    ![Компоненты панели мониторинга](./media/tutorial-define-device-type/dashboardcomponents3.png)

9. Настройте компонент **Settings and Properties** (Параметры и свойства), используя сведения в следующей таблице:
...
10. Чтобы добавить на панель мониторинга версию встроенного ПО устройства, щелкните **Settings and Properties** (Параметры и свойства).

    ![Компоненты панели мониторинга](./media/tutorial-define-device-type/dashboardcomponents4.png)

11. Настройте компонент **Settings and Properties** (Параметры и свойства), используя сведения в следующей таблице:
...
12. Чтобы просмотреть панель мониторинга в качестве оператора, отключите **режим конструктора** в правом верхнем углу страницы.
