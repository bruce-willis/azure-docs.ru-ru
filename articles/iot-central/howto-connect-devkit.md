...
#### <a name="before-you-start-configuring-the-device"></a>Перед настройкой устройства сделайте следующее:
1. В **примере Devkits** IoT Central выберите `Device Explorer`-> `select MXChip Template` -> `Click on +New and choose **Real** Device` -> `Connect this device` (в правом верхнем углу). 
2. Скопируйте основную строку подключения.
3. Обязательно сохраните строку подключения, так как вы будете временно отключены от Интернета при подготовке устройства DevKit. 
...
1. При перезапуске устройства DevKit отображается следующий экран:
    ...
    > [!NOTE]
    > Если на экране отображается что-либо еще, нажмите одновременно кнопки **A** и **B** на устройстве, чтобы перезагрузить его. 
...
1. Откройте веб-браузер и перейдите по адресу [http://192.168.0.1/start](http://192.168.0.1/start). Откроется следующая веб-страница:
    ...
    - Укажите строку подключения устройства (вы должны были сохранить ее). Строку подключения можно найти здесь: `https://apps.iotcentral.com` -> `Device Explorer` -> `Device` -> `Select or Create a new Real Device` -> `Connect this device` (в правом верхнем углу).
