...
Являясь службой Azure, Аналитика временных рядов отличается высоким уровнем доступности, достигаемым за счет использования избыточности на уровне регионов Azure. При этом к решению не выдвигаются никакие дополнительные требования. Платформа Microsoft Azure также включает в себя ряд возможностей для создания решений с функциями аварийного восстановления или межрегиональной доступностью. Чтобы обеспечить глобальную высокую доступность между регионами для устройств или пользователей, используйте эти функции аварийного восстановления Azure. В [техническом руководстве по обеспечению устойчивости в Azure](../resiliency/resiliency-technical-guidance.md) описаны встроенные функции Azure, которые позволяют обеспечить непрерывность бизнес-процессов и возможности аварийного восстановления. Статья [Аварийное восстановление и высокая доступность для приложений Azure][Аварийное восстановление и высокая доступность для приложений Azure] содержит требования к архитектуре, которая позволяет обеспечить высокий уровень доступности приложений Azure и реализовать для них возможности аварийного восстановления.

Аналитика временных рядов не имеет встроенных функций непрерывности бизнес-процессов и аварийного восстановления (BCDR).  Однако клиенты, которым требуется BCDR, могут реализовать стратегию восстановления. Создайте вторую среду Аналитики временных рядов в резервном регионе Azure и отправляйте в эту вторую среду события из основного источника событий, используя вторую выделенную группу потребителей и рекомендации по BCDR для этого источника события.  

1.  Создайте среду во втором регионе.  Дополнительные сведения о создании среды Аналитики временных рядов см. [здесь](https://docs.microsoft.com/en-us/azure/time-series-insights/time-series-insights-get-started).
2.  Создайте вторую выделенную группу потребителей для источника событий и подключите его к ней.  Обязательно назначьте вторую выделенную группу потребителей.  Дополнительные сведения об этом см. в [документации по Центру Интернета вещей](https://docs.microsoft.com/en-us/azure/time-series-insights/time-series-insights-how-to-add-an-event-source-iothub) или [документации по концентратору событий](https://docs.microsoft.com/en-us/azure/time-series-insights/time-series-insights-data-access).
3.  Если во время инцидента ваш основной регион отключается, переключите операции на резервную среду Аналитики временных рядов.  

Дополнительные сведения о политиках BCDR для Центра Интернета вещей см. [здесь](https://docs.microsoft.com/en-us/azure/iot-hub/iot-hub-ha-dr).  Дополнительные сведения о политиках BCDR для концентратора событий см. [здесь](https://docs.microsoft.com/en-us/azure/event-hubs/event-hubs-geo-dr).  
