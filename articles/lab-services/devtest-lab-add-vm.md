...
1. Если вы хотите использовать пароль, хранящийся в [Azure Key Vault](devtest-lab-store-secrets-in-key-vault.md), выберите **Использовать сохраненный секрет** и укажите значение ключа, которое соответствует секрету (пароль). В противном случае — просто введите пароль в текстовое поле, в котором указано **Введите значение**. См. дополнительные сведения о [хранении секретов в Azure Key Vault и их использовании их при создании ресурсов лаборатории](devtest-lab-store-secrets-in-key-vault.md).
...
2. Щелкните **Размер виртуальной машины** и выберите один из стандартных пунктов, указывающих количество ядер процессора, объем ОЗУ и размер жесткого диска для создаваемой виртуальной машины.
3. Щелкните **Артефакты**, а затем выберите из списка и настройте артефакты, которые нужно добавить в базовый образ.
...
4. Чтобы настроить сетевые параметры виртуальной машины и срок ее действия, выберите **Дополнительные параметры**. 
