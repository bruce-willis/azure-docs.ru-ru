...
4. Используйте типы MIME по умолчанию либо измените список, удалив или добавив типы MIME.
   ...
   > [!NOTE]
   > Изменение стандартного списка по умолчанию типов MIME в настоящее время не поддерживается в Azure CDN уровня "Стандартный" от корпорации Майкрософт.
   > 
...
Для профилей **Azure CDN уровня "Стандартный" от Майкрософт** сжимаются только соответствующие файлы. Чтобы сжатие было применимо: — должен использоваться файл типа MIME, для которого [в настройках разрешено сжатие](#enabling-compression);
— размер файла должен быть больше 1 КБ и меньше 8 МБ.
