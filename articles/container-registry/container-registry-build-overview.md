...
**Сборка ACR** — это набор компонентов в Реестре контейнеров Azure. Она позволяет создавать образы облачных контейнеров для Linux, Windows и ARM, а также автоматизировать применение [исправлений платформы и ОС](#automate-os-and-framework-patching) для ваших контейнеров Docker.
...
Используя знакомый формат `docker build`, команда [az acr build][az-acr-build] в Azure CLI извлекает **контекст** (набор файлов для сборки), отправляет его в службу сборки ACR и по умолчанию отправляет встроенный образ в его реестр после завершения.

В следующей таблице приведено несколько примеров поддерживаемых расположений контекста для Сборки ACR:
...
Решение "Сборка ACR" также учитывает ваши геореплицированные реестры, благодаря чему географически распределенные группы разработчиков могут использовать ближайший реплицированный реестр.
...
> [!NOTE]
> В исходной предварительной версии обновления базовых образов активируют сборки, если базовые образы и образы приложения находятся в одном и том же реестре контейнеров Azure или общедоступных репозиториях Docker Hub.
