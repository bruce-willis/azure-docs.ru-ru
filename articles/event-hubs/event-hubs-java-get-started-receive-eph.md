...
Диспетчер контрольных точек по умолчанию использует хранилище BLOB-объектов, но если переопределить диспетчер контрольных точек, используемый EPH, с помощью вашей собственной реализации, вы сможете использовать любое хранилище для архивирования вашей реализации диспетчера контрольных точек.

Необходимо создать класс, реализующий интерфейс com.microsoft.azure.eventprocessorhost.ICheckpointManager.
...
В реализации можно переопределить механизм создания контрольных точек по умолчанию и реализовать свои собственные контрольные точки на основе хранилища данных (SQL Server, CosmosDB, Кэш Redis и т. д.). Мы рекомендуем предоставить доступ к хранилищу, используемому для архивации вашей реализации диспетчера контрольных точек, всем экземплярам EPH, обрабатывающим события для группы потребителей.

Вы можете использовать любое хранилище данных, которое будет доступно в вашей среде.

Класс com.microsoft.azure.eventprocessorhost.EventProcessorHost предоставляет 2 конструктора, которые позволяют переопределить диспетчер контрольных точек для вашего EventProcessorHost.
