...
1. В разделе ценовой категории можно выбрать **бесплатный** уровень, чтобы выполнить инструкции в руководствах. Для более глубокого анализа с использованием собственных данных создайте [платную службу](https://azure.microsoft.com/pricing/details/search/), например с уровнем **Базовый** или **Стандартный**. 
  ...
  > [!NOTE]
  > Служба когнитивного поиска находится в общедоступной предварительной версии. Выполнение набора навыков в настоящее время доступно на всех уровнях, включая бесплатный. Цена на эту функцию будет объявлена позже.
...
Иногда требуется извлечь текстовое представление из файлов отсканированных изображений, например PDF-файлов, создаваемых сканером. Служба "Поиск Azure" может автоматически извлекать содержимое из внедренных изображений в документе. Для этого выберите **Enable OCR and merge all text into merged_content field** (Включить OCR и объединить весь текст в поле merged_content). Это автоматически создаст поле `merged_content`, которое содержит текст, извлеченный из документа, а также текстовое представление внедренный в документ изображений. При выборе этого параметра для `Source data field` будет задано значение `merged_content`.
