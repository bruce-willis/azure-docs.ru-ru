...
### <a name="secured-virtual-networks-with-service-endpoints"></a>Защищенные виртуальные сети с конечными точками службы 

Виртуальные сети позволяют ограничить трафик к ресурсам Azure, разрешив только определенные виды трафика. Вы можете указать конечные точки службы, чтобы расширить виртуальную сеть и включить в нее службу Azure Monitor. Это позволит вашим ресурсам и дальше безопасно отправлять журналы и метрики в Azure Monitor из виртуальных сетей.  

Дополнительные сведения см. в статье [Конечные точки службы виртуальной сети](../virtual-network/virtual-network-service-endpoints-overview.md). 
