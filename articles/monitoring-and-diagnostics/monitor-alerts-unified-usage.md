...
1.  Определите условие оповещения, используя ссылку **Выбор ресурса** и указав ресурс. Отфильтруйте и выберите *подписку, *тип ресурса, а затем выберите требуемый *ресурс*.
...
1. *Метрики оповещений*. Если сигнал является метрикой, в окне оповещений можно выполнять фильтрацию с использованием нескольких точек данных или измерений для указанного ресурса Azure. 
    ...
    b. **Логику оповещений** можно выбрать в одном из показанных параметров *Условие*, *Агрегирование и *Пороговое значение*. В окне предпросмотра логики отображаются условия визуализации вместе с историей сигнала, чтобы указать, когда оповещение было вызвано в прошлом. 
...
1. Отображается **панель мониторинга службы "Оповещения"**, в которой все оповещения Azure объединены и отображаются в единственной панели мониторинга. ![Панель мониторинга оповещений](./media/monitoring-alerts-unified-usage/alerts-preview-overview.png)
1. В верхнем левом углу щелкните панель мониторинга, чтобы просмотреть подробный список:
    ...
    - *Всего правил генерации оповещений*. Количество активных созданных правил оповещений. 
        ...
        > [!NOTE]
        > Чтобы обеспечить согласованность данных на панели мониторинга, содержащей сведения обо всех сработавших оповещениях, включая оповещения журналов для Application Insights и Log Analytics, рекомендуем использовать [расширенный унифицированный интерфейс оповещений (предварительная версия)](monitoring-overview-unified-alerts.md#enhanced-unified-alerts-public-preview)
  
  
1. Отображается список всех оповещений, которые можно щелкнуть, чтобы просмотреть сведения.
1. Чтобы отфильтровать определенные оповещения, можно использовать параметры раскрывающегося списка *Подписка, Группы ресурсов или Ресурс*. Для неразрешенных оповещений выберите параметр *Filter alert* (Оповещение фильтра) для поиска ключевого слова: *имя, критерии оповещения, группа ресурсов и целевой ресурс*.
