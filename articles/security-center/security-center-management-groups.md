...
После повышения прав доступа администратор клиента может назначать роль RBAC соответствующим пользователям на уровне корневой группы управления. Мы советуем назначать роль [**Читатель**](../role-based-access-control/built-in-roles.md#reader). Эта роль позволяет обеспечить видимость на уровне клиента. Назначенная роль автоматически применяется ко всем группам управления и подпискам в корневой группе управления. Дополнительные сведения о ролях RBAC см. в разделе [Available roles](../active-directory/active-directory-assign-admin-roles-azure-portal.md#available-roles) (Доступные роли).
...
После назначения ролей RBAC откройте или обновите центр безопасности, чтобы проверить, можете ли вы отслеживать все подписки в своем клиенте Azure AD. 

1. Войдите на [портале Azure](https://portal.azure.com). 
2. В главном меню Azure выберите **Все службы**, а затем щелкните **Центр безопасности**.
3. На странице **Обзор** приведена диаграмма покрытия подписок. 
...
4. Щелкните **Покрытие**, чтобы просмотреть список покрытия подписок. 
