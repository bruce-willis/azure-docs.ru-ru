...
* Инструменты машинного обучения
  ...
  * [Rattle](https://togaware.com/rattle/): графический инструмент, который облегчает начало работы с анализом данных и машинным обучением в R.
...
1. В нижней части страницы нажмите кнопку **Создать**. Откроется мастер.![configure-data-science-vm](./media/dsvm-ubuntu-intro/configure-data-science-virtual-machine.png)
1. В следующих разделах приведены входные данные для всех шагов мастера (номера см. справа на предыдущем рисунке) по созданию виртуальной машины Майкрософт для обработки и анализа данных. Ниже приведено описание входных данных, необходимых для настройки на каждом из шагов.
   ...
   * **Тип диска виртуальной машины** — выберите **SSD (цен. категория "Премиум")**, если требуется твердотельный накопитель (SSD). В противном случае выберите тип **HDD (цен. категория "Стандартный")**. 
   ...
   * Выберите один из типов серверов, который соответствует вашим функциональным требованиям и финансовым ограничениям. Выберите виртуальную машину класса NC или ND для экземпляров виртуальной машины на основе GPU. На странице [Доступность продуктов по регионам](https://azure.microsoft.com/global-infrastructure/services/) перечислены регионы, для которых поддерживается использование GPU.
   
   c. **Параметры**:
   
   * В большинстве случаев используются значения по умолчанию. Чтобы использовать значения, отличные от значений по умолчанию, наведите указатель мыши на информационную ссылку, чтобы получить справку по конкретному полю.
   
   d. **Сводка**:
   
   * Проверьте, все ли сведения введены правильно. Отобразится ссылка на условия обслуживания. За использование виртуальной машины не взимается дополнительная плата, кроме платы за размер сервера, выбранный на шаге **Размер** . Чтобы начать подготовку, щелкните **Создать**. 
   
Подготовка занимает примерно 5 минут. Состояние подготовки отображается на портале Azure.
...
1. X2Go для графических сеансов
1. JupyterHub и JupyterLab для записных книжек Jupyter
...
* Изучите систематический подход к созданию комплексных аналитических решений с помощью [группового процесса обработки и анализа данных](http://aka.ms/tdsp).
