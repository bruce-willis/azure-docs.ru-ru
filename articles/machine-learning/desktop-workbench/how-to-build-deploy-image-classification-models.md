...
В этой статье вы узнаете, как обучать, тестировать и развертывать модели классификации изображений с помощью пакета машинного обучения Azure для компьютерного зрения (AMLPCV). Общие сведения об этом пакете и подробную справочную документацию см. [здесь](https://aka.ms/aml-packages/vision).
