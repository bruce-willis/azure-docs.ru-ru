...
- Необходимо иметь доступ к компьютерам, оснащенным графическими процессорами NVidia.
    ...
    - а также в контейнере Docker на компьютере Linux с графическими процессорами.
...
При выборе расположения виртуальной машины обратите внимание, что виртуальные машины GPU доступны только в определенных регионах Azure, например в **центрально-южной части США**. Дополнительные сведения см. в статье [Доступность продуктов по регионам](https://azure.microsoft.com/regions/services/).
...
Откройте `my_dsvm.compute`. Измените `baseDockerImage` на `microsoft/mmlspark:plus-gpu-0.9.9` и добавьте новую строку `nvidiaDocker: true`. Таким образом, файл должен содержать следующие две строки:
