---
title: Руководство. Автоматическое машинное обучение модели классификации с помощью службы "Машинное обучение Azure"
description: Узнайте, как создать модель машинного обучения с помощью автоматического машинного обучения.  Машинное обучение Azure может автоматически выполнять предварительную обработку данных, а также выбор алгоритма и гиперпараметров. После этого окончательная модель развертываться с помощью службы "Машинное обучение Azure".
...
> * Обучение с помощью автоматического классификатора на локальном компьютере.
...
Все настройки для работы по разработке можно сделать в Python Notebook.  Программа установки включает:

* Импорт пакетов Python.
* Настройка рабочей области для обеспечения связи между локальным компьютером и удаленными ресурсами.
* Создание каталога для хранения сценариев обучения.
...
В существующей рабочей области создайте объект. `Workspace.from_config()` считывает файл **aml_config/config.json** и загружает данные в объект `ws`.  `ws` используется в остальном коде в этом руководстве.

Получив объект рабочей области, укажите имя эксперимента, а также создайте локальный каталог и зарегистрируйте его в рабочей области. Все выполнения указанного эксперимента записываются в журнал под его именем.
...
В руководстве по начальному обучению использовалась версия набора данных MNIST с высоким разрешением (28x28 пикселей).  Так как для автоматического машинного обучения требуется много итераций, в этом руководстве используется версия с меньшим разрешением изображений (8x8 пикселей). Это позволит продемонстрировать принцип работы и сократить время, необходимое для каждой итерации.  
...
Загрузите данные в массивы `numpy`. Затем с помощью `matplotlib` постройте график 30 случайных изображений из набора данных с подписями над ними.
...
Чтобы автоматически обучить модель, сначала определите параметры конфигурации эксперимента, а затем выполните эксперимент.


### <a name="define-settings"></a>Определение параметров

Определите параметры эксперимента и параметры модели.

|Свойство| Значение в этом руководстве |ОПИСАНИЕ|
|----|----|---|
|**primary_metric**|AUC Weighted | Метрика, который вы хотите оптимизировать.|
|**max_time_sec**|12 000|Максимальная длительность итерации в секундах.|
...
Запустите эксперимент, чтобы выполнить его в локальной среде. Определите локальный целевой объект вычислений и включите вывод результатов (выберите значение true), чтобы просмотреть ход выполнения эксперимента.
...
По мере выполнения итераций по одной будут появляться строки выходных данных, как показано ниже.  Новая строка будет появляться каждые **10–15 секунд**.
...
Ознакомьтесь с результатами эксперимента, воспользовавшись мини-приложением Jupyter или журналом эксперимента.
...
Вот как выглядит статическое изображение мини-приложения.  В Notebook для просмотра графика каждой доступной метрики для каждой итерации можно также использовать раскрывающийся список над графиком.

![таблица мини-приложения](./media/tutorial-auto-train-models/table.png)
![график мини-приложения](./media/tutorial-auto-train-models/graph.png)

### <a name="retrieve-all-iterations"></a>Получение всех итераций

Откройте журнал эксперимента и просмотрите отдельные метрики для каждой итерации.
...
## <a name="register-the-best-model"></a>Регистрация наилучшей модели 

Используйте объект `local_run`, чтобы получить наилучшую модель, и зарегистрируйте ее в рабочей области. 
...
## <a name="test-the-best-model"></a>Тестирование наилучшей модели

Используйте модель для прогнозирования нескольких случайных цифр.  Отобразите прогнозируемое значение и изображение.  Красный шрифт и обратное изображение (белое на черном фоне) используются для выделения неправильно классифицированных изображений.
...
> * Настройка среды разработки
> * Подключение и проверка данных
> * Обучение в локальной среде с помощью автоматического классификатора с настраиваемыми параметрами.
> * Просмотр результатов.
> * Проверка результатов обучения.
> * Регистрация наилучшей модели.

Узнайте больше о [настройке параметров автоматического обучения]() или [способах использования автоматического обучения в удаленном ресурсе](how-to-auto-train-remote.md).  
