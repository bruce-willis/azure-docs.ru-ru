...
Контекст хранилища используется для определения того, где будут храниться различные выходные файлы, такие как файлы модели DNN. Дополнительные сведения см. в [документации по StorageContext](https://docs.microsoft.com/en-us/python/api/cvtk.core.context.storagecontext?view=azure-ml-py-latest). Как правило, содержимое хранилища необходимо задавать явно. Однако, чтобы избежать ограничения размера проекта Workbench на уровне 25 МБ, задайте каталог выходных данных для указания на расположение за пределами проекта AML ("../../../../cvtk_output"). Не забудьте удалить каталог cvtk_output, когда он перестанет использоваться.
