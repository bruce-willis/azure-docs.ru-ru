...
### <a name="counts-for-large-numbers-of-tasks"></a>Счетчики для большого количества задач

Операция Get Task Counts возвращает количество состояний выполнения задач в системе в определенный момент времени. Если ваше задание содержит большое количество задач, подсчеты, возвращаемые операцией Get Task Counts, могут передавать реальное состояние задач с задержкой в несколько секунд. Пакетная служба обеспечивает итоговую согласованность между результатами операции Get Task Counts и фактическим количеством состояний задач (которое вы можете запросить через API перечисления задач). Однако, если ваше задание содержит очень большое количество задач (> 200 000), мы рекомендуем вместо этого использовать API перечисления задач и [фильтрованный запрос](batch-efficient-list-queries.md), которые предоставляют более актуальные сведения. 
