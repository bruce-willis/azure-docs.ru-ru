...
В Maya и 3ds Max можно выполнять задания с помощью классического приложения [Batch Explorer](https://github.com/Azure/BatchExplorer) или [интерфейса командной строки для шаблонов Пакетной службы](batch-cli-templates.md). Интерфейс командной строки пакетной службы Azure позволяет выполнять пакетные задания без написания кода. Вместо этого вы можете создавать пулы, задания и задачи пакетной службы с помощью файлов шаблонов. Дополнительные сведения см. в статье [Использование шаблонов интерфейса командной строки для пакетной службы Azure и передачи файлов (предварительная версия)](batch-cli-templates.md).
...
- **Batch Explorer** (необязательно). [Azure Batch Explorer](https://azure.github.io/BatchExplorer) (ранее BatchLabs) — это бесплатное автономное клиентское средство с множеством функций для создания, отладки и мониторинга приложений в Пакетной службе Azure. Не обязательно использовать службу рендеринга, но она пригодится при разработке и отладке решений пакетной службы.
...
Azure предоставляет образы Windows и Linux, каждый из которых имеет отдельную предварительно установленную и готовую к использованию версию Maya, 3ds Max, Arnold и V-Ray. При создании пула образы можно выбрать на [портале Azure](https://portal.azure.com), в подключаемом модуле Maya или в [Batch Explorer](https://azure.github.io/BatchExplorer).

На портале Azure и в Batch Explorer один из образов виртуальной машины с предварительно установленными приложениями можно установить следующим образом. В учетной записи Пакетной службы в разделе пулов выберите **Создать**, а затем в разделе **Добавить пул** из раскрывающегося списка **Тип образа** выберите **Graphics and Rendering (Linux/Windows)** (Графика и рендеринг (Linux или Windows)).
...
- классическое приложение [Batch Explorer](https://azure.github.io/BatchExplorer);
- [интерфейс командной строки для шаблонов Пакетной службы](batch-cli-templates.md).

### <a name="3ds-max"></a>3ds Max

С 3ds Max можно использовать следующие средства:

- классическое приложение [Batch Explorer](https://azure.github.io/BatchExplorer) (руководство по использованию шаблонов 3ds Max см. в репозитории с [данными Batch Explorer](https://github.com/Azure/BatchExplorer-data/tree/master/ncj/3dsmax));
- [интерфейс командной строки для шаблонов Пакетной службы](batch-cli-templates.md).

Шаблоны 3ds Max Batch Labs позволяют преобразовать сцены VRay и Arnold для просмотра с помощью службы отрисовки пакетной службы. Имеется два варианта шаблона для VRay и Arnold: для стандартных сцен и для более сложных сцен, требующих файл пути 3ds Max к ресурсам и текстурам (MXP-файл). Дополнительные сведения о шаблонах 3ds Max см. в репозитории [данных Batch Explorer](https://github.com/Azure/BatchExplorer-data/tree/master/ncj/3dsmax) на сайте GitHub.
