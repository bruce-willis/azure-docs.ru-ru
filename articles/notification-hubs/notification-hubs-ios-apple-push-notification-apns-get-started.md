...
Можно проверить, поступают ли в приложение уведомления, с помощью параметра *Тестовая отправка* на [портала Azure]. Этот параметр позволяет отправить на устройство тестовое push-уведомление.
...
2. Отправьте тестовое push-уведомление с [портала Azure], как описано в предыдущем разделе.
