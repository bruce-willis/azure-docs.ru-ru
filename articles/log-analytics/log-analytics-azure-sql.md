...
Сейчас эта предварительная версия решения поддерживает 200 000 Баз данных SQL Azure и 5000 эластичных пулов SQL на каждую рабочую область.
...
Аналитика SQL Azure — это облачное решение только для мониторинга с поддержкой потоковой передачи диагностических данных телеметрии для Баз данных SQL Azure, эластичных пулов и Управляемых экземпляров. Так как эта служба не использует агенты для подключения к Log Analytics, она не поддерживает мониторинг локальных серверов SQL или виртуальных машин (сведения о совместимости см. в следующей таблице).
...
*Высокий уровень ЦП в эластичном пуле Базы данных SQL Azure*
