...
В конечном счете стратегия обращения с персональными данными определяется вами и вашей компанией (если она вообще нужна). Мы здесь лишь предлагаем некоторые из возможных подходов к ней. Варианты перечислены в порядке убывания предпочтительности с технической точки зрения.
...
Для всех запросов просмотра и экспорта данных следует использовать [API-интерфейс запросов](https://dev.loganalytics.io/). Вам необходимо самостоятельно создать логику преобразования данных в требуемый вид для предоставления пользователям. [Функции Azure](https://azure.microsoft.com/services/functions/) отлично подходят для размещения такой логики.
