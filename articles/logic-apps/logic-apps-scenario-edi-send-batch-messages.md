...
1. В конструкторе Logic Apps добавьте триггер **Пакет**, который запускает рабочий процесс приложения логики. В поле поиска введите слово "пакет" в качестве фильтра. Выберите триггер **Пакет — Пакетные сообщения**.

   ![Добавление триггера "Пакет"](./media/logic-apps-scenario-EDI-send-batch-messages/add-batch-receiver-trigger.png)

1. Задайте имя пакета и укажите критерии для его отправки, например:
...
1. Добавьте еще одно действие, которое кодирует сообщения, объединенные в группы или пакеты, и создает пакетированное сообщение X12. 
...
1. Задайте свойства для только что добавленного действия.
...
1. Для тестирования добавьте действие HTTP, чтобы отправить пакетированное сообщение в [службу RequestBin](https://requestbin.fullcontact.com/). 
   ...
   1. В списке **Метод** выберите **POST**. Для поля **Uri** создайте универсальный код ресурса (URI) с помощью службы RequestBin и введите полученный URI. В поле **Текст**, когда откроется динамический список, выберите значение **Текст** в разделе **Пакетная кодировка по имени соглашения**. Если значение **Текст** не отображается, то щелкните **Показать больше** рядом с пунктом **Пакетная кодировка по имени соглашения**.
...
1.  Когда получающее приложение логики создано, сохраните это приложение.
...
1. Добавьте новый шаг для отправки сообщений в пакет.

   1. Выберите **+ Новый шаг** > **Добавить действие**.

   1. В поле поиска введите слово "пакет" в качестве фильтра. 

1. Выберите действие **Отправить сообщения в пакет — Выберите рабочий процесс Logic Apps с триггером пакета**.
...
1. Теперь выберите приложение логики BatchX12Messages, которое вы создали ранее и которое теперь доступно как действие.
