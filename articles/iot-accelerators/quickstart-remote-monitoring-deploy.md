...
ms.locfileid: "37088033"
---
# <a name="quickstart-deploy-a-cloud-based-remote-monitoring-solution"></a>Краткое руководство. Развертывание облачного решения удаленного мониторинга

В этом кратком руководстве показано, как развернуть акселератор решения для удаленного мониторинга Интернета вещей в Azure для использования в качестве облачного решения удаленного мониторинга для устройств Интернета вещей. После развертывания акселератора решения мы зайдем на страницу **панели мониторинга**, чтобы понаблюдать за имитированными устройствами на карте и реакцией страницы **обслуживания** на оповещение о давлении в имитированном холодильнике.

Развертывание по умолчанию настраивает акселератор решения для удаленного мониторинга для компании Contoso. Contoso управляет различными устройствами, например холодильниками, в различных физических средах. Холодильник отправляет данные о температуре, влажности и давлении в акселератор решения для удаленного мониторинга.
...
Когда вы будете разворачивать акселератор решения в подписке Azure, необходимо указать некоторые параметры конфигурации.

Войдите на сайт [azureiotsolutions.com](https://www.azureiotsolutions.com/Accelerators) с использованием учетных данных учетной записи Azure.
...
Укажите уникальное **имя решения** для акселератора решения для удаленного мониторинга.

Выберите **подписку** и **регион**, которые вы хотите использовать для развертывания акселератора решения. Вы можете выбрать ближайший к вам регион. Вы должны быть [глобальным администратором или пользователем](iot-accelerators-permissions.md) в подписке.
...
После развертывания в подписке Azure вы сможете войти на панель мониторинга акселератора решения для удаленного мониторинга.
...
На панели мониторинга решения отображаются следующие сведения об устройствах компании Contoso:
...
На панели **Задания** выберите **Запустить метод**, а затем метод **EmergencyValveRelease**, добавьте имя задания **ChillerPressureRelease** и нажмите кнопку **Применить**. Эти параметры создают задание, которое будет выполнено немедленно.
...
Чтобы узнать, как обновить встроенное ПО на подключенных устройствах и упорядочить активы в акселераторе решений, перейдите к следующему руководству.
